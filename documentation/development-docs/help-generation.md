...
`New-MarkdownCommandHelp` writes into an `<ModuleName>` subfolder beneath `-OutputFolder`, so the files land in `$PathToHelpFolder/{{moduleName}}/`. In this repo, cmdlet markdown files are stored directly under the module's `help/` folder, so move the generated `*.md` files from `$PathToHelpFolder/{{moduleName}}/` up into `$PathToHelpFolder/` and remove the now-empty subfolder before committing.

Once the `help` folder has been generated, follow the steps provided in the below section to update the files with any changes made to the public interface of the cmdlets.
