if(($null -eq $TestName) -or ($TestName -contains 'Remove-AzPaloAltoNetworksMetricsObjectFirewall'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Remove-AzPaloAltoNetworksMetricsObjectFirewall.Recording.json'
...
Describe 'Remove-AzPaloAltoNetworksMetricsObjectFirewall' {
    It 'Delete' {
        # Create a metrics object first for deletion test
        ...
        # Now delete it
        { Remove-AzPaloAltoNetworksMetricsObjectFirewall -FirewallName "italynorth-test-fw" -ResourceGroupName "eastus-rg" -PassThru } | Should -Not -Throw
    }
