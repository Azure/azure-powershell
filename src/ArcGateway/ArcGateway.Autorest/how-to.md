# How-To
This document describes how to develop for `Az.ArcGateway`.

## Building `Az.ArcGateway`
...
There are multiple scripts created for performing different actions for developing `Az.ArcGateway`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.ArcGateway.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.ArcGateway.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.ArcGateway`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.ArcGateway` into the session.
