# How-To
This document describes how to develop for `Az.DeviceRegistry`.

## Building `Az.DeviceRegistry`
...
There are multiple scripts created for performing different actions for developing `Az.DeviceRegistry`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DeviceRegistry.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DeviceRegistry.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DeviceRegistry`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DeviceRegistry` into the session.
