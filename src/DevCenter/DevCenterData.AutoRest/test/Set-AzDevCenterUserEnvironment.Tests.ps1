...
Describe 'Set-AzDevCenterUserEnvironment' {
    ...
    It 'Replace' -skip {
        ...
        $environment = Set-AzDevCenterUserEnvironment -Endpoint $env.endpoint -Name "envtest3" -ProjectName $env.projectName -Body $functionAppBody
        $environment.CatalogName | Should -Be $env.catalogName
        $environment.DefinitionName | Should -Be $env.functionApp
        ...
        $environment = Set-AzDevCenterUserEnvironment -DevCenter $env.devCenterName -Name "envtest4" -ProjectName $env.projectName -Body $sandboxBody
        $environment.CatalogName | Should -Be $env.catalogName
        $environment.DefinitionName | Should -Be $env.sandbox
    ...
    It 'ReplaceViaIdentityExpanded' -skip {
        ...
        $environment = Set-AzDevCenterUserEnvironment -Endpoint $env.endpoint -InputObject $envInput1 -Body $sandboxBody
        $environment.CatalogName | Should -Be $env.catalogName
        $environment.DefinitionName | Should -Be $env.sandbox
        ...
        $environment = Set-AzDevCenterUserEnvironment -DevCenter $env.devCenterName -InputObject $envInput2 -Body $functionAppBody
        $environment.CatalogName | Should -Be $env.catalogName
        $environment.DefinitionName | Should -Be $env.functionApp
    ...
    It 'ReplaceViaIdentity' -skip {
        ...
        $environment = Set-AzDevCenterUserEnvironment -Endpoint $env.endpoint -InputObject $envInput1 -CatalogName $env.catalogName -EnvironmentDefinitionName $env.sandbox -EnvironmentType $env.environmentTypeName
        $environment.CatalogName | Should -Be $env.catalogName
        $environment.DefinitionName | Should -Be $env.sandbox
        ...
        $environment = Set-AzDevCenterUserEnvironment -DevCenter $env.devCenterName -InputObject $envInput2 -CatalogName $env.catalogName -EnvironmentDefinitionName $env.functionApp -EnvironmentType $env.environmentTypeName -Parameter $functionAppParameters
        $environment.CatalogName | Should -Be $env.catalogName
        $environment.DefinitionName | Should -Be $env.functionApp
