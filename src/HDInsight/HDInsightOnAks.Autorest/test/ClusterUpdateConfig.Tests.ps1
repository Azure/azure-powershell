...
Describe 'ClusterUpdateConfig' {
    ...
    It "New AzHDInsightAksClusterServiceConfigsProfile"{
        ...
        [Console]::WriteLine("New-AzHDInsightAksClusterServiceConfigsProfile done")
    }

    It "Update AzHdInsightOnAksCluster"{
        { Update-AzHdInsightOnAksCluster -ResourceGroupName $clusterResourceGroupName -Location $location -PoolName $clusterpoolName -Name $clusterName -ClusterProfileServiceConfigsProfile $yarnServiceConfigProfile} | Should -Not -Throw
