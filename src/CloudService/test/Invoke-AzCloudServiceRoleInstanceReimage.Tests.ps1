...
. ($loadEnvPath)
$TestRecordingFile = Join-Path $PSScriptRoot 'Invoke-AzCloudServiceRoleInstanceReimage.Recording.json'
...
Describe 'Invoke-AzCloudServiceRoleInstanceReimage' {

    It 'Reimage cloud service role instance' {
        Invoke-AzCloudServiceRoleInstanceReimage -ResourceGroupName $env.ResourceGroupName -CloudServiceName $env.CloudServiceName -RoleInstanceName $env.RoleInstanceName
    }
