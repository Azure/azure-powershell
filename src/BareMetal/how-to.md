# How-To
This document describes how to develop for `Az.BareMetal`.

## Building `Az.BareMetal`
...
There are multiple scripts created for performing different actions for developing `Az.BareMetal`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.BareMetal.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.BareMetal.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.BareMetal`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.BareMetal` into the session.
