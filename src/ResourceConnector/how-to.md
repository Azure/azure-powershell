# How-To
This document describes how to develop for `Az.ResourceConnector`.

## Building `Az.ResourceConnector`
...
There are multiple scripts created for performing different actions for developing `Az.ResourceConnector`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.ResourceConnector.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.ResourceConnector.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.ResourceConnector`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.ResourceConnector` into the session.
