...
namespace Microsoft.Azure.Commands.Common.Authentication
{
    ...
    internal class UserTokenProvider : ITokenProvider
    {
        ...
        private bool IsExpired(AdalAccessToken token)
        {
            ...
            TracingAdapter.Information(Messages.UPNTokenExpirationCheckTrace, expiration, currentTime, expirationThreshold,
                timeUntilExpiration);
        ...
        private void Renew(AdalAccessToken token)
        {
            ...
            {
                TracingAdapter.Information(
                    Messages.UPNRenewTokenUserInfoTrace,
                    user.DisplayableId,
                    user.FamilyName,
                    user.GivenName,
            ...
            if (IsExpired(token))
            {
                TracingAdapter.Information(Messages.UPNExpiredTokenTrace);
                AuthenticationResult result = AcquireToken(token.Configuration, null, token.UserId, null, true);

                if (result == null)
                {
                    throw new AuthenticationException(Messages.ExpiredRefreshToken);
                }
                else
                {
            ...
            out Exception ex)
        {
            ...
            catch (AdalException adalEx)
            {
                ...
                {
                    string message = Messages.AdalUserInteractionRequired;
                    if (adalEx.ErrorCode == AdalError.MultipleTokensMatched)
                    {
                        message = Messages.AdalMultipleTokens;
                    }
                ...
                {
                    ex = new AadAuthenticationFailedException(Messages.CredentialOrganizationIdMessage, adalEx);
                }
                else
                {
                    ex = adalEx;
            ...
            bool renew = false)
        {
            ...
            TracingAdapter.Information(
                Messages.UPNAcquireTokenContextTrace,
                context.Authority,
                context.CorrelationId,
                context.ValidateAuthority);
            TracingAdapter.Information(
                Messages.UPNAcquireTokenConfigTrace,
                config.AdDomain,
                config.AdEndpoint,
                config.ClientId,
