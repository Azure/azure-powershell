...
using Microsoft.WindowsAzure.Commands.Storage.ServiceManagement.Model;

namespace Microsoft.WindowsAzure.Commands.Storage.Utilities.Common
{
    public abstract class CloudBaseCmdlet<T> : AzureDataCmdlet
        where T : class
    {
        ...
        protected virtual T CreateChannel()
        {
            ...
            string certificateThumbprint = DefaultContext.Account.Id;
            Debug.Assert(SMProfile.Accounts[certificateThumbprint].Type == AzureAccount.AccountType.Certificate);

            return ChannelHelper.CreateServiceManagementChannel<T>(
                ServiceBinding,
                SMProfile.Context.Environment.GetEndpointAsUri(AzureEnvironment.Endpoint.ServiceManagement),
        ...
        {
            RetryCall(DefaultContext.Subscription.Id, call);
        }
        ...
        protected void RetryCall(Guid subsId, Action<string> call)
        {
            ...
            catch (MessageSecurityException ex)
            {
                ...
                {
                    WriteError(new ErrorRecord(new Exception("Could not communicate with the service.", ex), string.Empty, ErrorCategory.InvalidData, null));
                }
                else
                {
                    throw;
        ...
        {
            return RetryCall(DefaultContext.Subscription.Id, call);
        }
