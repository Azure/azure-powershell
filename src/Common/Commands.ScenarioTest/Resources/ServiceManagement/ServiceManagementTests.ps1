...
function Run-AutoGeneratedVirtualMachineCmdletTests
{
    ...
    try
    {
        ...
        $st = Invoke-AzureComputeMethod -FunctionName 'HostedServiceCreate' -Parameter $svcCreateParams;
        Assert-AreEqual $st.StatusCode 'Created';
        Assert-NotNull $st.RequestId;

        # Invoke Hosted Service Get
        $svcGetResult = Invoke-AzureComputeMethod -FunctionName 'HostedServiceGet' -Parameter $svcName;
        Assert-AreEqual $svcGetResult.ServiceName $svcName;
        Assert-AreEqual $svcGetResult.Properties.Description $svcName;
        Assert-AreEqual $svcGetResult.Properties.Label $svcName;
        ...
        $st = Invoke-AzureComputeMethod -FunctionName 'VirtualMachineCreateDeployment' -Parameter $svcName,$vmDeployment;
        Assert-AreEqual $st.StatusCode 'OK';
        Assert-NotNull $st.RequestId;

        # Invoke Virtual Machine Get
        $st = Invoke-AzureComputeMethod -FunctionName 'VirtualMachineGet' -Parameter $svcName,$svcName,$svcName;
        Assert-AreEqual $st.RoleName $svcName;

        # Invoke Hosted Service Delete
        $st = Invoke-AzureComputeMethod -FunctionName 'HostedServiceDeleteAll' -Parameter $svcName;
        Assert-AreEqual $st.StatusCode 'OK';
        Assert-NotNull $st.RequestId;
    }
    finally
