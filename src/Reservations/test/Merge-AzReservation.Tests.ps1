...
function ExecuteTestCases([object]$response) {
    ...
    $response[2].MergeProperties.MergeSources.Count | Should -Contain $reservationId1
    $response[2].MergeProperties.MergeSources.Count | Should -Contain $reservationId2
}

Describe 'Merge-AzReservation' {
    It 'MergeExpanded' {
        $reservationIds = @($reservationId1, $reservationId2)
        $res = Merge-AzReservation -ReservationOrderId "30000000-aaaa-bbbb-cccc-200000000013" -ReservationId $reservationIds
    }

    It 'Merge' {
        $reservationIds = @($reservationId1, $reservationId2)
        $mergeRequest = @{
            Source = $reservationIds
        }
        $res = Merge-AzReservation -ReservationOrderId "30000000-aaaa-bbbb-cccc-200000000013" -Body $mergeRequest
    }
    ...
    It 'MergeViaIdentityExpanded' {
        ...
        $res = Merge-AzReservation -InputObject $param -Body $mergeRequest
    }
    ...
    It 'MergeViaIdentity' {
        ...
        $reservationIds = @($reservationId1, $reservationId2)
        $res = Merge-AzReservation -InputObject $param -ReservationId $reservationIds
    }
}
