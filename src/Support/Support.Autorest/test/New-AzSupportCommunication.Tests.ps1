...
Describe 'New-AzSupportCommunication' {
    It 'CreateExpanded' -Skip:!$env.HasSubscription{
        ...
        # }
        # else{
            $supportMessage = New-AzSupportCommunication -Name $env.communicationNameForCreate -SupportTicketName $env.Name -Body $env.Body -Sender $env.Sender -Subject $env.Subject
