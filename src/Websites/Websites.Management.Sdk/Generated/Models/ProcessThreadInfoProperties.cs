// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) AutoRest Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

namespace Microsoft.Azure.Management.WebSites.Models
{
    using System.Linq;

    /// <summary>
    /// ProcessThreadInfo resource specific properties
    /// </summary>
    public partial class ProcessThreadInfoProperties
    {
        /// <summary>
        /// Initializes a new instance of the ProcessThreadInfoProperties class.
        /// </summary>
        public ProcessThreadInfoProperties()
        {
            CustomInit();
        }

        /// <summary>
        /// Initializes a new instance of the ProcessThreadInfoProperties class.
        /// </summary>

        /// <param name="identifier">Site extension ID.
        /// </param>

        /// <param name="href">HRef URI.
        /// </param>

        /// <param name="process">Process URI.
        /// </param>

        /// <param name="startAddress">Start address.
        /// </param>

        /// <param name="currentPriority">Current thread priority.
        /// </param>

        /// <param name="priorityLevel">Thread priority level.
        /// </param>

        /// <param name="basePriority">Base priority.
        /// </param>

        /// <param name="startTime">Start time.
        /// </param>

        /// <param name="totalProcessorTime">Total processor time.
        /// </param>

        /// <param name="userProcessorTime">User processor time.
        /// </param>

        /// <param name="state">Thread state.
        /// </param>

        /// <param name="waitReason">Wait reason.
        /// </param>
        public ProcessThreadInfoProperties(int? identifier = default(int?), string href = default(string), string process = default(string), string startAddress = default(string), int? currentPriority = default(int?), string priorityLevel = default(string), int? basePriority = default(int?), System.DateTime? startTime = default(System.DateTime?), string totalProcessorTime = default(string), string userProcessorTime = default(string), string state = default(string), string waitReason = default(string))

        {
            this.Identifier = identifier;
            this.Href = href;
            this.Process = process;
            this.StartAddress = startAddress;
            this.CurrentPriority = currentPriority;
            this.PriorityLevel = priorityLevel;
            this.BasePriority = basePriority;
            this.StartTime = startTime;
            this.TotalProcessorTime = totalProcessorTime;
            this.UserProcessorTime = userProcessorTime;
            this.State = state;
            this.WaitReason = waitReason;
            CustomInit();
        }

        /// <summary>
        /// An initialization method that performs custom operations like setting defaults
        /// </summary>
        partial void CustomInit();


        /// <summary>
        /// Gets site extension ID.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "identifier")]
        public int? Identifier {get; private set; }

        /// <summary>
        /// Gets or sets hRef URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "href")]
        public string Href {get; set; }

        /// <summary>
        /// Gets or sets process URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "process")]
        public string Process {get; set; }

        /// <summary>
        /// Gets or sets start address.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "start_address")]
        public string StartAddress {get; set; }

        /// <summary>
        /// Gets or sets current thread priority.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "current_priority")]
        public int? CurrentPriority {get; set; }

        /// <summary>
        /// Gets or sets thread priority level.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "priority_level")]
        public string PriorityLevel {get; set; }

        /// <summary>
        /// Gets or sets base priority.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "base_priority")]
        public int? BasePriority {get; set; }

        /// <summary>
        /// Gets or sets start time.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "start_time")]
        public System.DateTime? StartTime {get; set; }

        /// <summary>
        /// Gets or sets total processor time.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "total_processor_time")]
        public string TotalProcessorTime {get; set; }

        /// <summary>
        /// Gets or sets user processor time.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "user_processor_time")]
        public string UserProcessorTime {get; set; }

        /// <summary>
        /// Gets or sets thread state.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "state")]
        public string State {get; set; }

        /// <summary>
        /// Gets or sets wait reason.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "wait_reason")]
        public string WaitReason {get; set; }
    }
}