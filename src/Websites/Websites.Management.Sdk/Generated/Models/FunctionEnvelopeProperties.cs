// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) AutoRest Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

namespace Microsoft.Azure.Management.WebSites.Models
{
    using System.Linq;

    /// <summary>
    /// FunctionEnvelope resource specific properties
    /// </summary>
    public partial class FunctionEnvelopeProperties
    {
        /// <summary>
        /// Initializes a new instance of the FunctionEnvelopeProperties class.
        /// </summary>
        public FunctionEnvelopeProperties()
        {
            CustomInit();
        }

        /// <summary>
        /// Initializes a new instance of the FunctionEnvelopeProperties class.
        /// </summary>

        /// <param name="functionAppId">Function App ID.
        /// </param>

        /// <param name="scriptRootPathHref">Script root path URI.
        /// </param>

        /// <param name="scriptHref">Script URI.
        /// </param>

        /// <param name="configHref">Config URI.
        /// </param>

        /// <param name="testDataHref">Test data URI.
        /// </param>

        /// <param name="secretsFileHref">Secrets file URI.
        /// </param>

        /// <param name="href">Function URI.
        /// </param>

        /// <param name="config">Config information.
        /// </param>

        /// <param name="files">File list.
        /// </param>

        /// <param name="testData">Test data used when testing via the Azure Portal.
        /// </param>

        /// <param name="invokeUrlTemplate">The invocation URL
        /// </param>

        /// <param name="language">The function language
        /// </param>

        /// <param name="isDisabled">Gets or sets a value indicating whether the function is disabled
        /// </param>
        public FunctionEnvelopeProperties(string functionAppId = default(string), string scriptRootPathHref = default(string), string scriptHref = default(string), string configHref = default(string), string testDataHref = default(string), string secretsFileHref = default(string), string href = default(string), object config = default(object), System.Collections.Generic.IDictionary<string, string> files = default(System.Collections.Generic.IDictionary<string, string>), string testData = default(string), string invokeUrlTemplate = default(string), string language = default(string), bool? isDisabled = default(bool?))

        {
            this.FunctionAppId = functionAppId;
            this.ScriptRootPathHref = scriptRootPathHref;
            this.ScriptHref = scriptHref;
            this.ConfigHref = configHref;
            this.TestDataHref = testDataHref;
            this.SecretsFileHref = secretsFileHref;
            this.Href = href;
            this.Config = config;
            this.Files = files;
            this.TestData = testData;
            this.InvokeUrlTemplate = invokeUrlTemplate;
            this.Language = language;
            this.IsDisabled = isDisabled;
            CustomInit();
        }

        /// <summary>
        /// An initialization method that performs custom operations like setting defaults
        /// </summary>
        partial void CustomInit();


        /// <summary>
        /// Gets or sets function App ID.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "function_app_id")]
        public string FunctionAppId {get; set; }

        /// <summary>
        /// Gets or sets script root path URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "script_root_path_href")]
        public string ScriptRootPathHref {get; set; }

        /// <summary>
        /// Gets or sets script URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "script_href")]
        public string ScriptHref {get; set; }

        /// <summary>
        /// Gets or sets config URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "config_href")]
        public string ConfigHref {get; set; }

        /// <summary>
        /// Gets or sets test data URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "test_data_href")]
        public string TestDataHref {get; set; }

        /// <summary>
        /// Gets or sets secrets file URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "secrets_file_href")]
        public string SecretsFileHref {get; set; }

        /// <summary>
        /// Gets or sets function URI.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "href")]
        public string Href {get; set; }

        /// <summary>
        /// Gets or sets config information.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "config")]
        public object Config {get; set; }

        /// <summary>
        /// Gets or sets file list.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "files")]
        public System.Collections.Generic.IDictionary<string, string> Files {get; set; }

        /// <summary>
        /// Gets or sets test data used when testing via the Azure Portal.
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "test_data")]
        public string TestData {get; set; }

        /// <summary>
        /// Gets or sets the invocation URL
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "invoke_url_template")]
        public string InvokeUrlTemplate {get; set; }

        /// <summary>
        /// Gets or sets the function language
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "language")]
        public string Language {get; set; }

        /// <summary>
        /// Gets or sets gets or sets a value indicating whether the function is
        /// disabled
        /// </summary>
        [Newtonsoft.Json.JsonProperty(PropertyName = "isDisabled")]
        public bool? IsDisabled {get; set; }
    }
}