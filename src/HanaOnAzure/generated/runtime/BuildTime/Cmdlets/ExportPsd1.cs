...
using static Microsoft.Azure.PowerShell.Cmdlets.HanaOnAzure.Runtime.PowerShell.PsHelpers;
  ...
  public class ExportPsd1 : PSCmdlet
  {
    ...
    protected override void ProcessRecord()
    {
      try
      {
        ...
        sb.AppendLine($@"{Indent}RootModule = '{"./Az.HanaOnAzure.psm1"}'");
        ...
        sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: HanaOn cmdlets"}'");
        sb.AppendLine($@"{Indent}PowerShellVersion = '5.1'");
        sb.AppendLine($@"{Indent}DotNetFrameworkVersion = '4.7.2'");
        sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.HanaOnAzure.private.dll"}'");
        ...
        var formatList = customFormatPs1xmlFiles.Prepend("./Az.HanaOnAzure.format.ps1xml").ToPsList();
        ...
        sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule HanaOn".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
