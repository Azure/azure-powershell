...
using static Microsoft.Azure.PowerShell.Cmdlets.AD.Runtime.PowerShell.PsHelpers;

namespace Microsoft.Azure.PowerShell.Cmdlets.AD.Runtime.PowerShell
{
  ...
  public class ExportPsd1 : PSCmdlet
  {
    ...
    protected override void ProcessRecord()
    {
      try
      {
        ...
        sb.AppendLine($@"{Indent}RootModule = '{"./Az.AD.psm1"}'");
        ...
        sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: Ad cmdlets"}'");
        sb.AppendLine($@"{Indent}PowerShellVersion = '5.1'");
        sb.AppendLine($@"{Indent}DotNetFrameworkVersion = '4.7.2'");
        sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.AD.private.dll"}'");
        ...
        var formatList = customFormatPs1xmlFiles.Prepend("./Az.AD.format.ps1xml").ToPsList();
        ...
        sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule Ad".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
