# How-To
This document describes how to develop for `Policy`.
...
There are multiple scripts created for performing different actions for developing `Policy`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Policy.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Policy.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Policy`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Policy` into the session.
