...
Describe 'PolicyExemptionCRUD' {
    ...
    It 'Make policy exemption' {
        ...
        $exemption.Id | Should -Be "$($rg.ResourceId)/providers/Microsoft.Authorization/policyExemptions/$testExemption"
    ...
    AfterAll {
        # clean up
        $remove = Remove-AzPolicyExemption -Name $testExemption -Scope $rg.ResourceId -Force -PassThru
        $remove = (Remove-AzPolicyExemption -Name $testExemption2 -Scope $rg.ResourceId -Force -PassThru) -and $remove
        $remove = (Remove-AzPolicyAssignment -Name $testPA -Scope $rg.ResourceId -PassThru) -and $remove
