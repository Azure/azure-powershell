...
Describe 'PolicyExemptionCRUD' -Tag 'LiveOnly' {
    ...
    BeforeAll {
        ...
        $rgname = $env.rgname
        $assignment = New-AzPolicyAssignment -Name $testPA -PolicyDefinition $policy -Scope $env.scope -DisplayName $description
    ...
    It 'Make policy exemption' {
        ...
        $exemption = New-AzPolicyExemption -Name $testExemption -PolicyAssignment $assignment -Scope $env.scope -ExemptionCategory Waiver -Description $description -DisplayName $description -Metadata $metadata
        $exemption.Name | Should -Be $testExemption
        $exemption.Type | Should -Be 'Microsoft.Authorization/policyExemptions'
        $exemption.Id | Should -Be "$($env.scope)/providers/Microsoft.Authorization/policyExemptions/$testExemption"
    ...
    It 'Get policy exemption by name' {
        ...
        $exemption = Get-AzPolicyExemption -Name $testExemption -Scope $env.scope
        $exemption.Name | Should -Be $testExemption
        $exemption.Type | Should -Be 'Microsoft.Authorization/policyExemptions'
        $exemption.Id | Should -Be "$($env.scope)/providers/Microsoft.Authorization/policyExemptions/$testExemption"
        $exemption.PolicyAssignmentId | Should -Be $assignment.Id
    ...
    It 'List policy exemptions by scope' {
        ...
        $exemption2 = New-AzPolicyExemption -Name $testExemption2 -PolicyAssignment $assignment -Scope $env.scope -ExemptionCategory Mitigated -ExpiresOn $future2
        $list = Get-AzPolicyExemption -Scope $env.scope | ?{ $_.Name -in @($testExemption, $testExemption2) }
    ...
    AfterAll {
        # clean up
        $remove = Remove-AzPolicyExemption -Name $testExemption -Scope $env.scope -Force -PassThru
        $remove = (Remove-AzPolicyExemption -Name $testExemption2 -Scope $env.scope -Force -PassThru) -and $remove
        $remove = (Remove-AzPolicyAssignment -Name $testPA -Scope $env.scope -PassThru) -and $remove
