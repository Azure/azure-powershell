...
. (Join-Path $PSScriptRoot 'Common.ps1') 'PolicyDefinitionVersionWithParameters'

Describe 'PolicyDefinitionVersionWithParameters' {
    ...
    It 'make a policy definition with parameters on the command line' {
        ...
        $actual = New-AzPolicyDefinition -Name $testPDWP -Policy "$testFilesFolder\SamplePolicyDefinitionWithParameters.json" -Parameter $fullParameterDefinition -Description $description -Version $someNewVersion
