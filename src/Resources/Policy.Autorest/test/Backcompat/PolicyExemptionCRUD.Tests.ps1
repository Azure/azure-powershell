...
Describe 'Backcompat-PolicyExemptionCRUD' -Tag 'LiveOnly' {
    ...
    BeforeAll {
        ...
        $rgname = $env.rgname
        $assignment = New-AzPolicyAssignment -Name $testPA -PolicyDefinition $policy -Scope $env.scope -DisplayName $description -BackwardCompatible
    ...
    It 'Make policy exemption' {
        {
            ...
            $exemption = New-AzPolicyExemption -Name $testExemption -PolicyAssignment $assignment -Scope $env.scope -ExemptionCategory Waiver -Description $description -DisplayName $description -Metadata $metadata -BackwardCompatible
            ...
            Assert-AreEqual "$($env.scope)/providers/Microsoft.Authorization/policyExemptions/$testExemption" $exemption.ResourceId
    ...
    It 'Get policy exemption by name' {
        {
            ...
            $exemption = Get-AzPolicyExemption -Name $testExemption -Scope $env.scope -BackwardCompatible
            Assert-AreEqual $testExemption $exemption.Name 
            Assert-AreEqual Microsoft.Authorization/policyExemptions $exemption.ResourceType
            Assert-AreEqual "$($env.scope)/providers/Microsoft.Authorization/policyExemptions/$testExemption" $exemption.ResourceId
            Assert-AreEqual $assignment.ResourceId $exemption.Properties.PolicyAssignmentId
    ...
    It 'List policy exemptions by scope' {
        {
            ...
            $exemption2 = New-AzPolicyExemption -Name $testExemption2 -PolicyAssignment $assignment -Scope $env.scope -ExemptionCategory Mitigated -ExpiresOn $future2 -BackwardCompatible
            $list = Get-AzPolicyExemption -Scope $env.scope -BackwardCompatible | ?{ $_.Name -in @($testExemption, $testExemption2) }
    ...
    AfterAll {
        # clean up
        $remove = Remove-AzPolicyExemption -Name $testExemption -Scope $env.scope -Force -BackwardCompatible
        $remove = (Remove-AzPolicyExemption -Name $testExemption2 -Scope $env.scope -Force -BackwardCompatible) -and $remove
        $remove = (Remove-AzPolicyAssignment -Name $testPA -Scope $env.scope -BackwardCompatible) -and $remove
