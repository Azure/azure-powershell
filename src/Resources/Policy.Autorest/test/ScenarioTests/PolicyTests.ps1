...
$testFolder = 'D:\source\git\repos\azure-powershell\src\Resources\Policy.Autorest\test\ScenarioTests'
...
function Test-PolicyDefinitionCRUD
{
    ...
    $expected = New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinition.json" -Mode Indexed -Description $description
    ...
    $actual = Set-AzPolicyDefinition -Name $policyName -DisplayName testDisplay -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    ...
    $expected = New-AzPolicyDefinition -Name test3 -Policy "$testFolder\SamplePolicyDefinitionFromExport.json" -Description $description
...
function Test-PolicyDefinitionMode
{
    ...
    $expected = New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinition.json" -Mode All -Description $description
    $actual = Get-AzPolicyDefinition -Name $policyName
    Assert-NotNull $actual
    Assert-AreEqual $expected.Name $actual.Name
    Assert-AreEqual $expected.PolicyDefinitionId $actual.PolicyDefinitionId
    ...
    $actual = Set-AzPolicyDefinition -Name $policyName -DisplayName testDisplay -Mode 'All' -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    $actual = Set-AzPolicyDefinition -Name $policyName -DisplayName testDisplay -Mode 'Indexed' -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    # make a policy definition with non-default mode, get it back and validate
    $expected = New-AzPolicyDefinition -ManagementGroupName $managementGroup -Name $policyName -Policy "$testFolder\SamplePolicyDefinition.json" -Mode All -Description $description
    ...
    # update the same policy definition without touching mode, get it back and validate
    $actual = Set-AzPolicyDefinition -ManagementGroupName $managementGroup -Name $policyName -DisplayName testDisplay -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    ...
    # update the same policy definition explicitly providing the same mode, get it back and validate
    $actual = Set-AzPolicyDefinition -ManagementGroupName $managementGroup -Name $policyName -DisplayName testDisplay -Mode 'All' -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -ManagementGroupName $managementGroup -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    # update the same policy definition explicitly providing a different mode, get it back and validate
    $actual = Set-AzPolicyDefinition -ManagementGroupName $managementGroup -Name $policyName -DisplayName testDisplay -Mode 'Indexed' -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -ManagementGroupName $managementGroup -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    # make a policy definition with non-default mode, get it back and validate
    $expected = New-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -Policy "$testFolder\SamplePolicyDefinition.json" -Mode All -Description $description
    ...
    # update the same policy definition without touching mode, get it back and validate
    $actual = Set-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -DisplayName testDisplay -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    ...
    # update the same policy definition explicitly providing the same mode, get it back and validate
    $actual = Set-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -DisplayName testDisplay -Mode 'All' -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    # update the same policy definition explicitly providing a different mode, get it back and validate
    $actual = Set-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -DisplayName testDisplay -Mode 'Indexed' -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    $expected = New-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -Policy "$testFolder\SampleKeyVaultDataPolicyDefinition.json" -Mode 'Microsoft.KeyVault.Data' -Description $description
    $actual = Get-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName
    Assert-NotNull $actual
    Assert-AreEqual $expected.Name $actual.Name
    Assert-AreEqual $expected.PolicyDefinitionId $actual.PolicyDefinitionId
    ...
    # update the same policy definition without touching mode, get it back and validate
    $actual = Set-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -DisplayName testDisplay -Description $updatedDescription -Policy "$testFolder\SampleKeyVaultDataPolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
    ...
    # update the same policy definition explicitly providing the same mode, get it back and validate
    $actual = Set-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName -DisplayName testDisplay -Mode 'Microsoft.KeyVault.Data' -Description $updatedDescription -Policy "$testFolder\SampleKeyVaultDataPolicyDefinition.json" -Metadata $metadata
    $expected = Get-AzPolicyDefinition -SubscriptionId $subscriptionId -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    Assert-AreEqual $expected.Properties.Description $actual.Properties.Description
    Assert-NotNull($actual.Properties.Metadata)
...
function Test-PolicyDefinitionWithFullObject
{
    ...
    $actual = New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinitionObject.json"
    $expected = Get-AzPolicyDefinition -Name $policyName
    Assert-AreEqual $expected.Name $actual.Name
    Assert-AreEqual $expected.PolicyDefinitionId $actual.PolicyDefinitionId
    Assert-NotNull($actual.Properties.PolicyRule)
    ...
    $actual = New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinitionObject.json" -DisplayName testDisplay -Description $description -Mode Indexed -Metadata $metadata -Parameter "$testFolder\SamplePolicyDefinitionParameters.json"
    $expected = Get-AzPolicyDefinition -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
    ...
    New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinition.json"
    $actual = Set-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinitionObject.json"
    $expected = Get-AzPolicyDefinition -Name $policyName
    Assert-AreEqual $expected.Properties.DisplayName $actual.Properties.DisplayName
...
function Test-PolicyAssignmentCRUD
{
    ...
    $policyDefinition1 = New-AzPolicyDefinition -Name $policyDefName1 -Policy "$testFolder\SamplePolicyDefinition.json" -Description $description
    $policyDefinition2 = New-AzPolicyDefinition -Name $policyDefName2 -Policy "$testFolder\SamplePolicyDefinition.json" -Description $description
...
function Test-PolicyAssignmentAssignIdentity
{
    ...
    $policy = New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinition.json" -Description $description
...
function Test-PolicySetDefinitionCRUD
{
    ...
    $policyDefinition = New-AzPolicyDefinition -Name $policyDefName -Policy "$testFolder\SamplePolicyDefinition.json" -Description $description
...
function Test-PolicyDefinitionWithParameters
{
    ...
    $actual = New-AzPolicyDefinition -Name testPDWP -Policy "$testFolder\SamplePolicyDefinitionWithParameters.json" -Parameter "$testFolder\SamplePolicyDefinitionParameters.json" -Description $description
    ...
    $actual = New-AzPolicyDefinition -Name testPDWP -Policy "$testFolder\SamplePolicyDefinitionWithParameters.json" -Parameter $fullParameterDefinition -Description $description
    $expected = Get-AzPolicyDefinition -Name testPDWP
    Assert-AreEqual $expected.Name $actual.Name
    Assert-AreEqual $expected.PolicyDefinitionId $actual.PolicyDefinitionId
    Assert-NotNull($actual.Properties.PolicyRule)
...
function Test-PolicySetDefinitionWithParameters
{
    ...
    $policyDefinition = New-AzPolicyDefinition -Name $policyDefName -Policy "$testFolder\SamplePolicyDefinitionWithParameters.json" -Description $description -Parameter "$testFolder\SamplePolicyDefinitionParameters.json"
...
function Test-PolicyAssignmentWithParameters
{
    ...
    $rg = New-AzResourceGroup -Name $rgname -Location "west us"
    $policy = New-AzPolicyDefinition -Name $policyName -Policy "$testFolder\SamplePolicyDefinitionWithParameters.json" -Parameter "$testFolder\SamplePolicyDefinitionParameters.json" -Description $description
    ...
    $actual = New-AzPolicyAssignment -Name testPAWP -Scope $rg.ResourceId -PolicyDefinition $policy -PolicyParameter "$testFolder\SamplePolicyAssignmentParameters.json" -Description $description
    $expected = Get-AzPolicyAssignment -Name testPAWP -Scope $rg.ResourceId
    Assert-AreEqual $expected.Name $actual.Name
    Assert-AreEqual Microsoft.Authorization/policyAssignments $actual.ResourceType
    Assert-AreEqual $expected.PolicyAssignmentId $actual.PolicyAssignmentId
...
function Test-PolicyDefinitionCRUDAtManagementGroup
{
    ...
    # make a policy definition, get it back and validate
    $expected = New-AzPolicyDefinition -Name $policyName -ManagementGroupName $managementGroup -Policy "$testFolder\SamplePolicyDefinition.json" -Mode Indexed -Description $description
    ...
    # update the same policy definition, get it back and validate the new properties
    $actual = Set-AzPolicyDefinition -Name $policyName -ManagementGroupName $managementGroup -DisplayName testDisplay -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
    ...
    # do assertion after cleanup to help avoid leaving turds
    Assert-AreEqual 2 $list.Count
    #Assert-True { $list.Count -eq 2 }
}
...
function Test-PolicyDefinitionCRUDAtSubscription
{
    ...
    # make a policy definition, get it back and validate
    $expected = New-AzPolicyDefinition -Name $policyName -SubscriptionId $subscriptionId -Policy "$testFolder\SamplePolicyDefinition.json" -Mode Indexed -Description $description
    ...
    # update the same policy definition, get it back and validate the new properties
    $actual = Set-AzPolicyDefinition -Name $policyName -SubscriptionId $subscriptionId -DisplayName testDisplay -Description $updatedDescription -Policy "$testFolder\SamplePolicyDefinition.json" -Metadata $metadata
...
function Test-PolicySetDefinitionCRUDAtManagementGroup
{
    ...
    # make a policy definition and policy set definition that references it, get the policy set definition back and validate
    $policyDefinition = New-AzPolicyDefinition -Name $policyDefName -ManagementGroupName $managementGroup -Policy "$testFolder\SamplePolicyDefinition.json" -Description $description
    $policySet = "[{""policyDefinitionId"":""" + $policyDefinition.PolicyDefinitionId + """}]"
...
function Test-PolicySetDefinitionCRUDAtSubscription
{
    ...
    # make a policy definition and policy set definition that references it, get the policy set definition back and validate
    $policyDefinition = New-AzPolicyDefinition -Name $policyDefName -SubscriptionId $subscriptionId -Policy "$testFolder\SamplePolicyDefinition.json" -Description $description
    $policySet = "[{""policyDefinitionId"":""" + $policyDefinition.PolicyDefinitionId + """}]"
...
{
    # policy definitions
    $builtins = Get-AzPolicyDefinition -Builtin | Select-Object -First 50
    ...
    # policy set definitions
    $builtins = Get-AzPolicySetDefinition -Builtin | Select-Object -First 50
    foreach ($builtin in $builtins)
    {
        $setDefinition = Get-AzPolicySetDefinition -Name $builtin.Name
...
function Test-PolicyObjectPiping
{
    ...
    $policyDefinition = New-AzPolicyDefinition -Name $policyDefName -SubscriptionId $subscriptionId -Policy "$testFolder\SamplePolicyDefinitionObject.json" -Description $description
    $policySet = "[{""policyDefinitionId"":""" + $policyDefinition.PolicyDefinitionId + """}]"
    $expected = New-AzPolicySetDefinition -Name $policySetDefName -SubscriptionId $subscriptionId -PolicyDefinition $policySet -Description $description
