...
. (Join-Path $PSScriptRoot 'Common.ps1') 'PolicySetDefinitionVersionCRUDAtManagementGroup'

Describe 'PolicySetDefinitionVersionCRUDAtManagementGroup' -Tag 'LiveOnly' {
    ...
    BeforeAll {
        ...
        $baseDefinition = New-AzPolicySetDefinition -Name $policySetDefName -ManagementGroupName $managementGroup -PolicyDefinition $policySet -Description $description -Version $someNewVersion
    }

    It 'Get policy set definition at MG level' {
    ...
    It 'Make policy set definition version at MG level' {
        # make a policy set definition version, get it back and validate
        $expected = New-AzPolicySetDefinitionVersion -Name $policySetDefName -ManagementGroupName $managementGroup -PolicyDefinition $policySet -Description $description -Version $someOldVersion
    ...
    It 'List policy set definitions at MG level' {
        $list = Get-AzPolicySetDefinition -ManagementGroupName $managementGroup | ?{ $_.Name -in @($policySetDefName) }
        $list | Should -HaveCount 1
        $list = Get-AzPolicySetDefinition -Id $baseDefinition.Id -ListVersion
        $list | Should -HaveCount 2
    }

    It 'Remove policy set definition version at MG level' {
