...
[CmdletBinding(DefaultParameterSetName='Name', PositionalBinding=$false, SupportsShouldProcess, ConfirmImpact='High')]
param(
...
begin {
    try {
        ...
        if ($writeln) {
            Write-Host -ForegroundColor Cyan "Remove-AzPolicyDefinition(" $PSBoundParameters ") - (Begin:ParameterSet: $($PSCmdlet.ParameterSetName))"
        }
...
process {
    try {
        if ($writeln) {
            Write-Host -ForegroundColor Cyan "Remove-AzPolicyDefinition(" $PSBoundParameters ") - (Process:ParameterSet: $($PSCmdlet.ParameterSetName))"
        }
        ...
        # previous remove cmdlet always returned a value
        ...
        if ($resolved.Scope -and $PSCmdlet.ShouldProcess($target)) {
            ...
            # mapping table to generated remove cmdlets
            $mapping = @{
        ...
        if ($BackwardCompatible -or $PassThru) {
