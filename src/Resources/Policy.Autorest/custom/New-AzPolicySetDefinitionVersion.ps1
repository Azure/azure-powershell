...
[CmdletBinding(DefaultParameterSetName='PolicySetDefinitionName', SupportsShouldProcess=$true, ConfirmImpact='Low')]
param(
...
process {
    if ($writeln) {
        Write-Host -ForegroundColor Cyan "process:New-AzPolicySetDefinitionVersion(" $PSBoundParameters ") - (ParameterSet: $($PSCmdlet.ParameterSetName))"
    }
