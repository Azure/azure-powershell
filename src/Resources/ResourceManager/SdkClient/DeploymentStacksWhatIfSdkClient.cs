...
namespace Microsoft.Azure.Commands.ResourceManager.Cmdlets.SdkClient
{
    ...
    public class DeploymentStacksWhatIfSdkClient
    {
            ...
            string debugSettingDetailLevel = null)
        {
                ...
                catch (Exception postEx)
                {
                    ...
                    return ConvertToPSDeploymentStackWhatIfResult(finalResult);
                                                     ...
                                                     (ex.Response.StatusCode == System.Net.HttpStatusCode.BadRequest && 
                                                      ex.Body?.Message?.IndexOf("what-if result", StringComparison.OrdinalIgnoreCase) >= 0))
            {
            ...
            {
                WriteError($"Error executing What-If (HTTP {ex.Response?.StatusCode}): {ex.Body?.Message ?? ex.Message}");
                if (ex.Body?.Details != null && ex.Body.Details.Any())
                {
                    foreach (var detail in ex.Body.Details)
                    {
