...
function Test-RaClassicAdmins
{
	...
	# Test
	$classic =  Get-AzRoleAssignment -IncludeClassicAdministrators  | Where-Object { $_.Scope -ieq ('/subscriptions/' + $subscription[0].Id) -and $_.RoleDefinitionName.ToLower().Contains('administrator')}	
...
function Test-RaDeleteByPSRoleAssignment
{
    ...
    $newAssignment = New-AzRoleAssignment `
                        -ObjectId $users[0].Id `
...
function Test-RaValidateInputParameters ($cmdName)
{
    ...
    $subscription = Get-AzSubscription | Select-Object -Last 1 -Wait
...
function Test-RaPropertiesValidation
{
    ...
    $rd = Get-AzRoleDefinition -Name "Custom Reader Test"
