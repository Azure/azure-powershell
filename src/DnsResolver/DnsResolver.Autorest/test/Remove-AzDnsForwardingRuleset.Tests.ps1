...
Describe 'Remove-AzDnsForwardingRuleset' {
    BeforeAll {
        ...
        $rgName = "ps-frs-rm-$(Get-Random -Max 99999)"
        if ($TestMode -ne 'playback') {
            ...
            New-AzDnsForwardingRuleset -Name "frs-rm-1" -ResourceGroupName $rgName -Location $location -DnsResolverOutboundEndpoint @(@{id = $oeId})
        }
    }
    AfterAll { if ($TestMode -ne 'playback') { Remove-AzResourceGroup -Name $rgName -ErrorAction SilentlyContinue -AsJob | Out-Null } }
    It 'Delete a forwarding ruleset' {
        Remove-AzDnsForwardingRuleset -Name "frs-rm-1" -ResourceGroupName $rgName
        { Get-AzDnsForwardingRuleset -Name "frs-rm-1" -ResourceGroupName $rgName } | Should -Throw
    }
}
