...
Describe 'New-AzDnsForwardingRulesetForwardingRule' {
    BeforeAll {
        ...
        $rgName = "ps-frs-fr-new-$(Get-Random -Max 99999)"
    ...
    It 'Create a forwarding rule' {
        $target = New-AzDnsResolverTargetDnsServerObject -IPAddress "10.0.0.1" -Port 53
        $rule = New-AzDnsForwardingRulesetForwardingRule -DnsForwardingRulesetName "frs-fr" -Name "fr-new-1" -ResourceGroupName $rgName -DomainName "contoso.com." -TargetDnsServer @($target)
