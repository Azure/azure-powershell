...
Describe 'New-AzDnsForwardingRuleset' {
    BeforeAll {
        ...
        $rgName = "ps-frs-new-$(Get-Random -Max 99999)"
    ...
    It 'Create a forwarding ruleset' {
        ...
        $ruleset = New-AzDnsForwardingRuleset -Name "frs-new-1" -ResourceGroupName $rgName -Location $location -DnsResolverOutboundEndpoint @(@{id = $oeId})
        $ruleset.ProvisioningState | Should -Be "Succeeded"
    }
}
