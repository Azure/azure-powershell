...
Describe 'Update-AzDnsForwardingRulesetForwardingRule' {
    BeforeAll {
        ...
        $rgName = "ps-frs-fr-update-$(Get-Random -Max 99999)"
        if ($TestMode -ne 'playback') {
            ...
            New-AzDnsForwardingRulesetForwardingRule -DnsForwardingRulesetName "frs-fr" -Name "fr-update-1" -ResourceGroupName $rgName -DomainName "contoso.com." -TargetDnsServer @($target)
        }
    }
    AfterAll { if ($TestMode -ne 'playback') { Remove-AzResourceGroup -Name $rgName -ErrorAction SilentlyContinue -AsJob | Out-Null } }
    It 'Update forwarding rule metadata' {
        $rule = Update-AzDnsForwardingRulesetForwardingRule -DnsForwardingRulesetName "frs-fr" -Name "fr-update-1" -ResourceGroupName $rgName -Metadata @{"key"="value"}
