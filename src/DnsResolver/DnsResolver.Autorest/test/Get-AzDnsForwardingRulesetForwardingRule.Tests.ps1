...
Describe 'Get-AzDnsForwardingRulesetForwardingRule' {
    BeforeAll {
        ...
        $rgName = "ps-frs-fr-get-$(Get-Random -Max 99999)"
        if ($TestMode -ne 'playback') {
            ...
            New-AzDnsForwardingRulesetForwardingRule -DnsForwardingRulesetName "frs-fr" -Name "fr-get-1" -ResourceGroupName $rgName -DomainName "contoso.com." -TargetDnsServer @($target)
        }
    }
    AfterAll { if ($TestMode -ne 'playback') { Remove-AzResourceGroup -Name $rgName -ErrorAction SilentlyContinue -AsJob | Out-Null } }
    It 'Get a forwarding rule' {
        $rule = Get-AzDnsForwardingRulesetForwardingRule -DnsForwardingRulesetName "frs-fr" -Name "fr-get-1" -ResourceGroupName $rgName
        $rule.ProvisioningState | Should -Be "Succeeded"
    }
    It 'List forwarding rules' {
        $rules = Get-AzDnsForwardingRulesetForwardingRule -DnsForwardingRulesetName "frs-fr" -ResourceGroupName $rgName
