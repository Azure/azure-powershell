...
Describe 'Update-AzDnsForwardingRuleset' {
    BeforeAll {
        ...
        if ($TestMode -ne 'playback') {
            ...
            New-AzDnsForwardingRuleset -Name "frs-upd-1" -ResourceGroupName $rgName -Location $location -DnsResolverOutboundEndpoint @(@{id = $oeId})
        }
    }
    AfterAll { if ($TestMode -ne 'playback') { Remove-AzResourceGroup -Name $rgName -ErrorAction SilentlyContinue -AsJob | Out-Null } }
    It 'Update forwarding ruleset tags' {
        $tag = @{ "updated" = "true" }
        $rs = Update-AzDnsForwardingRuleset -Name "frs-upd-1" -ResourceGroupName $rgName -Tag $tag
