...
Function BeSuccessfullyCreatedForwardingRule {
    <#
    .SYNOPSIS
    Tests whether a Forwarding rule is created successfully
    #>
        [CmdletBinding()]
        Param(
            $ActualValue,
        ...
        If ( -not($Pass) ) {
            $FailureMessage = 'The Forwarding rule is not created successfully.'
        }
...
Function BeSameForwardingRuleAsExpected {
    <#
    .SYNOPSIS
    Tests whether an Forwarding rule is created successfully
    #>
        [CmdletBinding()]
        Param(
            $ActualValue,
        ...
        If ( -not($Pass) ) {
            $FailureMessage = 'The Forwarding rule is different from the expected.'
        }
