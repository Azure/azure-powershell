...
Function BeSameOutboundEndpointCollectionAsExpected {
    <#
    .SYNOPSIS
    Tests whether outbound endpoint collection is same as expected
    #>
        [CmdletBinding()]
        Param(
            $ActualValue,
        ...
        If ( -not($Pass) ) {
            $FailureMessage = 'The Outbound ENdpoint(s) are different from the expected.'
        }
