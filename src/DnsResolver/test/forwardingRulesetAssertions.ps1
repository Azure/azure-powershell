...
Function BeSameDnsForwardingRulesetCollectionAsExpected {
    <#
    .SYNOPSIS
    Tests whether DNS forwarding ruleset collection is same as expected
    #>
        [CmdletBinding()]
        Param(
            $ActualValue,
        ...
        If ( -not($Pass) ) {
            $FailureMessage = 'The DNS forwarding ruleset(s) are different from the expected.'
        }
