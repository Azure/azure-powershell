...
{
    [Cmdlet("Get", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "AnfPool"), OutputType(typeof(PSAnfPool))]
    public class GetAzureRmAnfPool : AzureAnfCmdletBase
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (PoolName != null)
            {
                var anfPool = AzureNetAppFilesManagementClient.Pools.Get(ResourceGroupName, AccountName, PoolName);
                WriteObject(anfPool.ToPsAnfPool());
            }
            else
            {
                var anfPool = AzureNetAppFilesManagementClient.Pools.List(ResourceGroupName, AccountName).Select(e => e.ToPsAnfPool());
