# How-To
This document describes how to develop for `Az.Communication`.

## Building `Az.Communication`
...
There are multiple scripts created for performing different actions for developing `Az.Communication`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.Communication.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.Communication.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.Communication`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.Communication` into the session.
