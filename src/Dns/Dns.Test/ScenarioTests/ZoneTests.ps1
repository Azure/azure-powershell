...
function Test-ZoneCrud
{
	...
	Assert-ThrowsLike { Get-AzDnsZone -Name $zoneName -ResourceGroupName $resourceGroup.ResourceGroupName } "*was not found*"
...
function Test-ZoneCrudWithPiping
{
	...
	Assert-ThrowsLike { Get-AzDnsZone -Name $zoneName -ResourceGroupName $resourceGroupName } "*was not found*"
...
function Test-ZoneSetNotFound
{
	...
	$message = [System.String]::Format("The Zone {0} has been modified (etag mismatch).", $zoneName);
	Assert-Throws { Set-AzDnsZone -Name $zoneName -ResourceGroupName $resourceGroup.ResourceGroupName } $message;
	Remove-AzResourceGroup -Name $resourceGroup.ResourceGroupName -Force
}
