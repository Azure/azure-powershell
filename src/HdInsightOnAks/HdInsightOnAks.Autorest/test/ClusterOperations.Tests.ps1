...
Describe 'ClusterOperations' {
    BeforeAll {
        ...
        { New-AzHdInsightOnAksClusterPool -Name $clusterpoolName -ResourceGroupName $clusterResourceGroupName -VmSize $vmSize -Location $location} | Should -Not -Throw
    ...
    It 'New AzHdInsightOnAksCluster' {
        { $script:ComputeProfileNode = New-AzHdInsightOnAksNodeProfileObject -Type "Worker" -Count 3 -VMSize "Standard_D8d_v5" } | Should -Not -Throw
        $script:ComputeProfileNode.Type | Should -Be "Worker"
        $script:ComputeProfileNode.Count | Should -Be 3
        $script:ComputeProfileNode.VMSize | Should -Be "Standard_D8d_v5"
