...
Creates a ChangeRecord that targets a single resource for a guarded operation. Use `resourceId` together with `httpMethod` (for example `DELETE`) when the change applies to a specific resource rather than an entire subscription.
