function New-AzDataProtectionPolicyTagCriteriaClientObject{
    ...
    process {
        $criteria = [Microsoft.Azure.PowerShell.Cmdlets.DataProtection.Models.Api20230501.ScheduleBasedBackupCriteria]::new()
        ...
        if($DaysOfMonth -ne $null){
            ...
            Foreach($dayOfMonth in $DaysOfMonth)
            {
                if($dayOfMonth -match "^[\d]+$")
                {
                    ...
                    $day = [Microsoft.Azure.PowerShell.Cmdlets.DataProtection.Models.Api20230501.Day]::new()
