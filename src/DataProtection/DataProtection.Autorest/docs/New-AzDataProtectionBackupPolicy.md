...
The first command gets the default vaulted policy template for AzureBlob.
The template ships with a `Default` (Vault) retention rule.
The second and third commands add an OperationalStore retention rule named `Default_OperationalStore` to the policy.
This is **additive** — the existing `Default` Vault rule is preserved.
...
The first command gets the default policy template for AzureCosmosDB.
AzureCosmosDB only supports VaultStore data store and Weekly backup frequency.
