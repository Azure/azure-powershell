...
function Add-AzScVmmVMDisk {
  ...
  begin {
      try {
          ...
          $mapping = @{
            CreateExpanded = 'Az.ScVmm.private\Update-AzScVmmVM_UpdateExpanded';
          }
          ...
          if (-not $PSBoundParameters.ContainsKey('vhdType')) {
            $vhdType = "Dynamic"
          }
          if (-not $PSBoundParameters.ContainsKey('busType')) {
            $busType = "SCSI"
          }
          ...
          $newDiskObject += $diskToAdd

          # List of parameters to remove
          $keysToRemove = @('vmName', 'ResourceGroupName', 'SubscriptionId', 'bus', 'lun', 'busType', 'vhdType', 'QosName', 'QosId', 'diskSizeGB', 'DiskName')
