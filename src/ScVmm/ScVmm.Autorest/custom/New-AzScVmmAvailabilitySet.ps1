...
.Example
New-AzScVmmAvailabilitySet -Name "test-avset" -ResourceGroupName "test-rg-01" -SubscriptionId "00000000-abcd-0000-abcd-000000000000" -Location "eastus" -vmmServerName "test-vmmserver-01"
    ...
    try {
        ...
        if ($parameterSet -eq 'CreateExpanded') {
            ...
            if (-not $PSBoundParameters.ContainsKey('VmmServerName') -and -not $PSBoundParameters.ContainsKey('VmmServerId')) {
                throw "Either VmmServerName or VmmServerId should be specified."
            }

            if ($PSBoundParameters.ContainsKey('VmmServerName') -and $PSBoundParameters.ContainsKey('VmmServerId')) {
                throw "Either VmmServerName or VmmServerId should be specified, not both."
            }

            if ($PSBoundParameters.ContainsKey('VmmServerId') -and -not $PSBoundParameters.ContainsKey('CustomLocationId')) {
                throw "CustomLocationId must be specified with VmmServerId."
            }

            if ($PSBoundParameters.ContainsKey('VmmServerName')) {
                try {
                    $VmmObj = Get-AzScVmmServer -Name $VmmServerName -ResourceGroupName $ResourceGroupName -SubscriptionId $SubscriptionId
                    $VmmId = $VmmObj.Id
                    $VmmCustomLocationId = $VmmObj.ExtendedLocationName
                    $PSBoundParameters['VmmServerId'] = $VmmId
                    $PSBoundParameters['ExtendedLocationName'] = $VmmCustomLocationId
            ...
            foreach ($key in @('VmmServerName', 'CustomLocationId')) {
