...
create a checkpoint in virtual machine.
.Example
New-AzScVmmVMCheckpoint -Name "test-vm" -ResourceGroupName "test-rg-01" -CheckpointName "Checkpoint1" -CheckpointDescription "Test-Checkpoint"
...
begin {
    try {
        ...
        if ($parameterSet -eq 'CreateExpanded') {
            $PSBoundParameters['Description'] = $CheckpointDescription
            $PSBoundParameters['Name'] = $CheckpointName
            foreach ($key in @('CheckpointDescription', 'CheckpointName')) {
                [void]$PSBoundParameters.Remove($key)
            }
        }
        # Custom Logic End
