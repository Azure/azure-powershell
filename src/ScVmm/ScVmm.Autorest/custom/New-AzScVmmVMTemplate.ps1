...
.Example
New-AzScVmmVMTemplate -Name "test-vmt" -ResourceGroupName "test-rg-01" -SubscriptionId "00000000-abcd-0000-abcd-000000000000" -Location "eastus" -vmmServerName "test-vmmserver-01"
    ...
    try {
        ...
        if ($parameterSet -eq 'CreateExpanded') {
            ...
            if ($PSBoundParameters.ContainsKey('InventoryUuid')) {
                $PSBoundParameters['Uuid'] = $InventoryUuid
            }

            foreach ($key in @('VmmServerName', 'CustomLocationId', 'InventoryUuid')) {
