...
namespace Microsoft.Azure.PowerShell.Cmdlets.Ssh.Common
{
    ...
    internal class ResourceTypeUtils
    {
        ...
        /*public void GetTypesOnResourceGroup(string rg_name, string name)
        {

            ResourceManagementClient resourceManagementClient = AzureSession.Instance.ClientFactory.CreateArmClient<ResourceManagementClient>(_context, AzureEnvironment.Endpoint.ResourceManager);
            string filter = $"name eq '{name}'";
            var resources = resourceManagementClient.ResourceGroups.ListResources(rg_name, filter);
