if(($null -eq $TestName) -or ($TestName -contains 'Update-AzBillingBenefitsSavingsPlanOrderAlias'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Update-AzBillingBenefitsSavingsPlanOrderAlias.Recording.json'
...
Describe 'Update-AzBillingBenefitsSavingsPlanOrderAlias' {
    It 'UpdateExpanded' -skip {
        $response = Update-AzBillingBenefitsSavingsPlanOrderAlias -Name "PSTest1" -AppliedScopeType "Shared" -BillingPlan "P1M" -BillingScopeId "/subscriptions/eef82110-c91b-4395-9420-fcfcbefc5a47" -CommitmentAmount 0.001 -CommitmentCurrencyCode "USD" -CommitmentGrain "Hourly" -SkuName "Compute_Savings_Plan" -DisplayName "PSTest1" -Term "P3Y"
    ...
    It 'Update' -skip {
        ...
        $response = Update-AzBillingBenefitsSavingsPlanOrderAlias -Name "PSTest5" -Body $request
        ExecuteTestCases($response)
    }
    ...
    It 'UpdateViaIdentityExpanded' -skip {
        ...
        $response = Update-AzBillingBenefitsSavingsPlanOrderAlias -InputObject $identity -Body $request
        ExecuteTestCases($response)
    }
