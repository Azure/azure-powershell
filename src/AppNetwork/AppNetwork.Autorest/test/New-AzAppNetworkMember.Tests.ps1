...
Describe 'New-AzAppNetworkMember' {
    ...
    # runs serially, so the full lifecycle is exercised inside a single ordered
    # Describe: the member is created, read (Get/List/UpgradeHistory), then deleted -
    # only ever one live member, satisfying the 1:1 constraint with a single cluster.
    # The standalone Get/Remove-AzAppNetworkMember and Get-AzAppNetworkMemberUpgradeHistory
    # tests are skipped because their coverage lives here. Uses $env.appLinkNameForMember
    # so the Remove-AzAppNetwork test can delete its own parent without a child cascade.
    It 'CreateExpanded' {
        {
