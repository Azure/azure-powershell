# How-To
This document describes how to develop for `Az.AppConfiguration`.

## Building `Az.AppConfiguration`
...
There are multiple scripts created for performing different actions for developing `Az.AppConfiguration`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.AppConfiguration.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.AppConfiguration.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.AppConfiguration`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.AppConfiguration` into the session.
