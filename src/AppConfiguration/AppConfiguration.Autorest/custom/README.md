# Custom
This directory contains custom implementation for non-generated cmdlets for the `Az.AppConfiguration` module. Both scripts (`.ps1`) and C# files (`.cs`) can be implemented here. They will be used during the build process in `build-module.ps1`, and create cmdlets into the `..\exports` folder. The only generated file into this folder is the `Az.AppConfiguration.custom.psm1`. This file should not be modified.
...
These provide functionality to our HTTP pipeline and other useful features. In script, you can forward these parameters using `$PSBoundParameters` to the other cmdlets you're calling within `Az.AppConfiguration`. For C#, follow the usage seen in the `ProcessRecordAsync` method.
