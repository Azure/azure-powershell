  ...
  public class ExportPsd1 : PSCmdlet
  {
    ...
    protected override void ProcessRecord()
    {
      try
      {
        ...
        sb.AppendLine($@"{Indent}RootModule = '{"./Az.AppConfiguration.psm1"}'");
        ...
        sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: AppConfiguration cmdlets"}'");
        sb.AppendLine($@"{Indent}PowerShellVersion = '5.1'");
        sb.AppendLine($@"{Indent}DotNetFrameworkVersion = '4.7.2'");
        sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.AppConfiguration.private.dll"}'");
        ...
        var formatList = customFormatPs1xmlFiles.Prepend("./Az.AppConfiguration.format.ps1xml").ToPsList();
        ...
        sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule AppConfiguration".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
