...
{
    [Cmdlet("Get", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "BatchPrivateLinkResource", DefaultParameterSetName = AccountAndResourceGroupParameterSet)]
    [OutputType(typeof(PSPrivateLinkResource))]
    public class GetBatchPrivateLinkResourceCommand : BatchCmdletBase
    {
        ...
        [ResourceNameCompleter("Microsoft.Batch/batchAccounts/privateLinkResources", nameof(ResourceGroupName), nameof(AccountName))]
        [ValidateNotNullOrEmpty]
        ...
        {
            WriteVerboseWithTimestamp(Resources.BeginMAMLCall, nameof(GetBatchPrivateLinkResourceCommand));
            if (!string.IsNullOrEmpty(AccountName) && !string.IsNullOrEmpty(ResourceGroupName))
            {
                // List
                if (string.IsNullOrEmpty(Name))
                {
                    foreach (var result in BatchClient.ListPrivateLinkResources(ResourceGroupName, AccountName, MaxCount))
                    {
                        WriteObject(result);
                    }
                }
                ...
                {
                    var result = BatchClient.GetPrivateLinkResource(ResourceGroupName, AccountName, Name);
                    WriteObject(result);
                }
            }
            else
            {
                ...
                var result = BatchClient.GetPrivateLinkResource(resourceId.ResourceGroupName, parentResource, resourceId.ResourceName);
                WriteObject(result);
            }
            WriteVerboseWithTimestamp(Resources.EndMAMLCall, nameof(GetBatchPrivateLinkResourceCommand));
        }
    }
}
