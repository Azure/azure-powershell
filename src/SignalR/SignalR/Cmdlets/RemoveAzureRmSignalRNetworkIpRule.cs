...
{
    [Cmdlet("Remove", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "SignalRNetworkIpRule", SupportsShouldProcess = true, DefaultParameterSetName = ResourceGroupParameterSet)]
    [OutputType(typeof(PSSignalRNetworkAcls))]
    public class RemoveAzureRmSignalRNetworkIpRule : SignalRCmdletBase, IWithResourceId, IWithSignalRInputObject
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            RunCmdlet(() =>
            {
                ...
                if (ShouldProcess($"SignalR service {ResourceGroupName}/{Name}", "remove network IP rule(s)"))
                {
                    var signalr = Client.SignalR.Get(ResourceGroupName, Name);
                    var networkACLs = signalr.NetworkAcLs;
                    if (networkACLs.IPRules != null && IpRule != null && IpRule.Length > 0)
                    {
                        networkACLs.IPRules = networkACLs.IPRules.Where(r => !IpRule.Any(toRemove => toRemove.Equals(r))).ToList();
                    }
