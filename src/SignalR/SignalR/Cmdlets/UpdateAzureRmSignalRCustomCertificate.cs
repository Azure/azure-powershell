...
{
    [Cmdlet("Update", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "SignalRCustomCertificate", SupportsShouldProcess = true, DefaultParameterSetName = ResourceGroupParameterSet)]
    [OutputType(typeof(PSCustomCertificateResource))]
    public class UpdateAzureRmSignalRCustomCertificate : SignalRCmdletBase, ISignalRChildResource
    {
        [Parameter(Mandatory = true, ParameterSetName = ResourceGroupParameterSet, HelpMessage = "The resource group name.")]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            RunCmdlet(() =>
            {
                ...
                var existing = Microsoft.Azure.Management.SignalR.SignalRCustomCertificatesOperationsExtensions.Get(Client.SignalRCustomCertificates, ResourceGroupName, SignalRName, Name) ?? throw new AzPSInvalidOperationException($"SignalR custom certificate {ResourceGroupName}/{SignalRName}/{Name}" + " doesn't exist.");
                if (ShouldProcess($"SignalR custom certificate {ResourceGroupName}/{SignalRName}/{Name}", "update"))
                {
                    var cert = new CustomCertificate(
                        KeyVaultBaseUri ?? existing.KeyVaultBaseUri,
                        KeyVaultSecretName ?? existing.KeyVaultSecretName,
                        keyVaultSecretVersion: KeyVaultSecretVersion ?? existing.KeyVaultSecretVersion,
                        id: existing.Id,
                        name: existing.Name,
                        type: existing.Type,
