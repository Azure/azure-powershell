...
Describe 'Start-AzFrontDoorCdnProfilePrepareMigration' {
    It 'MigrateExpanded' {
        ...
        $migratedProfileName = 'migrated-' + (RandomString -allChars $false -len 6)

        Install-Module -Name Az.FrontDoor
        Install-Module -Name Az.KeyVault

        $migrateLocation = Start-AzFrontDoorCdnProfilePrepareMigration -ResourceGroupName $env.ResourceGroupName -ClassicResourceReferenceId $classicResourceReferenceId -ProfileName $migratedProfileName -SkuName $profileSku 
