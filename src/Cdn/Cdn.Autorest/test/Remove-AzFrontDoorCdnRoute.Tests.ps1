...
Describe 'Remove-AzFrontDoorCdnRoute' {
    BeforeAll {
        $script:endpointName = 'e-clipstest-route-rm'
        $script:ogName = 'org-route-rm'
        $script:routeName = 'routeName-rm'
    ...
    It 'Delete' {
        Remove-AzFrontDoorCdnRoute -ResourceGroupName $env.ResourceGroupName -ProfileName $env.FrontDoorCdnProfileName -EndpointName $script:endpointName -Name $script:routeName
        { Get-AzFrontDoorCdnRoute -ResourceGroupName $env.ResourceGroupName -ProfileName $env.FrontDoorCdnProfileName -EndpointName $script:endpointName -Name $script:routeName -ErrorAction Stop } | Should -Throw
    }
    ...
    It 'DeleteViaIdentityAfdEndpoint' -skip {
        { throw [System.NotImplementedException] } | Should -Not -Throw
    }
