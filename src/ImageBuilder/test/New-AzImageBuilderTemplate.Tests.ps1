...
Describe 'New-AzImageBuilderTemplate' {
    ...
    It 'platformimg-managedimg' {
        ...
        #region OS:Windows
        $srcPlatformWind = New-AzImageBuilderSource -SourceTypePlatformImage -Publisher 'MicrosoftWindowsServer' -Offer 'WindowsServer' -Sku '2019-Datacenter' -Version 'latest'
        $disManagedImgWind = New-AzImageBuilderDistributor -ManagedImageDistributor -ArtifactTag @{source='azVmPlatform';baseofimg='MicrosoftWindowsServer'} -ImageId "/subscriptions/$($env.SubscriptionId)/resourceGroups/$($env.ResourceGroup)/providers/Microsoft.Compute/images/$($env.Resources.Distributor.distributorName001)" -Location $env.Location -RunOutputName $env.Resources.RunOutputName.runOutputName201
        $customizerName = 'downloadBuildArtifacts'
        $sha256Checksum = 'ade4c5214c3c675e92c66e2d067a870c5b81b9844b3de3cc72c49ff36425fc93'
        ...
        $customizer = New-AzImageBuilderCustomizer -FileCustomizer -CustomizerName $customizerName -Sha256Checksum  $sha256Checksum -Destination $destination -SourceUri $sourceUri
