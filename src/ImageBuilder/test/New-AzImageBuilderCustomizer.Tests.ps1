...
Describe 'New-AzImageBuilderCustomizer' {
    It 'WindowsUpdateCustomizer' {
        ...
        $customizer = New-AzImageBuilderCustomizer -WindowsUpdateCustomizer -Filter $filters -SearchCriterion $searchCriterion -UpdateLimit $updateLimit -CustomizerName $customizerName
    ...
    It 'PowerShellCustomizer' {
        ...
        $customizer = New-AzImageBuilderCustomizer -PowerShellCustomizer -CustomizerName $customizerName -RunElevated $false -Sha256Checksum $sha256Checksum -Inline $inline
        $customizer.Name | Should -Be $customizerName
    }
    ...
    It 'RestartCustomizer' {
        ...
        $restartTimeout = "10m" 
        $customizer = New-AzImageBuilderCustomizer -RestartCustomizer -CustomizerName $customizerName -RestartCommand $restartCommand -RestartCheckCommand $restartCheckCommand -RestartTimeout $restartTimeout
        $customizer.Name | Should -Be $customizerName
    }
}
