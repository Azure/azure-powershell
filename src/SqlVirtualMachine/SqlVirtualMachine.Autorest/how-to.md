# How-To
This document describes how to develop for `Az.SqlVirtualMachine`.

## Building `Az.SqlVirtualMachine`
...
There are multiple scripts created for performing different actions for developing `Az.SqlVirtualMachine`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.SqlVirtualMachine.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.SqlVirtualMachine.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.SqlVirtualMachine`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.SqlVirtualMachine` into the session.
