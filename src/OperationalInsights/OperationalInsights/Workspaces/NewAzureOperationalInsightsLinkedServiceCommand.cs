...
{
    [Cmdlet("New", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "OperationalInsightsLinkedService", SupportsShouldProcess = true), OutputType(typeof(PSLinkedService))]
    public class NewAzureOperationalInsightsLinkedServiceCommand : OperationalInsightsBaseCmdlet
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(this.LinkedServiceName,
                string.Format("Create linked service: {0} for workspace: {1}", this.LinkedServiceName, this.WorkspaceName)))
            {
                WriteObject(this.OperationalInsightsClient.CreatePSLinkedService(this.ResourceGroupName, this.WorkspaceName, this.LinkedServiceName, parameters));
            }
        }
    }
}
