...
function Test-ClusterCRUD
{
	...
	try
	{
		...
		$job = Update-AzOperationalInsightsCluster -ResourceGroupName $rgNameExisting -ClusterName $clusterNameExisting -SkuCapacity 5500 -AsJob
		$job | Wait-Job
		$cluster = $job | Receive-Job

		Assert-NotNull $cluster
		Assert-AreEqual $keyNameExisting $cluster.KeyVaultProperties.KeyName
		Assert-AreEqual 5500 $cluster.Sku.Capacity
