...
#>
function Get-AzLogzAccountMonitoredResource {
[OutputType([Microsoft.Azure.PowerShell.Cmdlets.Logz.Models.Api20201001Preview.IMonitoredResource])]
[CmdletBinding(DefaultParameterSetName='ListByAccount', PositionalBinding=$false, SupportsShouldProcess, ConfirmImpact='Medium')]
param(
    [Parameter(ParameterSetName='ListByAccount',Mandatory)]
...
process {
    try {
        if ($PSCmdlet.ParameterSetName -eq 'ListBySubAccount') {
            $AccountName = $PSBoundParameters['Name']

            $PSBoundParameters['Name'] = $SubAccountName;

            $PSBoundParameters.Remove('SubAccountName') | Out-Null
            $PSBoundParameters.Add('AccountName', $AccountName)

            return Az.Logz.internal\Get-AzLogzSubAccountMonitoredResource @PSBoundParameters
        }

        return Az.Logz.internal\Get-AzLogzAccountMonitoredResource @PSBoundParameters
    } catch {
        throw
    }
}
