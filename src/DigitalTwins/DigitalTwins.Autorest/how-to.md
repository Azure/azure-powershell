# How-To
This document describes how to develop for `Az.DigitalTwins`.

## Building `Az.DigitalTwins`
...
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DigitalTwins.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DigitalTwins.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DigitalTwins`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DigitalTwins` into the session.
