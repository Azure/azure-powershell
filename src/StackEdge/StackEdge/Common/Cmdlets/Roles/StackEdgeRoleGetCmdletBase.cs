...
namespace Microsoft.Azure.PowerShell.Cmdlets.StackEdge.Common.Cmdlets.Roles
{
    ...
    public class StackEdgeRoleGetCmdletBase : AzureStackEdgeCmdletBase
    {
        ...
        private List<PSStackEdgeRole> GetByResourceName()
        {
            var resourceModel = GetResourceModel();
            return new List<PSStackEdgeRole>() {new PSStackEdgeRole(resourceModel)};
        }

        private IPage<Role> ListResourceModel()
        {
            return this.StackEdgeManagementClient.Roles.ListByDataBoxEdgeDevice(
                this.DeviceName,
                this.ResourceGroupName);
        }

        private IPage<Role> ListResourceModel(string nextPageLink)
        {
            return this.StackEdgeManagementClient.Roles.ListByDataBoxEdgeDeviceNext(
                nextPageLink
            );
        }

        private List<PSStackEdgeRole> ListPSResourceModels()
        {
            ...
            var paginatedResult = new List<Role>(resourceModel);
            ...
            return paginatedResult.Select(t => new PSStackEdgeRole(t)).ToList();
        }
