...
namespace Microsoft.Azure.PowerShell.Cmdlets.StackEdge.Common.Cmdlets.Shares
{
    ...
    public class StackEdgeShareGetCmdletBase : AzureStackEdgeCmdletBase
    {
        ...
        private List<PSStackEdgeShare> GetByResourceName()
        {
            var resourceModel = GetResourceModel();
            return new List<PSStackEdgeShare>() {new PSStackEdgeShare(resourceModel)};
        }

        private IPage<Share> ListResourceModel()
        {
            return this.StackEdgeManagementClient.Shares.ListByDataBoxEdgeDevice(
                this.DeviceName,
                this.ResourceGroupName);
        }

        private IPage<Share> ListResourceModel(string nextPageLink)
        {
            return this.StackEdgeManagementClient.Shares.ListByDataBoxEdgeDeviceNext(
                nextPageLink
            );
        }

        private List<PSStackEdgeShare> ListPSResourceModels()
        {
            ...
            var paginatedResult = new List<Share>(resourceModel);
            ...
            return paginatedResult.Select(t => new PSStackEdgeShare(t)).ToList();
        }
