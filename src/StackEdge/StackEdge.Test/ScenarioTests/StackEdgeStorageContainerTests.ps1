...
{	
	...
	# Test
	Assert-ThrowsContains { Get-AzStackEdgeStorageContainer $rgname $dfname $ecntname } "not find"	
}
...
{	
	...
	$edgeStorageAccount = New-AzStackEdgeStorageAccount $rgname $dfname $edgeStorageAccountName -StorageAccountCredentialName $storageAccountCredential.Name
	# Test
	try
	{
		$expected = New-AzStackEdgeStorageContainer $rgname $dfname $edgeStorageAccountName -Name $edgeStorageContainerName -DataFormat BlockBlob
		Assert-AreEqual $expected.Name $edgeStorageContainerName
		
	}
	finally
	{
		Remove-AzStackEdgeStorageContainer $rgname $dfname $edgeStorageAccountName $edgeStorageContainerName
...
{	
	...
	try
	{
		$expected = New-AzStackEdgeStorageContainer $rgname $dfname $edgeStorageAccountName -Name $edgeStorageContainerName -DataFormat BlockBlob
		Remove-AzStackEdgeStorageContainer $rgname $dfname $edgeStorageAccountName $edgeStorageContainerName
		Assert-ThrowsContains { Get-AzStackEdgeStorageContainer $rgname $dfname $edgeStorageAccountName $edgeStorageContainerName} "not find"	
