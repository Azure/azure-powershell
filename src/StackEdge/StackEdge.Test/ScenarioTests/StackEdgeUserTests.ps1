...
{	
	...
	# Test
	Assert-ThrowsContains { Get-AzStackEdgeUser -ResourceGroupName $rgname -DeviceName $dfname -Name $name  } "not find"	
}
...
{	
	...
	try
	{
		$expected = New-AzStackEdgeUser $rgname $dfname $name -Password $password -EncryptionKey $encryptionKey
		Assert-AreEqual $expected.Name $name
	}
	finally
	{
		Remove-AzStackEdgeUser $rgname $dfname $name
	}  
}
