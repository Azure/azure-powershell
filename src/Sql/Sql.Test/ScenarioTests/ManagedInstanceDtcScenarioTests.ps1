...
{
    # DTC test requires a paasv2 managed instance. Those instances currently aren't available on every region, and their creation can last too long.
    # Because of that we are fetching a precreated instance.
    Write-Debug "Fetching test MI"
    $managedInstance = Get-AzSqlInstance -ResourceGroupName CustomerExperienceTeam_RG -Name skrivokapic-paasv2

    #Get by name
    Write-Debug "Get by name"
    $originalDtcObject = Get-AzSqlInstanceDtc -ResourceGroupName $managedInstance.ResourceGroupName -InstanceName skrivokapic-paasv2
    ...
    $dtcJob = Set-AzSqlInstanceDtc -ResourceGroupName $managedInstance.ResourceGroupName -InstanceName skrivokapic-paasv2 -DtcEnabled $dtc.DtcEnabled -AsJob
    $dtcJob | Wait-Job
