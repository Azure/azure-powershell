...
Backup immutability can be locked only for live managed databases.
The cmdlet returns an error if `LockImmutability` is specified for a restorable dropped managed database.
...
Note you can reduce the retention period only for deleted databases whose backup immutability was not locked before deletion. You cannot specify `LockImmutability` for a deleted database.
...
This command sets the short term retention policy for the live database database01 to 7 days and locks backup immutability.
The `-Force` parameter skips the confirmation prompt for this irreversible action.
...
This parameter is supported only for live managed databases and cannot be used with restorable dropped managed databases.
