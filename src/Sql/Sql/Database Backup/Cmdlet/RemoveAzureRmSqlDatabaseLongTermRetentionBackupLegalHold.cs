...
namespace Microsoft.Azure.Commands.Sql.Database_Backup.Cmdlet
{
    ...
    public class RemoveAzureRmSqlDatabaseLongTermRetentionBackupLegalHold : AzureSqlDatabaseLongTermRetentionBackupCmdletBase<AzureSqlDatabaseLongTermRetentionBackupModel>
    {
        ...
        /// <summary>
        /// Defines whether it is ok to skip the extra check for removing legal hold from an expired backup.
        /// </summary>
        [Parameter(HelpMessage = "Skip confirmation message for removing a legal hold from an expired backup.")]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(this.BackupName))
            {
                if (entity.BackupExpirationTime <= DateTime.UtcNow)
                {
                    ...
                    string.Format(CultureInfo.InvariantCulture, Properties.Resources.RemoveLegalHoldAzureSqlDatabaseLongTermRetentionBackupWarning, this.BackupName, this.DatabaseName, this.ServerName, this.Location),
                    string.Format(CultureInfo.InvariantCulture, Properties.Resources.RemoveLegalHoldAzureSqlDatabaseLongTermRetentionBackupDescription, this.BackupName, this.DatabaseName, this.ServerName, this.Location)))
                {
