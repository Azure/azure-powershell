...
namespace Microsoft.Azure.Commands.Sql.ImportExport.Service
{
    ...
    /// </summary>
    public class ImportExportDatabaseArmAdapter
    {
        /// <summary>
        /// Gets or sets the AzureEndpointsCommunicator which has all the needed management clients
        /// </summary>
        private ImportExportDatabaseArmCommunicator Communicator { get; set; }
        ...
        public ImportExportDatabaseArmAdapter(IAzureContext context)
        {
            Context = context;
            Communicator = new ImportExportDatabaseArmCommunicator(Context);
        }
        ...
        public AzureSqlDatabaseImportExportBaseModel Export(AzureSqlDatabaseImportExportBaseModel exportRequest)
        {
            ...
            if (exportRequest.WaitForOperationComplete)
            {
                response = Communicator.Export(
        ...
        public AzureSqlDatabaseImportExportBaseModel ImportNewDatabase(AzureSqlDatabaseImportModel importRequest)
        {
            ...
            if (importRequest.WaitForOperationComplete)
            {
                response = Communicator.ImportNewDatabase(importRequest.ResourceGroupName, importRequest.ServerName, parameters);
            }
            else
            {
        ...
        public AzureSqlDatabaseImportExportBaseModel ImportExistingDatabase(AzureSqlDatabaseImportModel importRequest)
        {
            ...
            if (importRequest.WaitForOperationComplete)
            {
                response = Communicator.ImportExistingDatabase(importRequest.ResourceGroupName, importRequest.ServerName, importRequest.DatabaseName, parameters);
            }
            else
            {
