...
process {
    try {
        ...
        if ($PSBoundParameters.ContainsKey('StorageInMb')) {
            $PSBoundParameters.StorageSizeGb = [int]([Math]::floor($PSBoundParameters['StorageInMb'] / 1024))
        ...
        if (!$PSBoundParameters.ContainsKey('Version')) {
            $PSBoundParameters.Version = '8.0.21'
        }
        ...
        Write-Host $Msg
        $null = New-AzMySqlFlexibleServerDatabase -Name $DEFAULT_DB_NAME -ResourceGroupName $PSBoundParameters.ResourceGroupName -ServerName $PSBoundParameters.Name -Charset "utf8" -Collation "utf8_general_ci"
