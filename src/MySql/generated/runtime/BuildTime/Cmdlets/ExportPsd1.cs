...
using static Microsoft.Azure.PowerShell.Cmdlets.MySql.Runtime.PowerShell.PsHelpers;
  ...
  public class ExportPsd1 : PSCmdlet
  {
    ...
    protected override void ProcessRecord()
    {
      try
      {
        ...
        sb.AppendLine($@"{Indent}RootModule = '{"./Az.MySql.psm1"}'");
        ...
        sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: MySql cmdlets"}'");
        sb.AppendLine($@"{Indent}PowerShellVersion = '5.1'");
        sb.AppendLine($@"{Indent}DotNetFrameworkVersion = '4.7.2'");
        sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.MySql.private.dll"}'");
        ...
        var formatList = customFormatPs1xmlFiles.Prepend("./Az.MySql.format.ps1xml").ToPsList();
        ...
        sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule MySql".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
