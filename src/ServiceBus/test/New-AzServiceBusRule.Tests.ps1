...
Describe 'New-AzServiceBusRule' {
    It 'CreateExpanded' {
        ...
        $rule2 = New-AzServiceBusRule -ResourceGroupName $env.resourceGroup -NamespaceName $env.namespace -TopicName $env.topic -SubscriptionName $env.subscription -Name correlationRule1 -FilterType CorrelationFilter -ContentType contenttype -CorrelationFilterProperty @{a='b';c='d'} -SessionId sessionid -CorrelationId correlationid -MessageId messageid -Label label -ReplyTo replyto -ReplyToSessionId replytosessionid
