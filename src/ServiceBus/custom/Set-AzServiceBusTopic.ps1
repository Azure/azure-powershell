...
function Set-AzServiceBusTopic{
	...
	param(
        ...
        [Parameter(HelpMessage = "A value that indicates whether the topic is to be partitioned across multiple message brokers.")]
        [Microsoft.Azure.PowerShell.Cmdlets.ServiceBus.Category('Body')]
        [System.Management.Automation.SwitchParameter]
        # A value that indicates whether the topic is to be partitioned across multiple message brokers.
        ${EnablePartitioning},

        [Parameter(HelpMessage = "A value indicating if this topic requires duplicate detection.")]
        [Microsoft.Azure.PowerShell.Cmdlets.ServiceBus.Category('Body')]
        [System.Management.Automation.SwitchParameter]
    ...
    process{
        try{
            ...
            if ($hasRequiresSession) {
                $topic.RequiresSession = $RequiresSession
            }
            if ($hasStatus) {
                $topic.Status = $Status
            }
