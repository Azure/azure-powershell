    ...
    public class AuthenticationFactoryTests
    {
       ...
       public void VerifyValidateAuthorityFalseForOnPremise()
        ...
        public void AppServiceManagedIdentity()
        {
            ...
            const string endpoing = @"http://127.0.0.1:41217/MSI/token/";
            var expectedUri = $"{endpoing}?resource={resource}&api-version=2017-09-01";
            account.SetProperty(AzureAccount.Property.MSILoginUri , endpoing);
            account.SetProperty(AzureAccount.Property.MSILoginSecret , @"bar");
            const string expectedAccessToken = "foo";
            var expectedExpiresOn = DateTime.Parse("1/23/2019 7:15:42 AM +00:00");
            var responses = new Dictionary<string, ManagedServiceTokenInfoAppServiceManagedIdentityAdHoc>(StringComparer.OrdinalIgnoreCase)
            {
                {
                    expectedUri,
                    new ManagedServiceTokenInfoAppServiceManagedIdentityAdHoc()
                    {
            ...
            AzureSession.Instance.RegisterComponent(HttpClientOperationsFactory.Name, () => TestHttpOperationsFactory.Create(responses, _output), true);
            var msat = new ManagedServiceAccessTokenAppServiceManagedIdentityAdHoc(account, environment, tenant);
