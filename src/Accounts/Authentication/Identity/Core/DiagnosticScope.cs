...
namespace Microsoft.Azure.PowerShell.Authenticators.Identity.Core
{
    internal readonly struct DiagnosticScope : IDisposable
    {
        ...
        public void AddAttribute<T>(string name,
            ...
            T value)
        {
            AddAttribute(name, value, v => Convert.ToString(v, CultureInfo.InvariantCulture) ?? string.Empty);
        }
...
#pragma warning restore 109
        ...
        private class ActivityAdapter : IDisposable
        {
...
#else
                    _tagCollection = new ActivityTagsCollection() ?? _tagCollection;
...
#else
                    ActivityTagsCollection linkTagsCollection = new ActivityTagsCollection();
                    foreach (var tag in activity.Tags)
                    {
            ...
            public void AddLink(string traceparent, string tracestate, IDictionary<string, string> attributes)
            {
                ...
                _links = new List<Activity>() ?? _links;
