...
namespace Microsoft.Azure.Commands.Common.Authentication
{
    public class TestCoverage : ITestCoverage
    {
        ...
        private string GenerateCsvItem(string commandName, string parameterSetName, string parameters, string sourceScript, int scriptLineNumber, string startDateTime, string endDateTime, string duration, string sanitizeDuration, string sanitizePercentage, bool isSuccess)
        {
        ...
        public void LogRawData(AzurePSQoSEvent qos)
        {
            ...
            try
            {
                ...
                csvData.AppendLine();
                var duration = qos.Duration.TotalMilliseconds;
                var sanitizeDuration = qos.SanitizerInfo?.SanitizeDuration.TotalMilliseconds ?? 0;
                var sanitizePercentage = duration == 0 ? 0 : sanitizeDuration / duration;
                var csvItem = GenerateCsvItem(commandName, qos.ParameterSetName, qos.Parameters, sourceScriptName, qos.ScriptLineNumber, qos.StartTime.UtcDateTime.ToString("yyyy-MM-ddTHH:mm:ss"), qos.EndTime.UtcDateTime.ToString("yyyy-MM-ddTHH:mm:ss"), duration.ToString("F4"), sanitizeDuration.ToString("F4"), sanitizePercentage.ToString("P2"), qos.IsSuccess);
