# How-To
This document describes how to develop for `Azs.Subscriptions.Admin`.

## Building `Azs.Subscriptions.Admin`
...
## Testing `Azs.Subscriptions.Admin`
...
## Packing `Azs.Subscriptions.Admin`
...
There are multiple scripts created for performing different actions for developing `Azs.Subscriptions.Admin`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Azs.Subscriptions.Admin.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `Azs.Subscriptions.Admin.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Azs.Subscriptions.Admin`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Azs.Subscriptions.Admin` into the session.
