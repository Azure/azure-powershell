...
process {
    ...
    {
        if ($null -ne $StorageSubSystem -and $StorageSubSystem.Contains('/'))
        {
            $PSBoundParameters['StorageSubSystem'] = $StorageSubSystem.Split("/")[-1]
        }
    }

    # Generated cmdlet does not support {prefix}/{name} for Drive name, so extract the {name} part here
    ...
    Azs.Fabric.Admin.internal\Get-AzsDrive @PSBoundParameters
}
}
