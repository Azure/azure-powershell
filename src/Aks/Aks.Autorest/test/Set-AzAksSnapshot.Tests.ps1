if(($null -eq $TestName) -or ($TestName -contains 'Set-AzAksSnapshot'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Set-AzAksSnapshot.Recording.json'
...
Describe 'Set-AzAksSnapshot' {
    It 'UpdateExpanded' {
        ...
        $tags = @{"key1" = "value1"}
        $Snapshot = Set-AzAksSnapshot -ResourceGroupName $env.ResourceGroupName -ResourceName 'snapshot1' -Location $env.location -SnapshotType 'NodePool' -CreationDataSourceResourceId $nodepoolId2
