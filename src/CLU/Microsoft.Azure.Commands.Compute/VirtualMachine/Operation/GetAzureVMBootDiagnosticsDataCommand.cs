    ...
    public class GetAzureVMBootDiagnosticsDataCommand : VirtualMachineBaseCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            ExecuteClientAction(() =>
            {
                ...
                if (result.DiagnosticsProfile == null
                    || result.DiagnosticsProfile.BootDiagnostics == null
                    || result.DiagnosticsProfile.BootDiagnostics.Enabled == null
                    || !result.DiagnosticsProfile.BootDiagnostics.Enabled.Value
                    || result.DiagnosticsProfile.BootDiagnostics.StorageUri == null)
                {
                    ThrowTerminatingError
                (new ErrorRecord(
                    new InvalidOperationException(string.Format(CultureInfo.InvariantCulture,
                ...
                if (result.InstanceView == null
                    || result.InstanceView.BootDiagnostics == null)
                {
                    ThrowTerminatingError
                (new ErrorRecord(
                    new InvalidOperationException(string.Format(CultureInfo.InvariantCulture,
                ...
                {
                    var screenshotUri = new Uri(result.InstanceView.BootDiagnostics.ConsoleScreenshotBlobUri);
                ...
                {
                    var logUri = new Uri(result.InstanceView.BootDiagnostics.SerialConsoleLogBlobUri);
                    ...
                    using (var reader = new StreamReader(System.IO.File.OpenRead(localFile)))
                    {
                        string line;
