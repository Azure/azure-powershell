...
namespace Microsoft.Azure.Commands.Resources.Models.Authorization
{
    public class AuthorizationClient
    {
        ...
        public PSRoleAssignment CreateRoleAssignment(FilterRoleAssignmentsOptions parameters, string subscriptionId)
        {
            ...
            RoleAssignment assignment = AuthorizationManagementClient.RoleAssignments.Create(parameters.Scope, roleAssignmentId.ToString(), createParameters);
        ...
        public PSRoleDefinition CreateRoleDefinition(PSRoleDefinition roleDefinition)
        {
            ...
            {
                Name = newRoleDefinitionId.ToString(),
            ...
            try
            {
                roleDef = AuthorizationManagementClient.RoleDefinitions
                    .CreateOrUpdate(roleDefinition.AssignableScopes.First(), newRoleDefinitionId.ToString(), parameters)
                    .ToPSRoleDefinition();
            }
            catch (CloudException ce)
            {
                if (ce.Response.StatusCode == HttpStatusCode.Unauthorized && ce.Body.Code.Equals("TenantNotAllowed", StringComparison.OrdinalIgnoreCase))
                {
        ...
        private static void ValidateRoleDefinition(PSRoleDefinition roleDefinition)
        {
            ...
            if (roleDefinition.AssignableScopes == null || 
                !roleDefinition.AssignableScopes.Any() || 
                roleDefinition.AssignableScopes.Any(s => string.IsNullOrWhiteSpace(s)))
            {
