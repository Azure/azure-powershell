...
Describe 'New/Get/Update/Remove-AzStorageTaskAssignment, List task assignment reports' {
    It 'New/Get/Update/Remove-AzStorageTaskAssignment, List task assignment reports' {
        ...
        $reportprefix = "testc1"
        $assignment1 = New-AzStorageTaskAssignment -AccountName $env.TaskAssignmentAccount -Name $assignmentname1 -ResourceGroupName $env.TaskAssignmentResourceGroup -TaskId $env.TaskID -ReportPrefix $reportprefix -TriggerType RunOnce -Description "task assignment1" -Enabled:$false
