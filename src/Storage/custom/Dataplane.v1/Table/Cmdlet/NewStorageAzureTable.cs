...
namespace Microsoft.WindowsAzure.Commands.Storage.Table.Cmdlet
{
    ...
    [Cmdlet("New", Azure.Commands.ResourceManager.Common.AzureRMConstants.AzurePrefix + "StorageTable!V1"),OutputType(typeof(AzureStorageTable))]
    public class NewAzureStorageTableCommand : StorageCloudTableCmdletBase
    {
        ...
        internal AzureStorageTable CreateAzureTable(string name)
        {
            ...
            if (!created)
            {
                throw new ResourceAlreadyExistException(String.Format(ResourceV1.TableAlreadyExists, name));
            }
