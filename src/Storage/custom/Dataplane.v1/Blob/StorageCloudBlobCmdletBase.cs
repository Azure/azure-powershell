...
namespace Microsoft.WindowsAzure.Commands.Storage
{
    ...
    public class StorageCloudBlobCmdletBase : StorageCloudCmdletBase<IStorageBlobManagement>
    {
        ...
        internal void ValidatePipelineCloudBlob(CloudBlob blob)
        {
            ...
            {
                throw new ArgumentException(String.Format(ResourceV1.InvalidBlobName, blob.Name));
            }
            ...
            //{
            //    throw new ResourceNotFoundException(String.Format(ResourceV1.BlobNotFound, blob.Name, blob.Container.Name));
            //}
        }
        ...
        internal void ValidatePipelineCloudBlobContainer(CloudBlobContainer container)
        {
            if (null == container)
            {
                throw new ArgumentException(String.Format(ResourceV1.ObjectCannotBeNull, typeof(CloudBlobContainer).Name));
            }

            if (!NameUtil.IsValidContainerName(container.Name))
            {
                throw new ArgumentException(String.Format(ResourceV1.InvalidContainerName, container.Name));
            }
            ...
            //{
            //    throw new ResourceNotFoundException(String.Format(ResourceV1.ContainerNotFound, container.Name));
            //}
        }
