...
namespace Microsoft.WindowsAzure.Commands.Storage.Common
{
        ...
        where T : class
    {
        ...
        internal AzureStorageContext GetCmdletStorageContext(IStorageContext inContext)
        {
            ...
            if (context != null)
            {
                WriteDebugLog(String.Format(ResourceV1.UseStorageAccountFromContext, context.StorageAccountName));
            }
            else
            {
        ...
        private static TimeSpan? ConvertToTimeSpan(int timeoutInSeconds)
        {
            ...
            throw new ArgumentOutOfRangeException(string.Format(CultureInfo.CurrentCulture, ResourceV1.InvalidTimeoutValue, timeoutInSeconds));
        }
        ...
        {
            connectionString = Environment.GetEnvironmentVariable(ResourceV1.EnvConnectionString);
        ...
        private CloudStorageAccount GetStorageAccountFromConnectionString(string connectionString)
        {
            ...
            WriteDebugLog(ResourceV1.GetStorageAccountFromEnvironmentVariable);
            ...
            {
                WriteVerboseWithTimestamp(ResourceV1.CannotGetStorageAccountFromEnvironmentVariable);
                throw;
            }
        }
        ...
        {
            Debug.Assert(e != null, ResourceV1.ExceptionCannotEmpty);
        ...
        internal void InitMutltiThreadResources()
        {
            ...
            var summary = String.Format(ResourceV1.TransmitActiveSummary, _taskScheduler.TotalTaskCount,
                _taskScheduler.FinishedTaskCount, _taskScheduler.FailedTaskCount, _taskScheduler.ActiveTaskCount);
            var activity = string.Format(ResourceV1.TransmitActivity, MyInvocation.MyCommand);
        ...
        {
            WriteVerbose(String.Format(ResourceV1.TransferSummary, _taskScheduler.TotalTaskCount,
                _taskScheduler.FinishedTaskCount, _taskScheduler.FailedTaskCount, _taskScheduler.ActiveTaskCount));
        }
        ...
        protected override void BeginProcessing()
        {
            ...
            WriteDebugLog(String.Format(ResourceV1.InitOperationContextLog, GetType().Name, CmdletOperationContext.ClientRequestId));
...
// TODO: Remove IfDef code
        ...
        protected override void EndProcessing()
        {
            ...
            var message = string.Format(ResourceV1.EndProcessingLog,
