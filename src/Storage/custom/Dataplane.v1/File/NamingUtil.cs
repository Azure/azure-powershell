...
namespace Microsoft.WindowsAzure.Commands.Storage.File
{
    ...
    internal static class NamingUtil
    {
        ...
        public static string[] ValidatePath(string path, out bool isDirectory, bool isFileName = false)
        {
            ...
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, ResourceV1.FileNameShouldNotEndWithSlash, path));
            }

            if (path.Length > MaxPathLength)
            {
                throw new ArgumentOutOfRangeException(string.Format(CultureInfo.CurrentCulture, ResourceV1.PathTooLong, path, MaxPathLength));
            }
            ...
            foreach (var fileName in result)
            {
                ...
                {
                    throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, ResourceV1.PathInvalid, fileName));
                }
            }
        ...
        public static void ValidateShareName(string shareName, bool validatingPrefix)
        {
            ...
            throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, ResourceV1.ShareNameInvalid, shareName));
        }
    }
}
