...
namespace Microsoft.WindowsAzure.Commands.Storage.File
{
    ...
    public abstract class StorageFileDataManagementCmdletBase : AzureStorageFileCmdletBase
    {
        ...
        {
            return this.Force || this.OutputStream.ConfirmAsync(string.Format(CultureInfo.CurrentCulture, ResourceV1.OverwriteConfirmation, Util.ConvertToString(destination))).Result;
        }
        ...
        {
            return this.Force || await this.OutputStream.ConfirmAsync(string.Format(CultureInfo.CurrentCulture, ResourceV1.OverwriteConfirmation, Util.ConvertToString(destination)));
        }
        ...
        protected SingleTransferContext GetTransferContext(ProgressRecord record, long totalTransferLength)
        {
            ...
            transferContext.ProgressHandler = new TransferProgressHandler((transferProgress) =>
            {
                if (record != null)
                {
                    ...
                    record.StatusDescription = string.Format(CultureInfo.CurrentCulture, ResourceV1.FileTransmitStatus, record.PercentComplete);
