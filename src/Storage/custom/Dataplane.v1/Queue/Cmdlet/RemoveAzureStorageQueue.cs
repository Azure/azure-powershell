...
namespace Microsoft.WindowsAzure.Commands.Storage.Queue
{
    ...
    [Cmdlet("Remove", Azure.Commands.ResourceManager.Common.AzureRMConstants.AzurePrefix + "StorageQueue!V1", SupportsShouldProcess = true),OutputType(typeof(Boolean))]
    public class RemoveAzureStorageQueueCommand : StorageQueueBaseCmdlet
    {
        ...
        public override void ExecuteCmdlet()
        {
            if (ShouldProcess(Name, "Remove queue"))
            {
                ...
                if (success)
                {
                    result = String.Format(ResourceV1.RemoveQueueSuccessfully, Name);
                }
                else
                {
                    result = String.Format(ResourceV1.RemoveQueueCancelled, Name);
                }
