...
namespace Microsoft.Azure.Commands.Storage.File.Cmdlet
{
    ...
    [Cmdlet("Stop", Azure.Commands.ResourceManager.Common.AzureRMConstants.AzurePrefix + "StorageFileCopy!V2", SupportsShouldProcess = true), OutputType(typeof(void))]
    public class StopAzureStorageFileCopyCommand : AzureStorageFileCmdletBase
    {
        ...
        private async Task StopCopyFile(long taskId, IStorageFileManagement localChannel, CloudFile file, string copyId)
        {
            ...
            if (string.IsNullOrEmpty(copyId) || Force)
            {
                ...
                if (!Force)
                {
                    string confirmation = String.Format(ResourceV2.ConfirmAbortFileCopyOperation, file.SnapshotQualifiedUri.ToString(), abortCopyId);
                    if (!await OutputStream.ConfirmAsync(confirmation).ConfigureAwait(false))
                    {
                        string cancelMessage = String.Format(ResourceV2.StopCopyOperationCancelled, file.SnapshotQualifiedUri.ToString());
            ...
            string message = String.Format(ResourceV2.StopCopyFileSuccessfully, file.SnapshotQualifiedUri.ToString());
