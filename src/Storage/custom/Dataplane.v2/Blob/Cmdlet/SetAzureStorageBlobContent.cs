...
namespace Microsoft.Azure.Commands.Storage.Blob
{
    ...
    [Cmdlet("Set", Azure.Commands.ResourceManager.Common.AzureRMConstants.AzurePrefix + "StorageBlobContent!V2", SupportsShouldProcess = true, DefaultParameterSetName = ManualParameterSet),OutputType(typeof(AzureStorageBlob))]
    public class SetAzureBlobContentCommand : StorageDataMovementCmdletBase
    {
        ...
        {
            string activity = String.Format(ResourceV2.SendAzureBlobActivity, filePath, blob.Name, blob.Container.Name);
            string status = ResourceV2.PrepareUploadingBlob;
        ...
        internal string GetFullSendFilePath(string fileName)
        {
            ...
            {
                throw new ArgumentException(ResourceV2.FileNameCannotEmpty);
            }
        ...
        internal void SetAzureBlobContent(string fileName, string blobName)
        {
            ...
            if (!isFile)
            {
                WriteWarning(String.Format(ResourceV2.CannotSendDirectory, filePath));
            }
        }
        ...
        private void ValidateBlobProperties(Hashtable properties)
        {
            ...
            foreach (DictionaryEntry entry in properties)
            {
                ...
                {
                    throw new ArgumentException(String.Format(ResourceV2.InvalidBlobProperties, entry.Key.ToString(), entry.Value.ToString()));
                }
            }
        }
