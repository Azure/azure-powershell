...
namespace Microsoft.Azure.Commands.Storage.Blob.Cmdlet
{
    ...
    [Cmdlet("Stop", Azure.Commands.ResourceManager.Common.AzureRMConstants.AzurePrefix + "StorageBlobCopy!V2", SupportsShouldProcess = true, DefaultParameterSetName = NameParameterSet),OutputType(typeof(AzureStorageBlob))]
    ...
    public class StopAzureStorageBlobCopy : StorageCloudBlobCmdletBase
    {
        ...
        private async Task StopCopyBlob(long taskId, IStorageBlobManagement localChannel, CloudBlob blob, string copyId, bool fetchCopyIdFromBlob = false)
        {
            ...
            if (string.IsNullOrEmpty(specifiedCopyId) || Force)
            {
                ...
                if (!Force)
                {
                    string confirmation = String.Format(ResourceV2.ConfirmAbortCopyOperation, blob.Name, blob.Container.Name, abortCopyId);
                    if (!await OutputStream.ConfirmAsync(confirmation).ConfigureAwait(false))
                    {
                        string cancelMessage = String.Format(ResourceV2.StopCopyOperationCancelled, blob.Name, blob.Container.Name);
            ...
            string message = String.Format(ResourceV2.StopCopyBlobSuccessfully, blob.Name, blob.Container.Name);
