...
function Test-NewRegisteredServerWithIdentityError
{
    ...
    if (IsLive)
    {
        $storageSyncRegistryPath = "HKLM:\SOFTWARE\Microsoft\Azure\StorageSync"

        try
        {
            $serverType = Get-ItemPropertyValue -Path $storageSyncRegistryPath -Name "ServerType" -ErrorAction Stop
        }
        catch
        {
            throw "Test prerequisite failed: unable to determine the Azure File Sync agent server type. $($_.Exception.Message)"
        }

        if ($serverType -ne "HybridServer")
        {
            throw "Test prerequisite failed: Test-NewRegisteredServerWithIdentityError requires a non-Arc, non-Azure-VM server. The Azure File Sync agent reports '$serverType'."
        }
    }
    ...
    try
    {
        ...
        $registrationFailedAsExpected = $false
        try
        {
            Write-Verbose "Resource: <auto-generated> | Loc: $resourceLocation | Type : RegisteredServer"
            $job = Register-AzStorageSyncServer -ResourceGroupName $resourceGroupName -StorageSyncServiceName $storageSyncServiceName -AsJob -AssignIdentity
        ...
        catch
        {
            ...
            $registrationFailedAsExpected = $true
        }

        if (-not $registrationFailedAsExpected)
        {
            throw "Register-AzStorageSyncServer unexpectedly succeeded on a server without managed identity."
        }
...
<#
.SYNOPSIS
Test PatchRegisteredServer with a server ID that doesn't match the local server
.DESCRIPTION
SmokeTest
#>
function Test-PatchRegisteredServer-InvalidServerId
{
    # Setup
    $resourceGroupName = Get-ResourceGroupName
    Write-Verbose "RecordMode : $(Get-StorageTestMode)"
    try
    {
        ...
        $job = Register-AzStorageSyncServer -ResourceGroupName $resourceGroupName -StorageSyncServiceName $storageSyncServiceName -AssignIdentity -AsJob 
        $job | Wait-Job
        $validRegisteredServer = get-job -Id $job.Id | receive-job -Keep

        $wrongServerId = New-Guid

        Write-Verbose "Patch Server: registered server ID: $($validRegisteredServer.ServerId), invalid server ID: $($wrongServerId)"

        try
        {
            $job = Set-AzStorageSyncServer -ResourceGroupName $resourceGroupName -StorageSyncServiceName $storageSyncServiceName -ServerId $wrongServerId -Identity -AsJob
            $job | Wait-Job
        }
         catch 
        {
           $expectedMessage = "The provided ServerId '$($wrongServerId)' does not match the local machine's server ID '$($validRegisteredServer.ServerId)'. Run this command on the correct server."
           if($_.Exception.Message -ne $expectedMessage)
           {
               throw $_
           }
        ...
        Write-Verbose "Unregister Server: $($validRegisteredServer.ServerId)"
        Unregister-AzStorageSyncServer -Force -ResourceGroupName $resourceGroupName -StorageSyncServiceName $storageSyncServiceName -ServerId $validRegisteredServer.ServerId -AsJob | Wait-Job
