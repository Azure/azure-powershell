# How-To
This document describes how to develop for `Az.DiagnosticSetting`.

## Building `Az.DiagnosticSetting`
...
There are multiple scripts created for performing different actions for developing `Az.DiagnosticSetting`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DiagnosticSetting.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DiagnosticSetting.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DiagnosticSetting`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DiagnosticSetting` into the session.
