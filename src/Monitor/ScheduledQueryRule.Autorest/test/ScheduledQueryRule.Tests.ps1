...
Describe 'ScheduledQueryRule' {
    It 'CRUD' {
        ...
        New-AzScheduledQueryRule -Name $env.scheduledQueryRuleName -ResourceGroupName $env.resourceGroupName -Location $env.location -DisplayName test-rule -Scope $env.vmId -Severity 4 -WindowSize ([System.TimeSpan]::New(0,10,0)) -EvaluationFrequency ([System.TimeSpan]::New(0,5,0)) -CriterionAllOf $condition

        $rule = Get-AzScheduledQueryRule -Name $env.scheduledQueryRuleName -ResourceGroupName $env.resourceGroupName
        $rule.Name | Should -Be $env.scheduledQueryRuleName
        $update = Update-AzScheduledQueryRule -Name $env.scheduledQueryRuleName -ResourceGroupName $env.resourceGroupName -Description "Updated description"
