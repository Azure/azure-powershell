...
A terminal status of `Succeeded` means the configuration is ready to run; `RequiresAttention` means the errors on `ErrorPermission` and `ErrorResource` must be resolved first.
