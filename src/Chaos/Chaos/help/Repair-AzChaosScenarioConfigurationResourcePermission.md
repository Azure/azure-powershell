...
The service stores a terminal permission-fix record that you can read later with `Get-AzChaosScenarioConfigurationResourcePermission`.
...
Uses the common PowerShell `-WhatIf` switch.
This is client-side preview behavior: PowerShell stops the request before it is sent, so the service does not calculate role assignments and no permission-fix result is stored.
