...
namespace Microsoft.Azure.Commands.KeyVault.SecurityDomain.Cmdlets
{
    ...
    public class RestoreSecurityDomain : SecurityDomainCmdlet
    {
        ...
        [Parameter(HelpMessage = "Name of the managed HSM.", Mandatory = true, ParameterSetName = ByRestoreBlob)]
        ...
        [Parameter(HelpMessage = "Specify the path to the prepared security domain data.", Mandatory = true, ParameterSetName = ByRestoreBlob)]
        ...
        [ValidateNotNullOrEmpty]
        public string ExchangeKeyOutputPath { get; set; }
        ...
        [Parameter(HelpMessage = "When specified, SecurityDomainPath should be encrypted security domain data generated by Restore-AzKeyVaultSecurityDomainBlob.", Mandatory = true, ParameterSetName = ByRestoreBlob)]
        ...
        public override void DoExecuteCmdlet()
        {
            ...
            {
                if (ShouldProcess($"managed HSM {Name}", $"download exported key to '{ExchangeKeyOutputPath}'"))
                {
                    var exchangeKey = Client.DownloadSecurityDomainExchangeKeyAsPem(Name, CancellationToken);
                    ExchangeKeyOutputPath = ResolveUserPath(ExchangeKeyOutputPath);

                    if (!AzureSession.Instance.DataStore.FileExists(ExchangeKeyOutputPath) || Force || ShouldContinue(string.Format(Resources.FileOverwriteMessage, ExchangeKey), Resources.FileOverwriteCaption))
                    {
                        AzureSession.Instance.DataStore.WriteFile(ExchangeKeyOutputPath, exchangeKey);
                        WriteDebug($"Security domain data of managed HSM '{Name}' downloaded to '{ExchangeKeyOutputPath}'.");
                    }
                }
            }
