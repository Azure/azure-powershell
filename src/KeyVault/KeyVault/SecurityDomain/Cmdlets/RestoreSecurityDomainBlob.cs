...
{
    [Cmdlet(VerbsData.Restore, ResourceManager.Common.AzureRMConstants.AzurePrefix + CmdletNoun.KeyVault + "SecurityDomainBlob", SupportsShouldProcess = true, DefaultParameterSetName = ByName)]
    ...
    public class RestoreSecurityDomainBlob : SecurityDomainCmdlet
    {
        ...
        [Parameter(HelpMessage = "Information about the exchange key used to decrypt the security domain data. See examples for how it is constructed.", Mandatory = true)]
        [ValidateNotNullOrEmpty]
        ...
        public override void DoExecuteCmdlet()
        {
            ...
            if (ShouldProcess($"managed HSM {Name}", $"restore security domain data from file \"{SecurityDomainPath}\""))
            {
                ...
                ExchangeKey = this.ResolveUserPath(ExchangeKey);
                ...
                Client.RestoreSecurityDomainBlob(encryptedSecurityDomain, CancellationToken);
                ...
                if (!AzureSession.Instance.DataStore.FileExists(SecurityDomainRestoreBlob) || Force || ShouldContinue(string.Format(Resources.FileOverwriteMessage, OutputPath), Resources.FileOverwriteCaption))
                {
