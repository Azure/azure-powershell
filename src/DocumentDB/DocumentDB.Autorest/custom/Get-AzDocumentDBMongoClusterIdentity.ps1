...
.Link
https://learn.microsoft.com/powershell/module/az.documentdb/get-azdocumentdbmongoclusteridentity
#>
function Get-AzDocumentDBMongoClusterIdentity {
...
param(
    ...
    process {
        try {
            $cluster = Az.DocumentDB\Get-AzDocumentDBMongoCluster @PSBoundParameters -ErrorAction Stop

            [PSCustomObject]@{
                PrincipalId          = $cluster.IdentityPrincipalId
                TenantId             = $cluster.IdentityTenantId
                Type                 = $cluster.IdentityType
                UserAssignedIdentity = $cluster.IdentityUserAssignedIdentity
            }
        } catch {
            throw
        }
