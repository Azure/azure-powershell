if(($null -eq $TestName) -or ($TestName -contains 'Switch-AzDocumentDBMongoCluster'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Switch-AzDocumentDBMongoCluster.Recording.json'
...
Describe 'Switch-AzDocumentDBMongoCluster' {
    BeforeAll {
        $rg = $env.promoteRg
    ...
    It 'rejects a source cluster that does not match the replica source' {
        ...
        { Switch-AzDocumentDBMongoCluster -Name $replica -ResourceGroupName $rg `
