...
# ----------------------------------------------------------------------------------
param([switch]$Isolated, [switch]$Live, [switch]$Record, [switch]$Playback, [string]$TestCase)
...
try {
  ...
  $testCaseFolder = join-Path $PSScriptRoot "test\$TestCase"
  $testFolder = Join-Path $PSScriptRoot 'test'
  #Invoke-Pester -Script @{ Path = $testCaseFolder } -EnableExit -OutputFile (Join-Path $testFolder "$moduleName-TestResults.xml")
}
Finally
{
  if ($TestMode -ne 'playback') {
