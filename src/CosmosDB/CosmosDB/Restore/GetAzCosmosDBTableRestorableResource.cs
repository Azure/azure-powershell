...
{
    [Cmdlet(VerbsCommon.Get, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBTableRestorableResource", DefaultParameterSetName = NameParameterSet), OutputType(typeof(PSTablesToRestore))]
    public class GetAzCosmosDBTableRestorableResource : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            IEnumerable<RestorableTableResourcesGetResult> restorableTables = CosmosDBManagementClient.RestorableTableResources.ListWithHttpMessagesAsync(Location, DatabaseAccountInstanceId, RestoreLocation, dateTimeInUtc.ToString()).GetAwaiter().GetResult().Body;
            
            List<string> tableNames = new List<string>();

            foreach(RestorableTableResourcesGetResult restorableTable in restorableTables)
            {
                tableNames.Add(restorableTable.Name);
            }

            WriteObject(new PSTablesToRestore(tableNames));
        }
    }
}
