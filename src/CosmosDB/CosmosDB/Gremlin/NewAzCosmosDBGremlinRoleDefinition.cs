...
{
    [Cmdlet(VerbsCommon.New, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBGremlinRoleDefinition", DefaultParameterSetName = FieldsDataActionsParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSGremlinRoleDefinitionGetResults))]
    public class NewAzCosmosDBGremlinRoleDefinition : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            Id = string.IsNullOrWhiteSpace(Id) ? Guid.NewGuid().ToString() : GremlinRoleHelper.ParseToRoleDefinitionId(Id);
            ...
            if (readGremlinRoleDefinitionGetResults != null)
            {
                throw new ConflictingResourceException(message: string.Format(ExceptionMessage.ConflictGremlinRoleResourceId, "Definition", Id));
            }
            ...
            if (ShouldProcess(Id, "Creating a new CosmosDB Gremlin Role Definition"))
            {
