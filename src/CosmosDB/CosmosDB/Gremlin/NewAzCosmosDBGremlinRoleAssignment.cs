...
{
    [Cmdlet(VerbsCommon.New, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBGremlinRoleAssignment", DefaultParameterSetName = FieldsParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSGremlinRoleAssignmentGetResults))]
    public class NewAzCosmosDBGremlinRoleAssignment : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            {
                IEnumerable<GremlinRoleDefinitionResource> gremlinRoleDefinitions = CosmosDBManagementClient.GremlinResources.ListGremlinRoleDefinitionsWithHttpMessagesAsync(ResourceGroupName, AccountName).GetAwaiter().GetResult().Body.Where(r => String.Equals(r.RoleName, RoleDefinitionName));
            ...
            Id = string.IsNullOrWhiteSpace(Id) ? Guid.NewGuid().ToString() : GremlinRoleHelper.ParseToRoleAssignmentId(Id);
            ...
            if (readGremlinRoleAssignmentGetResults != null)
            {
                throw new ConflictingResourceException(message: string.Format(ExceptionMessage.ConflictGremlinRoleResourceId, "Assignment", Id));
            }

            GremlinRoleAssignmentResource gremlinRoleAssignmentCreateUpdateParameters = new GremlinRoleAssignmentResource
            {
                RoleDefinitionId = GremlinRoleHelper.ParseToFullyQualifiedRoleDefinitionId(RoleDefinitionId, DefaultProfile.DefaultContext.Subscription.Id, ResourceGroupName, AccountName),
                Scope = GremlinRoleHelper.ParseToFullyQualifiedScope(Scope, DefaultProfile.DefaultContext.Subscription.Id, ResourceGroupName, AccountName),
                PrincipalId = PrincipalId
            };

            if (ShouldProcess(Id.ToString(), "Creating a new CosmosDB Gremlin Role Assignment"))
            {
                GremlinRoleAssignmentResource gremlinRoleAssignmentGetResults = CosmosDBManagementClient.GremlinResources.CreateUpdateGremlinRoleAssignmentWithHttpMessagesAsync(ResourceGroupName, AccountName, Id, gremlinRoleAssignmentCreateUpdateParameters).GetAwaiter().GetResult().Body;
