    ...
    public class GetAzCosmosDBAccount : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            if (ParameterSetName.Equals(NameParameterSet, StringComparison.OrdinalIgnoreCase))
            {
                if (Name != null)
                {
                    var response = CosmosClient.DatabaseAccounts.GetWithHttpMessagesAsync(ResourceGroupName, Name).Result; 
                    WriteObject(response);
                }
                else
                {
                    var response = CosmosClient.DatabaseAccounts.ListByResourceGroupWithHttpMessagesAsync(ResourceGroupName).Result;
                    WriteObject(response);
                }
            }
            else if (ParameterSetName.Equals(ResourceIdParameterSet, StringComparison.OrdinalIgnoreCase))
            {
                ...
                var response = CosmosClient.DatabaseAccounts.GetWithHttpMessagesAsync(Name, ResourceGroupName).Result;
                WriteObject(response);
            }
        }
    }
