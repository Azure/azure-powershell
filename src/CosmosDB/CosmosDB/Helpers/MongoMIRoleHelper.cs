...
{
    public static class MongoMIRoleHelper
    {
        private static Regex roleDefinitionPrefix = new Regex("/subscriptions/(?<Subscription>.*)/resourceGroups/(?<ResourceGroup>.*)/providers/Microsoft.DocumentDB/databaseAccounts/(?<DatabaseAccount>.*)/mongoMIRoleDefinitions/(?<RoleDefinitionId>.*)", RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static Regex roleAssignmentPrefix = new Regex("/subscriptions/(?<Subscription>.*)/resourceGroups/(?<ResourceGroup>.*)/providers/Microsoft.DocumentDB/databaseAccounts/(?<DatabaseAccount>.*)/mongoMIRoleAssignments/(?<RoleAssignmentId>.*)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public static string ParseToFullyQualifiedRoleDefinitionId(string id, string subscription, string resourceGroup, string databaseAccount)
        {
            ...
            {
                return $"/subscriptions/{subscription}/resourceGroups/{resourceGroup}/providers/Microsoft.DocumentDB/databaseAccounts/{databaseAccount}/mongoMIRoleDefinitions/{id}";
            }
        }
