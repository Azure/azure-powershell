...
{
    public static class TableRoleHelper
    {
        private static Regex roleDefinitionPrefix = new Regex("/subscriptions/(?<Subscription>.*)/resourceGroups/(?<ResourceGroup>.*)/providers/Microsoft.DocumentDB/databaseAccounts/(?<DatabaseAccount>.*)/tableRoleDefinitions/(?<RoleDefinitionId>.*)", RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static Regex roleAssignmentPrefix = new Regex("/subscriptions/(?<Subscription>.*)/resourceGroups/(?<ResourceGroup>.*)/providers/Microsoft.DocumentDB/databaseAccounts/(?<DatabaseAccount>.*)/tableRoleAssignments/(?<RoleAssignmentId>.*)", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public static string ParseToFullyQualifiedRoleDefinitionId(string id, string subscription, string resourceGroup, string databaseAccount)
        {
            ...
            {
                return $"/subscriptions/{subscription}/resourceGroups/{resourceGroup}/providers/Microsoft.DocumentDB/databaseAccounts/{databaseAccount}/tableRoleDefinitions/{id}";
            }
        }
