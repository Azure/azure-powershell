...
namespace Microsoft.Azure.Commands.CosmosDB
{
    ...
    public class NewAzCosmosDBSqlSpatialSpec : AzureCosmosDBCmdletBase
    {
        ...
        [Parameter(Mandatory = false, HelpMessage = Constants.SpatialPathHelpMessage)]
        public string Path { get; set; }

        public override void ExecuteCmdlet()
        {
            PSSqlSpatialSpec spatialSpec = new PSSqlSpatialSpec();
            if (SpatialType != null)
                spatialSpec.SpatialType = SpatialType;
            if (Path != null)
                spatialSpec.Path = Path;

            WriteObject(spatialSpec);
            return;
        }
    }
}
