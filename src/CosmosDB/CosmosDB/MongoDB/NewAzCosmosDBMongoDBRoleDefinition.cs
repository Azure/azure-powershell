...
{
    [Cmdlet(VerbsCommon.New, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBMongoDBRoleDefinition", DefaultParameterSetName = FieldsDataActionsParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSMongoDBRoleDefinitionGetResults))]
    public class NewAzCosmosDBMongoDBRoleDefinition : AzureCosmosDBCmdletBase
    {
        ...
        [ValidateNotNullOrEmpty]
        [Parameter(Mandatory = true, HelpMessage = Constants.MongoDBRoleNameHelpMessage)]
        public string RoleName { get; set; }

        [PSArgumentCompleter("BuiltInRole", "CustomRole")]
        [Parameter(Mandatory = true, HelpMessage = Constants.MongoDBTypeHelpMessage)]
        ...
        [ValidateNotNull]
        [Parameter(Mandatory = true, HelpMessage = Constants.MongoRoleDefinitionPrivilegesHelpMessage)]
        public PSMongoPrivilege[] Privileges { get; set; }

        [ValidateNotNull]
        [Parameter(Mandatory = false, HelpMessage = Constants.MongoDBInheritedRolesHelpMessage)]
        public PSMongoRole[] Roles { get; set; }
        ...
        public override void ExecuteCmdlet()
        {
            ...
            IList<Privilege> privileges = Privileges.Select(privilege => PSMongoPrivilege.ToSDKModel(privilege)).ToList();
            ...
            MongoRoleDefinitionCreateUpdateParameters mongoRoleDefinitionCreateUpdateParameters = new MongoRoleDefinitionCreateUpdateParameters(
                roleName : RoleName,
                type : (MongoRoleDefinitionType)Enum.Parse(typeof(MongoRoleDefinitionType), Type),
