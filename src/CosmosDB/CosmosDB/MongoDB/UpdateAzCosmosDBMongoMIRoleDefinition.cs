...
{
    [Cmdlet("Update", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBMongoMIRoleDefinition", DefaultParameterSetName = FieldsParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSMongoMIRoleDefinitionGetResults), typeof(ResourceNotFoundException))]
    public class UpdateAzCosmosDBMongoMIRoleDefinition : AzureCosmosDBCmdletBase
    {
        ...
        public PSMongoMIRoleDefinitionGetResults InputObject { get; set; }
        ...
        public override void ExecuteCmdlet()
        {
            ...
            Id = MongoMIRoleHelper.ParseToRoleDefinitionId(Id);

            MongoMIRoleDefinitionResource readMongoMIRoleDefinitionGetResults = null;
            try
            {
                readMongoMIRoleDefinitionGetResults = CosmosDBManagementClient.MongoMiResources.GetMongoMIRoleDefinition(ResourceGroupName, AccountName, Id);
            }
            catch (ErrorResponseException e)
            {
                if (e.Response.StatusCode == System.Net.HttpStatusCode.NotFound)
                {
                    throw new ResourceNotFoundException(message: string.Format(ExceptionMessage.NotFoundMongoMIRoleResourceId, "Definition", Id), innerException: e);
                }
                else
                {
                    throw e;
            ...
            AssignableScope = AssignableScope ?? new List<string>(readMongoMIRoleDefinitionGetResults.AssignableScopes);
            AssignableScope = new List<string>(AssignableScope.Select(s => MongoMIRoleHelper.ParseToFullyQualifiedScope(s, DefaultProfile.DefaultContext.Subscription.Id, ResourceGroupName, AccountName)));

            MongoMIRoleDefinitionResource mongoMIRoleDefinitionCreateUpdateParameters = new MongoMIRoleDefinitionResource
            {
                RoleName = RoleName ?? readMongoMIRoleDefinitionGetResults.RoleName,
                PropertiesType = (RoleDefinitionType)Enum.Parse(typeof(RoleDefinitionType), Type ?? readMongoMIRoleDefinitionGetResults.Type),
                AssignableScopes = AssignableScope,
                Permissions = permissions ?? readMongoMIRoleDefinitionGetResults.Permissions,
            };

            if (ShouldProcess(Id, "Updating an existing CosmosDB MongoMI Role Definition"))
            {
                MongoMIRoleDefinitionResource mongoMIRoleDefinitionGetResults = CosmosDBManagementClient.MongoMiResources.CreateUpdateMongoMIRoleDefinitionWithHttpMessagesAsync(ResourceGroupName, AccountName, Id, mongoMIRoleDefinitionCreateUpdateParameters).GetAwaiter().GetResult().Body;
                WriteObject(new PSMongoMIRoleDefinitionGetResults(mongoMIRoleDefinitionGetResults));
            }
