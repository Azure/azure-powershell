...
{
    [Cmdlet(VerbsCommon.New, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBMongoMIRoleAssignment", DefaultParameterSetName = FieldsParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSMongoMIRoleAssignmentGetResults))]
    public class NewAzCosmosDBMongoMIRoleAssignment : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            {
                IEnumerable<MongoMIRoleDefinitionResource> mongoMIRoleDefinitions = CosmosDBManagementClient.MongoMiResources.ListMongoMIRoleDefinitionsWithHttpMessagesAsync(ResourceGroupName, AccountName).GetAwaiter().GetResult().Body.Where(r => String.Equals(r.RoleName, RoleDefinitionName));
            ...
            Id = string.IsNullOrWhiteSpace(Id) ? Guid.NewGuid().ToString() : MongoMIRoleHelper.ParseToRoleAssignmentId(Id);
            ...
            if (readMongoMIRoleAssignmentGetResults != null)
            {
                throw new ConflictingResourceException(message: string.Format(ExceptionMessage.ConflictMongoMIRoleResourceId, "Assignment", Id));
            }

            MongoMIRoleAssignmentResource mongoMIRoleAssignmentCreateUpdateParameters = new MongoMIRoleAssignmentResource
            {
                RoleDefinitionId = MongoMIRoleHelper.ParseToFullyQualifiedRoleDefinitionId(RoleDefinitionId, DefaultProfile.DefaultContext.Subscription.Id, ResourceGroupName, AccountName),
                Scope = MongoMIRoleHelper.ParseToFullyQualifiedScope(Scope, DefaultProfile.DefaultContext.Subscription.Id, ResourceGroupName, AccountName),
                PrincipalId = PrincipalId
            };

            if (ShouldProcess(Id.ToString(), "Creating a new CosmosDB MongoMI Role Assignment"))
            {
                MongoMIRoleAssignmentResource mongoMIRoleAssignmentGetResults = CosmosDBManagementClient.MongoMiResources.CreateUpdateMongoMIRoleAssignmentWithHttpMessagesAsync(ResourceGroupName, AccountName, Id, mongoMIRoleAssignmentCreateUpdateParameters).GetAwaiter().GetResult().Body;
