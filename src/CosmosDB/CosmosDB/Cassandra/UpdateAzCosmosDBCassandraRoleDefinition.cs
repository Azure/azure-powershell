...
{
    [Cmdlet("Update", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBCassandraRoleDefinition", DefaultParameterSetName = FieldsParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSCassandraRoleDefinitionGetResults), typeof(ResourceNotFoundException))]
    public class UpdateAzCosmosDBCassandraRoleDefinition : AzureCosmosDBCmdletBase
    {
        ...
        public PSCassandraRoleDefinitionGetResults InputObject { get; set; }

        public override void ExecuteCmdlet()
        {
            List<PermissionAutoGenerated3> permissions = null;
            ...
            else if (ParameterSetName.Equals(ObjectParameterSet))
            {
                ...
                Id = InputObject.Id;
                permissions = new List<PermissionAutoGenerated3>(InputObject.Permissions);
            ...
            Id = CassandraRoleHelper.ParseToRoleDefinitionId(Id);
            ...
            catch (ErrorResponseAutoGeneratedException e)
            {
                ...
                {
                    throw new ResourceNotFoundException(message: string.Format(ExceptionMessage.NotFoundCassandraRoleResourceId, "Definition", Id), innerException: e);
                }
                else
                {
                    throw e;
            ...
            AssignableScope = AssignableScope ?? new List<string>(readCassandraRoleDefinitionGetResults.AssignableScopes);
            ...
            {
                RoleName = RoleName ?? readCassandraRoleDefinitionGetResults.RoleName,
                PropertiesType = (RoleDefinitionType)Enum.Parse(typeof(RoleDefinitionType), Type ?? readCassandraRoleDefinitionGetResults.Type),
                AssignableScopes = AssignableScope,
                Permissions = permissions ?? readCassandraRoleDefinitionGetResults.Permissions,
            };

            if (ShouldProcess(Id, "Updating an existing CosmosDB Cassandra Role Definition"))
            {
