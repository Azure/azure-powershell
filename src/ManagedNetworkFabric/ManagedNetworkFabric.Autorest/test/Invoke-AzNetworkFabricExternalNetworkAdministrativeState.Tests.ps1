...
Describe 'Invoke-AzNetworkFabricExternalNetworkAdministrativeState' {
    It 'UpdateExpanded' {
        {
            Invoke-AzNetworkFabricExternalNetworkAdministrativeState -L3IsolationDomainName $global:config.l3domain.name -ExternalNetworkName $global:config.externalnetwork.name -ResourceGroupName $global:config.common.resourceGroupName -SubscriptionId $global:config.common.subscriptionId
        } | Should -Not -Throw
    }
