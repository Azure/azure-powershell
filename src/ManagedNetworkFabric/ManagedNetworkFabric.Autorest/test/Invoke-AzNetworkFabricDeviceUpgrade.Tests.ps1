...
Describe 'Invoke-AzNetworkFabricDeviceUpgrade' {
    It 'UpgradeExpanded' {
        {
            Invoke-AzNetworkFabricDeviceUpgrade -Name $global:config.networkDevice.name -Version $global:config.networkDevice.upgradeVersion -ResourceGroupName $global:config.common.resourceGroupName -SubscriptionId $global:config.common.subscriptionId
        } | Should -Not -Throw
    }
