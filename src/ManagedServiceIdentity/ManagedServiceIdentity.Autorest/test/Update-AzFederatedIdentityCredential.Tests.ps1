if(($null -eq $TestName) -or ($TestName -contains 'Update-AzFederatedIdentityCredential'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Update-AzFederatedIdentityCredential.Recording.json'
...
Describe 'Update-AzFederatedIdentityCredential' {
    It 'UpdateExpanded' {
        $fic = Update-AzFederatedIdentityCredential -ResourceGroupName $env.ficResourceGroup -IdentityName $env.ficUserIdentityName -Name $env.ficName01 -Issuer $($env.Issuer01 + "-updated") -Subject $($env.Subject01 + "-updated") -Audience @("updated")
    ...
    It 'UpdateViaIdentityExpanded' {
        ...
        $fic = Update-AzFederatedIdentityCredential -InputObject $fic -Issuer $($env.Issuer01 + "-updated2") -Subject $($env.Subject01 + "-updated2") -Audience @("updated2")
        $fic.Name | Should -Be $env.ficName01
    ...
    It 'UpdateViaIdentityExpandedAlias' {
