...
namespace Microsoft.Azure.Commands.Management.IotHub
{
    ...
    [Cmdlet("Invoke", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "IotHubModuleMethod", DefaultParameterSetName = ResourceParameterSet, SupportsShouldProcess = true)]
    [OutputType(typeof(PSCloudToDeviceMethodResult))]
    public class InvokeAzIotHubModuleMethod : IotHubBaseCmdlet
    {
        ...
        [Parameter(Position = 2, Mandatory = true, ParameterSetName = InputObjectParameterSet, HelpMessage = "Target device's module id.")]
        [Parameter(Position = 2, Mandatory = true, ParameterSetName = ResourceIdParameterSet, HelpMessage = "Target device's module id.")]
        [Parameter(Position = 3, Mandatory = true, ParameterSetName = ResourceParameterSet, HelpMessage = "Target device's module id.")]
        [ValidateNotNullOrEmpty]
        public string ModuleId { get; set; }

        [Parameter(Mandatory = true, HelpMessage = "The name of the method to invoke on this device module.")]
        public string Name { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "The payload for the method to invoke on this device module.")]
        ...
        public override void ExecuteCmdlet()
        {
            if (ShouldProcess(this.IotHubName, Properties.Resources.InvokeIotHubDeviceMethod))
            {
                ...
                CloudToDeviceMethodResult result = serviceClient.InvokeDeviceMethodAsync(this.DeviceId, this.ModuleId, method).GetAwaiter().GetResult();
