...
function Update-AzStorageMoverS3WithHmacEndpoint {
    ...
    param(
        ...
        [Parameter(HelpMessage="The Azure Key Vault secret URI which stores the access key. Use empty string to clean-up existing value.")]
        [string]
        ${CredentialsAccessKeyUri},

        [Parameter(ParameterSetName = 'UpdateExpanded')]
        [Parameter(ParameterSetName = 'UpdateViaIdentityExpanded')]
        [Parameter(HelpMessage="The Azure Key Vault secret URI which stores the secret key. Use empty string to clean-up existing value.")]
        [string]
    ...
    process {
        $Properties = [Microsoft.Azure.PowerShell.Cmdlets.StorageMover.Models.S3WithHmacEndpointUpdateProperties]::New()
