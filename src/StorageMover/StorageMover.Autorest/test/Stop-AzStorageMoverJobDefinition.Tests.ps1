if(($null -eq $TestName) -or ($TestName -contains 'Stop-AzStorageMoverJobDefinition'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Stop-AzStorageMoverJobDefinition.Recording.json'
...
Describe 'Stop-AzStorageMoverJobDefinition' {
    It 'Stop' {
        ...
        $storageMoverName = "stopTestC2C" + $env.RandomString
        ...
        $endpointName = "stopTestC2CSource" + $env.RandomString
        $endpoint = New-AzStorageMoverMultiCloudConnectorEndpoint -Name $endpointName -ResourceGroupName $env.ResourceGroupName -StorageMoverName $storageMoverName -MultiCloudConnectorId $env.MultiCloudConnectorId -AWSS3BucketId $env.AwsS3BucketId -Description "CloudToCloud source for Stop test"
        $endpoint.Name | Should -Be $endpointName

        # Create a container endpoint as target
        $targetEndpointName = "stopTestC2CTarget" + $env.RandomString
        ...
        # Create a project
        $projectName = "stopTestC2CProj" + $env.RandomString
        ...
        $jobName = "stopTestC2CJob" + $env.RandomString
        ...
        # Wait for the job to enter a running state before stopping
        $jobRunName = $startResult.JobRunResourceId1.Split("/")[-1]
        $maxPolls = 20
        $pollInterval = 15
        $runningStates = @("Started", "Running")
        for ($i = 0; $i -lt $maxPolls; $i++) {
            ...
            if ($jobRun.Status -in $runningStates) {
                break
            }
        ...
        # Stop the job
        $stopResult = Stop-AzStorageMoverJobDefinition -JobDefinitionName $jobName -ProjectName $projectName -ResourceGroupName $env.ResourceGroupName -StorageMoverName $storageMoverName
        $stopResult | Should -Not -Be $null

        # Poll the job run status until it reaches a terminal state
        $maxPolls = 60
        $pollInterval = 30
