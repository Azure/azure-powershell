...
```powershell
New-AzConfluentOrganizationApiKey -ResourceGroupName confluent-rg -OrganizationName confluentorg-01 -EnvironmentId env-abc123 -ClusterId lkc-abc123 -Description "API key for automation"
```

```output
Id          Description                   
--          -----------                   
key-new123  API key for automation        
```

This command creates a new API key for the organization.

### Example 2: Create API key with name
```powershell
New-AzConfluentOrganizationApiKey -ResourceGroupName confluent-rg -OrganizationName confluentorg-01 -EnvironmentId env-abc123 -ClusterId lkc-abc123 -Name "prod-api-key" -Description "Production API key"
```

This command creates an API key with a specific name.
