# How-To
This document describes how to develop for `Az.ConnectedVMwarevSphere`.

## Building `Az.ConnectedVMwarevSphere`
...
## Testing `Az.ConnectedVMwarevSphere`
...
## Packing `Az.ConnectedVMwarevSphere`
...
There are multiple scripts created for performing different actions for developing `Az.ConnectedVMwarevSphere`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.ConnectedVMwarevSphere.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.ConnectedVMwarevSphere.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.ConnectedVMwarevSphere`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.ConnectedVMwarevSphere` into the session.
