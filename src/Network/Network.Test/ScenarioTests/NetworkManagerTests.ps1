...
function Test-NetworkManagerSecurityAdminRuleManualAggregationCRUD
{
    ...
    New-AzNetworkManagerSecurityAdminConfiguration -ResourceGroupName $rgname -NetworkManagerName $networkManagerName -Name $SecurityConfigurationName -Description "DESCription" -DeleteExistingNSG -NetworkGroupAddressSpaceAggregationOption $NetworkGroupAddressSpaceAggregationOption
    
    $securityConfig = Get-AzNetworkManagerSecurityAdminConfiguration -ResourceGroupName $rgname -NetworkManagerName $networkManagerName -Name $SecurityConfigurationName
    Assert-NotNull $securityConfig;
    Assert-AreEqual $SecurityConfigurationName $securityConfig.Name;
    Assert-AreEqual $NetworkGroupAddressSpaceAggregationOption $securityConfig.NetworkGroupAddressSpaceAggregationOption;

    $NetworkGroupAddressSpaceAggregationOption = @()
    $securityConfig.Description = "A different description."
    $securityConfig.NetworkGroupAddressSpaceAggregationOption = $NetworkGroupAddressSpaceAggregationOption
    $securityConfig = Set-AzNetworkManagerSecurityAdminConfiguration -InputObject $securityConfig
    Assert-NotNull $securityConfig;
    Assert-AreEqual "A different description." $securityConfig.Description;
    Assert-AreEqual $SecurityConfigurationName $securityConfig.Name;
...
function Test-NetworkManagerVerifierWorkspaceReachabilityAnalysisIntentCRUD
{
    ...
    try{
        ...
        New-AzNetworkManagerVerifierWorkspaceReachabilityAnalysisIntent -ResourceGroupName $rgName -NetworkManagerName $networkManagerName -VerifierWorkspaceName $verifierWorkspaceName -Location $rglocation -Name $reachabilityAnalysisIntentName -SourceResourceId "/subscriptions/c9295b92-3574-4021-95a1-26c8f74f8359/resourceGroups/ipam-test-rg/providers/Microsoft.Compute/virtualMachines/testVM" -DestinationResourceId "/subscriptions/c9295b92-3574-4021-95a1-26c8f74f8359/resourceGroups/ipam-test-rg/providers/Microsoft.Compute/virtualMachines/ipam-test-vm-integration-test" -IpTraffic $ipTraffic
        ...
        Assert-AreEqual $reachabilityAnalysisIntent.Properties.SourceResourceId $sourceResourceId;
        Assert-AreEqual $reachabilityAnalysisIntent.Properties.DestinationResourceId $destinationResourceId;

        # Delete analysis intent
        $job = Remove-AzNetworkManagerVerifierWorkspace -ResourceGroupName $rgName -NetworkManagerName $networkManagerName -VerifierWorkspaceName $verifierWorkspaceName -Name $reachabilityAnalysisIntentName -PassThru -Force -AsJob;
        $job | Wait-Job;
        $removeResult = $job | Receive-Job;
	}
    finally{
