...
"@;
		...
		# AzureRm.Resources\New-AzResourceGroupDeployment -ResourceGroupName "$rgn" -TemplateFile "$templateFile" -TemplateParameterFile $paramFile
    New-AzResourceGroupDeployment -ResourceGroupName "$rgn" -TemplateFile "$templateFile" -TemplateParameterFile $paramFile
}
...
function Test-Troubleshoot
{
    ...
    try 
    {
        ...
        Get-AzNetworkWatcherTroubleshootingResult -NetworkWatcher $nw -TargetResourceId $gw.Id
    }
    finally
    {
        # Cleanup
...
function Test-FlowLog
{
    ...
    try 
    {
        ...
        $job = Set-AzNetworkWatcherConfigFlowLog -NetworkWatcher $nw -TargetResourceId $getNsg.Id -EnableFlowLog $true -StorageAccountId $sto.Id -AsJob
        $job | Wait-Job
...
function Test-ConnectivityCheck
{
    ...
    try 
    {
        ...
        #Connectivity check
        $job = Test-AzNetworkWatcherConnectivity -NetworkWatcher $nw -SourceId $vm.Id -DestinationAddress "bing.com" -DestinationPort 80 -AsJob
		$job | Wait-Job
...
function Test-ConnectionMonitor
{
    ...
    try 
    {
        ...
        $cmList = Get-AzNetworkWatcherConnectionMonitor -NetworkWatcher $nw
