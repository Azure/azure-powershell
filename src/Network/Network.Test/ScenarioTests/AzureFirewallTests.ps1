...
Tests AzureFirewallCRUD.
#>
function Test-AzureFirewallWithFirewallPolicyCRUD {
    # Setup
    $rgname = Get-ResourceGroupName
    $azureFirewallName = Get-ResourceName
    $resourceTypeParent = "Microsoft.Network/AzureFirewalls"
    $firewallPolicyLocation = Get-ProviderLocation $resourceTypeParent "eastus"
    ...
    try {
        ...
        $publicip = New-AzPublicIpAddress -ResourceGroupName $rgname -name $publicIpName -location $location -AllocationMethod Static -Sku Standard
         ...
         # Create AzureFirewallPolicy (with no rules, ThreatIntel is in Alert mode by default)
        $azureFirewallPolicy = New-AzFirewallPolicy -Name $azureFirewallPolicyName -ResourceGroupName $rgname -Location $firewallPolicyLocation
