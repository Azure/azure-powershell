...
namespace Microsoft.Azure.Management.Network
{
    ...
    internal partial class LoadBalancersOperations : IServiceOperations<NetworkManagementClient>, ILoadBalancersOperations
    {
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<AzureOperationResponse<MigratedPools>> MigrateToIpBasedWithHttpMessagesAsync(string groupName, string loadBalancerName, MigrateLoadBalancerToIpBasedRequest parameters = default(MigrateLoadBalancerToIpBasedRequest), Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (groupName == null)
            {
            ...
            if (groupName != null)
            {
                ...
                {
                    throw new ValidationException(ValidationRules.Pattern, "groupName", "^[a-z][a-z0-9]*$");
                }
            }
            if (loadBalancerName == null)
            {
            ...
            if (loadBalancerName != null)
            {
                ...
                {
                    throw new ValidationException(ValidationRules.Pattern, "loadBalancerName", "^[a-z][a-z0-9]*$");
                }
            }
            if (Client.SubscriptionId == null)
            {
            ...
            if (_shouldTrace)
            {
                ...
                tracingParameters.Add("apiVersion", apiVersion);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "MigrateToIpBased", tracingParameters);
            }
            // Construct URL
            var _baseUrl = Client.BaseUri.AbsoluteUri;
            ...
            // Create Result
            var _result = new AzureOperationResponse<MigratedPools>();
            _result.Request = _httpRequest;
            _result.Response = _httpResponse;
            if (_httpResponse.Headers.Contains("x-ms-request-id"))
            {
            ...
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = Rest.Serialization.SafeJsonConvert.DeserializeObject<MigratedPools>(_responseContent, Client.DeserializationSettings);
                }
                catch (JsonException ex)
                {
                    _httpRequest.Dispose();
