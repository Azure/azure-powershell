...
{
    [Cmdlet("Invoke", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "VirtualNetworkGatewayExecuteMigration",
        DefaultParameterSetName = ParameterSetNames.ByName, SupportsShouldProcess = true), OutputType(typeof(PSVirtualNetworkGateway))]
    public class InvokeAzureVirtualNetworkGatewayExecuteMigrationCommand : VirtualNetworkGatewayBaseCmdlet
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void Execute()
        {
            ...
            string shouldProcessMessage = string.Format($"Execute Invoke-AzVirtualNetworkGatewayExecuteMigration for ResourceGroupName {0} VirtualNetworkGateway {1}", this.ResourceGroupName, this.Name);
            if (ShouldProcess(shouldProcessMessage))
            {
                this.VirtualNetworkGatewayClient.InvokeExecuteMigration(this.ResourceGroupName, this.Name);
