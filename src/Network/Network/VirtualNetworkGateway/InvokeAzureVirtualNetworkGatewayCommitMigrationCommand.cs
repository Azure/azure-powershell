...
{
    [Cmdlet("Invoke", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "VirtualNetworkGatewayCommitMigration",
        DefaultParameterSetName = ParameterSetNames.ByName, SupportsShouldProcess = true), OutputType(typeof(PSVirtualNetworkGateway))]
    public class InvokeAzureVirtualNetworkGatewayCommitMigrationCommand : VirtualNetworkGatewayBaseCmdlet
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void Execute()
        {
            ...
            string shouldProcessMessage = string.Format($"Execute Invoke-AzVirtualNetworkGatewayCommitMigration for ResourceGroupName {0} VirtualNetworkGateway {1}", this.ResourceGroupName, this.Name);
            if (ShouldProcess(shouldProcessMessage))
            {
                this.VirtualNetworkGatewayClient.InvokeCommitMigration(this.ResourceGroupName, this.Name);
