...
namespace Microsoft.Azure.Commands.Network
{
    ...
    public class SetAzureRmRouteMapRuleCommand : NetworkBaseCmdlet
    {
        ...
        {
            base.Execute();
            if (this.RouteMap == null || this.RouteMap.Rules == null)
            {
                throw new ArgumentException("The given route map does not exist nor route map rule.");
            }

            var routeMapRules = this.RouteMap.Rules.Where(
                   rule => string.Equals(rule.Name, this.Name, StringComparison.CurrentCultureIgnoreCase));

            if (routeMapRules == null || !routeMapRules.Any())
            {
                throw new ArgumentException("Route Map Rule with specified name does not exist");
            }

            var routingPolicyIndex = this.RouteMap.Rules.IndexOf(routeMapRules.Single());
            ...
            this.RouteMap.Rules[routingPolicyIndex] = routeMapRule;
