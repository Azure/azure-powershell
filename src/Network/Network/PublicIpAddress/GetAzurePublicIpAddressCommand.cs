        ...
        public override void Execute()
        {
            ...
            else
            {
                ...
                if (ShouldListByResourceGroup(ResourceGroupName, Name))
                {
                    if (ParameterSetName.Contains("ScaleSetIp"))
                    {
                        IPage<PublicIPAddressAutoGenerated> scaleSetPublicIpPage;
                        List<PublicIPAddressAutoGenerated> scaleSetPublicIPList;

                        if (string.IsNullOrEmpty(this.VirtualMachineIndex))
                        {
                            scaleSetPublicIpPage =
                                this.PublicIpAddressClient.ListVirtualMachineScaleSetPublicIPAddresses(
                                    this.ResourceGroupName,
                                    this.VirtualMachineScaleSetName);
                            scaleSetPublicIPList = ListNextLink<PublicIPAddressAutoGenerated>.GetAllResourcesByPollingNextLink(scaleSetPublicIpPage, this.PublicIpAddressClient.ListVirtualMachineScaleSetPublicIPAddressesNext);
                        }
                        else
                        {
                            scaleSetPublicIpPage =
                            ...
                            scaleSetPublicIPList = ListNextLink<PublicIPAddressAutoGenerated>.GetAllResourcesByPollingNextLink(scaleSetPublicIpPage, this.PublicIpAddressClient.ListVirtualMachineScaleSetVMPublicIPAddressesNext);
                        }

                        var psScaleSetPublicIps = new List<PSPublicIpAddress>();

                        foreach (var publicIp in scaleSetPublicIPList)
                        {
                            ...
                            psScaleSetPublicIps.Add(psPublicIp);
                        }

                        WriteObject(TopLevelWildcardFilter(ResourceGroupName, Name, psScaleSetPublicIps), true);
                        return;
                    }
                    else
                    {
                ...
                List<PublicIPAddress> publicIPList = ListNextLink<PublicIPAddress>.GetAllResourcesByPollingNextLink(publicipPage, this.PublicIpAddressClient.ListNext);
