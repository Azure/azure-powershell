        ...
        public override void Execute()
        {
            ...
            else
            {
                ...
                if (ShouldListByResourceGroup(ResourceGroupName, Name))
                {
                    if (ParameterSetName.Contains("ScaleSetNic"))
                    {
                        IPage<MNM.NetworkInterfaceAutoGenerated> scaleSetNicPage;
                        List<MNM.NetworkInterfaceAutoGenerated> scaleSetNicList;
                        ...
                        if (string.IsNullOrEmpty(this.VirtualMachineIndex))
                        {
                            ...
                            scaleSetNicList = ListNextLink<MNM.NetworkInterfaceAutoGenerated>.GetAllResourcesByPollingNextLink(scaleSetNicPage, this.NetworkInterfaceClient.ListVirtualMachineScaleSetNetworkInterfacesNext);
                        }
                        else
                        {
                            scaleSetNicPage =
                            ...
                            scaleSetNicList = ListNextLink<MNM.NetworkInterfaceAutoGenerated>.GetAllResourcesByPollingNextLink(scaleSetNicPage, this.NetworkInterfaceClient.ListVirtualMachineScaleSetVMNetworkInterfacesNext);
                        }

                        var psScaleSetNetworkInterfaces = new List<PSNetworkInterface>();

                        foreach (var nic in scaleSetNicList)
                        {
                            var psNic = this.ToPsNetworkInterface(nic);
                            psNic.ResourceGroupName = NetworkBaseCmdlet.GetResourceGroup(psNic.Id);
                            psScaleSetNetworkInterfaces.Add(psNic);
                        }

                        WriteObject(TopLevelWildcardFilter(ResourceGroupName, Name, psScaleSetNetworkInterfaces), true);
                        return;
                    }
                    else
                    {
