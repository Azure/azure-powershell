...
Creates a new Azure Database for PostgreSQL flexible server with a specific configuration.
...
Creates a new Azure Database for PostgreSQL flexible server elastic cluster with a specific configuration.
If subscription is not passed explicitly, it's taken from default context.
...
Restores a backup of an existing Azure Database for PostgreSQL flexible server onto a new server with its own specific configuration.
If subscription is not passed explicitly, it's taken from default context.
