...
```
Get-AzPostgreSqlFlexibleServerTuningOptionRecommendation -ResourceGroup <String> -ServerName <String>
...
Lists all index recommendations in an Azure Database for PostgreSQL flexible server with server name, resource group, and subscription explicitly passed as an arguments.
...
Lists all table recommendations in an Azure Database for PostgreSQL flexible server with server name, resource group, and subscription explicitly passed as an arguments.
If subscription is not passed explicitly, it's taken from default context.
...
Lists all table recommendations of VacuumTable type in an Azure Database for PostgreSQL flexible server with server name, resource group, and subscription explicitly passed as an arguments.
If subscription is not passed explicitly, it's taken from default context.
