...
Checks if a name is available or already used for an Azure Database for PostgreSQL flexible server with resource type, server name, location, and subscription explicitly passed as an arguments. If subscription is not passed explicitly, it's taken from default context.
