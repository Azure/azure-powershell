...
{
    [Cmdlet(VerbsCommon.Remove, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + SynapseConstants.SynapsePrefix + SynapseConstants.KqlScript,
        DefaultParameterSetName = RemoveByName, SupportsShouldProcess = true)]
    [OutputType(typeof(bool))]
    public class RemoveAzureSynapseKqlScript : SynapseArtifactsCmdletBase
    {
        ...
        [Parameter(ValueFromPipelineByPropertyName = false, ParameterSetName = RemoveByName,
            Mandatory = true, HelpMessage = HelpMessages.KqlScriptName)]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            ConfirmAction(
                Force.IsPresent,
                string.Format(Resources.RemoveSynapseKqlScript, Name),
                string.Format(Resources.RemovingSynapseKqlScript, this.Name, this.WorkspaceName),
                Name,
                () =>
                {
                    SynapseAnalyticsClient.DeleteKqlScript(this.Name);
                    if (PassThru)
                    {
                        WriteObject(true);
                    }
