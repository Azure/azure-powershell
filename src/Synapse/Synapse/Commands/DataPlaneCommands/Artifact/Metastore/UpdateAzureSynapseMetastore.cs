...
{
    [Cmdlet(VerbsData.Update, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + SynapseConstants.SynapsePrefix + SynapseConstants.Metastore,
        DefaultParameterSetName = UpdateByName, SupportsShouldProcess = true)]
    [OutputType(typeof(PSMetastore))]
    public class UpdateAzureSynapseMetastore : SynapseArtifactsCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (this.ShouldProcess(this.WorkspaceName, String.Format(Resources.UpdatingSynapseMetastore, this.DatabaseName, this.WorkspaceName)))
            {
                WriteObject(new PSMetastore(SynapseAnalyticsClient.UpdateMetaStore(this.DatabaseName, this.InputFolder), this.WorkspaceName, this.DatabaseName));
            }
        }
    }
}
