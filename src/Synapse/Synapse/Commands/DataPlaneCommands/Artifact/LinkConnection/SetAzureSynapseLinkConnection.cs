...
{
    [Cmdlet(VerbsCommon.Set, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + SynapseConstants.SynapsePrefix + SynapseConstants.LinkConnection,
        DefaultParameterSetName = SetByName, SupportsShouldProcess = true)]
    [Alias("New-" + ResourceManager.Common.AzureRMConstants.AzureRMPrefix + SynapseConstants.SynapsePrefix + SynapseConstants.LinkConnection)]
    [OutputType(typeof(PSLinkConnectionResource))]
    public class SetAzureSynapseLinkConnection : SynapseArtifactsCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (this.ShouldProcess(this.WorkspaceName, String.Format(Resources.SettingSynapseLinkConnection, this.Name, this.WorkspaceName)))
            {
                string rawJsonContent = SynapseAnalyticsClient.ReadJsonFileContent(this.TryResolvePath(DefinitionFile));
                WriteObject(new PSLinkConnectionResource(SynapseAnalyticsClient.CreateOrUpdateLinkConnection(this.Name, rawJsonContent), this.WorkspaceName));
            }
        }
    }
}