...
{
    [Cmdlet(VerbsLifecycle.Start, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + SynapseConstants.SynapsePrefix + SynapseConstants.LinkConnection, 
        DefaultParameterSetName = StartByName, SupportsShouldProcess = true)]
    [OutputType(typeof(bool))]
    public class StartAzureSynapseLinkConnection : SynapseArtifactsCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (this.ShouldProcess(this.Name, string.Format(Resources.StartingSynapseLinkConnection, this.Name, this.WorkspaceName)))
            {
                SynapseAnalyticsClient.StartLinkConnection(this.Name);
                if (PassThru)
                {
                    WriteObject(true);
                }
