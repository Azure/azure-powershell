...
{
    [Cmdlet(VerbsCommon.Remove, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + SynapseConstants.SynapsePrefix + SynapseConstants.WorkspacePackage,
        DefaultParameterSetName = RemoveByName, SupportsShouldProcess = true)]
    [OutputType(typeof(bool))]
    public class RemoveAzureSynapseWorkspacePackage : SynapseArtifactsCmdletBase
    {
        ...
        [Parameter(ValueFromPipelineByPropertyName = false, ParameterSetName = RemoveByName,
            Mandatory = true, HelpMessage = HelpMessages.WorkspacePackageName)]
        ...
        [Parameter(ValueFromPipeline = true, ParameterSetName = RemoveByInputObject,
            Mandatory = true, HelpMessage = HelpMessages.WorkspacePackageObject)]
        [ValidateNotNull]
        public PSSynapseWorkspacePackage InputObject { get; set; }
        ...
        public override void ExecuteCmdlet()
        {
            ...
            ConfirmAction(
                Force.IsPresent,
                string.Format(Resources.RemoveSynapseWorkspacePackage, Name),
                string.Format(Resources.RemovingSynapseWorkspacePackage, this.Name, this.WorkspaceName),
                Name,
                () =>
                {
                    SynapseAnalyticsClient.DeletePackage(this.Name);
                    if (PassThru)
                    {
                        WriteObject(true);
                    }
