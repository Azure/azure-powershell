...
using System.Linq;
using ProxyResource = Microsoft.Azure.Management.Synapse.Models.ProxyResource;
...
namespace Microsoft.Azure.Commands.Synapse.Models.Auditing
{
    ...
    public abstract class SqlAuditAdapter<AuditPolicyType, AuditModelType> where AuditPolicyType : ProxyResource
                                                                           where AuditModelType : WorkspaceDevOpsAuditModel
    {
        /// <summary>
        ...
        internal virtual IList<DiagnosticSettingsResource> GetDiagnosticsEnablingAuditCategory(
            string resourceGroup, string workspaceName,
        ...
        internal void GetAuditingSettings(string resourceGroup, string workspaceName, AuditModelType model)
        {
            AuditPolicyType policy = GetAuditingPolicy(resourceGroup, workspaceName);

            model.DiagnosticsEnablingAuditCategory = GetDiagnosticsEnablingAuditCategory(resourceGroup, 
                workspaceName, out string nextDiagnosticSettingsName);
        ...
        protected abstract AuditPolicyType GetAuditingPolicy(string resourceGroup, string workspaceName);
        ...
        protected virtual DiagnosticSettingsResource CreateDiagnosticSettings(
            string resourceGroup, string workspaceName, string settingsName,
        ...
        {
            return Communicator.UpdateDiagnosticSettings(settings, model.ResourceGroupName, model.WorkspaceName);
        }

        protected virtual bool RemoveDiagnosticSettings(
            DiagnosticSettingsResource settings, AuditModelType model)
        {
            return Communicator.RemoveDiagnosticSettings(settings.Name, model.ResourceGroupName, model.WorkspaceName);
        }
        ...
        private bool CreateDiagnosticSettingsForModel(
            ...
            AuditModelType model)
        {
            DiagnosticSettingsResource settings = CreateDiagnosticSettings(model.ResourceGroupName, model.WorkspaceName, model.NextDiagnosticSettingsName, eventHubName, eventHubAuthorizationRuleId, workspaceId);
        ...
        internal virtual void PolicizeStorageInfo(AuditModelType model, ProxyResource policy)
        {
            ...
            {
                Guid? principalId = Communicator.AssignWorkspaceIdentityIfNotAssigned(model.ResourceGroupName, model.WorkspaceName);
                AzureCommunicator.AssignRoleForWorkspaceIdentityOnStorageIfNotAssigned(model.StorageAccountResourceId, principalId.Value, RoleAssignmentId);
            }
            else
            {
    ...
    public abstract class SqlUserAuditAdapter<AuditPolicyType, ExtendedAuditPolicyType, AuditModelType> : SqlAuditAdapter<ExtendedAuditPolicyType, AuditModelType>
        ...
        where AuditModelType : WorkspaceAuditModel
    {
        ...
        protected abstract bool SetAuditingPolicy(string resourceGroup, string workspaceName, AuditPolicyType policy);

        protected abstract bool SetExtendedAuditingPolicy(string resourceGroup, string workspaceName, ExtendedAuditPolicyType policy);
        ...
        protected override bool SetAudit(AuditModelType model)
        {
            if (string.IsNullOrEmpty(model.PredicateExpression))
            {
                ...
                return SetAuditingPolicy(model.ResourceGroupName, model.WorkspaceName, policy);
            }
            ...
            return SetExtendedAuditingPolicy(model.ResourceGroupName, model.WorkspaceName, extendedPolicy);
        }
    ...
    public sealed class SynapseWorkspaceAuditAdapter : SqlUserAuditAdapter<ServerBlobAuditingPolicy, ExtendedServerBlobAuditingPolicy, WorkspaceAuditModel>
    {
        public SynapseWorkspaceAuditAdapter(IAzureContext context, Guid roleAssignmentId = default(Guid)) : base(context, roleAssignmentId)
        {
        }

        protected override ExtendedServerBlobAuditingPolicy GetAuditingPolicy(string resourceGroup, string workspaceName)
        {
            return Communicator.GetAuditingPolicy(resourceGroup, workspaceName);
        }

        protected override bool SetAuditingPolicy(string resourceGroup, string workspaceName, ServerBlobAuditingPolicy policy)
        {
            return Communicator.SetAuditingPolicy(resourceGroup, workspaceName, policy);
        }

        protected override bool SetExtendedAuditingPolicy(string resourceGroup, string workspaceName, ExtendedServerBlobAuditingPolicy policy)
        {
            return Communicator.SetExtendedAuditingPolicy(resourceGroup, workspaceName, policy);
        }

        protected override void PolicizeAuditModel(WorkspaceAuditModel model, ProxyResource policy)
        {
            dynamic dynamicPolicy = (dynamic)policy;
    ...
    public sealed class SynapseSqlPoolAuditAdapter : SqlUserAuditAdapter<SqlPoolBlobAuditingPolicy, ExtendedSqlPoolBlobAuditingPolicy, SqlPoolAuditModel>
    {
        public SynapseSqlPoolAuditAdapter(IAzureContext context, string sqlPoolName, Guid roleAssignmentId = default(Guid)) : base(context, roleAssignmentId)
        {
        ...
        protected override ExtendedSqlPoolBlobAuditingPolicy GetAuditingPolicy(string resourceGroup, string workspaceName)
        {
            return Communicator.GetAuditingPolicy(resourceGroup, workspaceName, SqlPoolName);
        }

        protected override bool SetAuditingPolicy(string resourceGroup, string workspaceName, SqlPoolBlobAuditingPolicy policy)
        {
            return Communicator.SetAuditingPolicy(resourceGroup, workspaceName, SqlPoolName, policy);
        }

        protected override bool SetExtendedAuditingPolicy(string resourceGroup, string workspaceName, ExtendedSqlPoolBlobAuditingPolicy policy)
        {
            return Communicator.SetExtendedAuditingPolicy(resourceGroup, workspaceName, SqlPoolName, policy);
        }
        ...
        protected override DiagnosticSettingsResource UpdateDiagnosticSettings(
            DiagnosticSettingsResource settings, SqlPoolAuditModel model)
        {
            return Communicator.UpdateDiagnosticSettings(settings,
                model.ResourceGroupName, model.WorkspaceName, SqlPoolName);
        }

        protected override bool RemoveDiagnosticSettings(
            DiagnosticSettingsResource settings, SqlPoolAuditModel model)
        {
            return Communicator.RemoveDiagnosticSettings(settings.Name, model.ResourceGroupName, model.WorkspaceName, SqlPoolName);
        }

        protected override void ModelizeAuditPolicy(SqlPoolAuditModel model, ExtendedSqlPoolBlobAuditingPolicy policy)
        {
            base.ModelizeAuditPolicy(model, policy);

            model.AuditAction = ExtractAuditActions(policy.AuditActionsAndGroups);
            model.SqlPoolName = SqlPoolName;
        }

        protected override bool SetAudit(SqlPoolAuditModel model)
        {
            return base.SetAudit(model);
        }

        protected override void PolicizeAuditModel(SqlPoolAuditModel model, ProxyResource policy)
        {
            dynamic dynamicPolicy = (dynamic)policy;
        ...
        internal string SqlPoolName { get; set; }
    }
}