...
function Update-AzKustoDatabase {
    [OutputType([Microsoft.Azure.PowerShell.Cmdlets.Kusto.Models.Api20210101.IDatabase])]
    ...
    process {
        try {
            if ($PSBoundParameters['Kind'] -eq 'ReadWrite') {
                $Parameter = [Microsoft.Azure.PowerShell.Cmdlets.Kusto.Models.Api20210101.ReadWriteDatabase]::new()
            }
            else {
                $Parameter = [Microsoft.Azure.PowerShell.Cmdlets.Kusto.Models.Api20210101.ReadOnlyFollowingDatabase]::new()
            }
