...
function setupEnv() {
  ...
  #  # Dynamically create a service principal for tests to avoid hardcoded AAD principal issues
  #  $testAppDisplayName = "KustoTestPrincipal-" + $rstr1
  #  Write-Host "Creating test service principal: $testAppDisplayName" -ForegroundColor Green
  #  $sp = New-AzADServicePrincipal -DisplayName $testAppDisplayName
  #  $env["principalAppId"] = $sp.AppId
  #  $env["principalAadObjectId"] = $sp.Id
  #  $env["testAppDisplayName"] = $testAppDisplayName

  #  # Create a secondary service principal for tests
  #  $testAppDisplayNameSecondary = "KustoTestPrincipalSecondary-" + $rstr1
  #  Write-Host "Creating secondary test service principal: $testAppDisplayNameSecondary" -ForegroundColor Green
  #  $spSecondary = New-AzADServicePrincipal -DisplayName $testAppDisplayNameSecondary
  #  $env["principalAppIdSecondary"] = $spSecondary.AppId
  #  $env["testAppDisplayNameSecondary"] = $testAppDisplayNameSecondary

  #  # Wait for AAD propagation
  #  Write-Host "Waiting 30 seconds for AAD propagation..." -ForegroundColor Yellow
...
function cleanupEnv() {
    ...
    # # Clean up test service principals
    # if ($env.testAppDisplayName) {
    #     Write-Host "Removing test service principal: $($env.testAppDisplayName)" -ForegroundColor Green
    #     Get-AzADServicePrincipal -DisplayName $env.testAppDisplayName | Remove-AzADServicePrincipal -ErrorAction SilentlyContinue
    #     Get-AzADApplication -DisplayName $env.testAppDisplayName | Remove-AzADApplication -ErrorAction SilentlyContinue
    # }
    # if ($env.testAppDisplayNameSecondary) {
    #     Write-Host "Removing secondary test service principal: $($env.testAppDisplayNameSecondary)" -ForegroundColor Green
    #     Get-AzADServicePrincipal -DisplayName $env.testAppDisplayNameSecondary | Remove-AzADServicePrincipal -ErrorAction SilentlyContinue
    #     Get-AzADApplication -DisplayName $env.testAppDisplayNameSecondary | Remove-AzADApplication -ErrorAction SilentlyContinue
    # }
}
