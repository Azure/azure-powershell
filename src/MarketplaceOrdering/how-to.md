# How-To
This document describes how to develop for `Az.MarketplaceOrdering`.

## Building `Az.MarketplaceOrdering`
...
## Testing `Az.MarketplaceOrdering`
...
## Packing `Az.MarketplaceOrdering`
...
There are multiple scripts created for performing different actions for developing `Az.MarketplaceOrdering`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.MarketplaceOrdering.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.MarketplaceOrdering.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.MarketplaceOrdering`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.MarketplaceOrdering` into the session.
