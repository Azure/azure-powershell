...
function Test-VirtualMachine
{
    ...
    try
    {
        ...
        $job = Start-AzVM -Name $vmname -ResourceGroupName $rgname -AsJob;
        $result = $job | Wait-Job;
        Assert-AreEqual "Completed" $result.State;
        $job = Restart-AzVM -Name $vmname -ResourceGroupName $rgname -AsJob;
        $result = $job | Wait-Job;
        Assert-AreEqual "Completed" $result.State;
        $job = Stop-AzVM -Name $vmname -ResourceGroupName $rgname -Force -StayProvisioned -AsJob;
        $result = $job | Wait-Job;
        Assert-AreEqual "Completed" $result.State;
        ...
        # Remove
        Remove-AzVM -Name $vmname2 -ResourceGroupName $rgname -Force;
    }
    finally
    {
        # Cleanup
...
function Test-VirtualMachinePiping
{
    ...
    try
    {
        ...
        Get-AzVM -ResourceGroupName $rgname | Stop-AzVM -Force -StayProvisioned;
...
function Test-VirtualMachineCapture
{
    ...
    try
    {
        ...
        $dest = Get-ComputeTestResourceName;
        $templatePath = "$TestOutputRoot\template.txt";
        Save-AzVMImage -ResourceGroupName $rgname -VMName $vmname -DestinationContainerName $dest -VHDNamePrefix 'pslib' -Overwrite -Path $templatePath;
        $template = Get-Content $templatePath;
        Assert-True { $template[1].Contains("$schema"); }
...
function Test-VirtualMachineTags
{
    ...
    try
    {
        ...
        $st = New-AzVM -ResourceGroupName $rgname -Location $loc -VM $p -Tags $tags;
        ...
        $st = $vm | Update-AzVM -Tags $new_tags;
        $vm = Get-AzVM -ResourceGroupName $rgname -Name $vmname;
...
function Test-VMImageCmdletOutputFormat
{
    ...
    Assert-OutputContains " Get-AzVMImagePublisher -Location '$locStr' | ? { `$_.PublisherName -eq `'$publisher`' } | Get-AzVMImageOffer | Get-AzVMImageSku " @('Id', 'Location', 'PublisherName', 'Offer', 'Sku');

    Assert-OutputContains " Get-AzVMImagePublisher -Location '$locStr' | ? { `$_.PublisherName -eq `'$publisher`' } | Get-AzVMImageOffer | Get-AzVMImageSku | Get-AzVMImage " @('Version', 'FilterExpression', 'Skus', 'Offer', 'PublisherName');
...
"@;
        ...
        $st = New-AzResourceGroupDeployment -Name "${rgname}dp" -ResourceGroupName $rgname -TemplateFile $templateFile -TemplateParameterFile $paramFile;
...
function Test-VirtualMachineWithPremiumStorageAccount
{
    ...
    try
    {
        ...
        New-AzStorageAccount -ResourceGroupName $rgname_storage -Name $stoname1 -Location $loc -Type $stotype1;
        $stoaccount1 = Get-AzStorageAccount -ResourceGroupName $rgname_storage -Name $stoname1;
...
function Test-VirtualMachineRedeploy
{
    ...
    try
    {
        ...
        # Redeploy the VM
        $job = Set-AzVM -ResourceGroupName $rgname -Name $vmname -Redeploy -AsJob;
        $result = $job | Wait-Job;
        Assert-AreEqual "Completed" $result.State;
