...
#### Using ScriptPath parameter when the script resides on the remote Windows VM
```powershell
...
#### Using ScriptString parameter to execute cmdlet on the Windows VM
```powershell
...
#### Using ScriptString parameter to run script blocks on the Windows VM
```powershell
$ScriptBlock = {
    param(
        [string] $NewTimeZone,
