...
Get-AzVM -Name "VirtualMachine11" | Set-AzVMSqlServerExtension -AutoPatchingSettings $AutoPatchingConfig | Update-AzVM
```
...
The second command gets the virtual machine named VirtualMachine11 by using the Get-AzVM cmdlet.
...
Get-AzVM -Name "VirtualMachine11" | Set-AzVMSqlServerExtension -AutoBackupSettings $AutoBackupConfig | Update-AzVM
```
...
The second command gets the virtual machine named VirtualMachine11, and then passes it to the current cmdlet.
...
```powershell
Get-AzVM -Name "VirtualMachine08" | Set-AzVMSqlServerExtension Disable
```

This command gets a virtual machine named VirtualMachine08, and then passes it to the current cmdlet.
...
```powershell
Get-AzVM -Name "VirtualMachine08" | Set-AzVMSqlServerExtension Uninstall
```
