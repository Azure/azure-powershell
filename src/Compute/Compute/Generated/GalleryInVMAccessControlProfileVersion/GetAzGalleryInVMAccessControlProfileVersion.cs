...
{
    [Cmdlet(VerbsCommon.Get, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "GalleryInVMAccessControlProfileVersion", DefaultParameterSetName = "DefaultParameter")]
    [OutputType(typeof(PSGalleryInVMAccessControlProfileVersion))]
    public partial class GetAzureRmGalleryInVMAccessControlProfileVersion : ComputeAutomationBaseCmdlet
    {
        public override void ExecuteCmdlet()
        {
            ...
            ExecuteClientAction(() =>
            {
                ...
                if (!string.IsNullOrEmpty(galleryInVMAccessControlProfileVersionName) && !WildcardPattern.ContainsWildcardCharacters(galleryInVMAccessControlProfileVersionName))
                {
                    var result = GalleryInVMAccessControlProfileVersionClient.Get(resourceGroupName, galleryName, galleryInVMAccessControlProfileName, galleryInVMAccessControlProfileVersionName);
                ...
                {
                    var result = GalleryInVMAccessControlProfileVersionClient.ListByGalleryInVMAccessControlProfile(resourceGroupName, galleryName, galleryInVMAccessControlProfileName);
                    var resultList = result.ToList();
                    var nextPageLink = result.NextPageLink;
                    while (!string.IsNullOrEmpty(nextPageLink))
                    {
                        var pageResult = GalleryInVMAccessControlProfileVersionClient.ListByGalleryInVMAccessControlProfileNext(nextPageLink);
                    ...
                    var psObject = new List<PSGalleryInVMAccessControlProfileVersionList>();
                    foreach (var r in resultList)
                    {
                        psObject.Add(ComputeAutomationAutoMapperProfile.Mapper.Map<GalleryInVMAccessControlProfileVersion, PSGalleryInVMAccessControlProfileVersionList>(r));
                    }
                    WriteObject(SubResourceWildcardFilter(galleryInVMAccessControlProfileVersionName, psObject), true);
                }
            });
        }
