...
{
    [Cmdlet(VerbsData.Update, ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "ProximityPlacementGroup", DefaultParameterSetName = "DefaultParameter", SupportsShouldProcess = true)]
    [OutputType(typeof(PSProximityPlacementGroup))]
    public partial class UpdateAzureRmProximityPlacementGroup : ComputeAutomationBaseCmdlet
    {
        public override void ExecuteCmdlet()
        {
            ...
            ExecuteClientAction(() =>
            {
                if (ShouldProcess(this.Name, VerbsData.Update))
                {
                    ...
                    IDictionary<string, string> tags = this.MyInvocation.BoundParameters.ContainsKey("Tag") ? this.Tag.Cast<DictionaryEntry>().ToDictionary(ht => (string)ht.Key, ht => (string)ht.Value) : null;

                    var result = ProximityPlacementGroupsClient.Update(resourceGroupName, proximityPlacementGroupName, tags);
