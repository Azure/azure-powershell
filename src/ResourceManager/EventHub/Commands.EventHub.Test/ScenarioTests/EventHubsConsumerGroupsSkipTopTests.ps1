...
{ # Setup
    ...
    while($count -lt 10)
	{
		...
		$result = New-AzureRmEventHub -ResourceGroup $resourceGroupName -Namespace $namespaceName -Name $eventHubName_new -MessageRetentionInDays $msgRetentionInDays -PartitionCount $partionCount
		$count = $count + 1
	}
			
    Write-Debug " Get the created eventHub "
    $listEventHub = Get-AzureRmEventHub -ResourceGroup $resourceGroupName -Namespace $namespaceName
    ...
    $listEventHub = Get-AzureRmEventHub -ResourceGroup $resourceGroupName -Namespace $namespaceName -MaxCount 5
	...
	while($count -lt 10 )
	{
		...
		$result_ConsumerGroup = New-AzureRmEventHubConsumerGroup -ResourceGroup $resourceGroupName -Namespace $namespaceName -EventHub $eventHubName_consumer -Name $consumerGroupName_new
	...
	$ListConsumerGroups = Get-AzureRmEventHubConsumerGroup -ResourceGroup $resourceGroupName -Namespace $namespaceName -EventHub $eventHubName_consumer -MaxCount 5
