...
function Test-ApplicationGatewayCRUD3
{
	...
	try
	{
		...
		$identity = New-AzUserAssignedIdentity -Name $appgwIdentityName -ResourceGroupName $rgname -Location $location

		# Create KeyVault
		$keyVault = New-AzKeyVault -VaultName $keyVaultName -ResourceGroupName $rgname -Location $location -EnableSoftDelete 

		# Upload certificate to Key Vault as a secret.
		$pw01 = ConvertTo-SecureString "P@ssw0rd" -AsPlainText -Force
		$sslCert01Path = $basedir + "/ScenarioTests/Data/ApplicationGatewaySslCert1.pfx"
		$cert = Import-AzKeyVaultCertificate -VaultName $keyVaultName -Name $sslCert01Name -FilePath $sslCert01Path -Password $pw01
		...
		Set-AzKeyVaultAccessPolicy -VaultName $keyVaultName -ResourceGroupName $rgname -PermissionsToSecrets get -ObjectId $identity.PrincipalId
		...
		$secret = Get-AzKeyVaultSecret -VaultName $keyVaultName -Name $sslCert01Name
		$secretId = $secret.Id.Replace($secret.Version, "")
		$sslCert01 = New-AzApplicationGatewaySslCertificate -Name $sslCert01Name -KeyVaultSecretId $secretId
		$listener01 = New-AzApplicationGatewayHttpListener -Name $listener01Name -Protocol Https -SslCertificate $sslCert01 -FrontendIPConfiguration $fipconfig -FrontendPort $fp01
		...
		# Create Application Gateway
		$appgw = New-AzApplicationGateway -Name $appgwName -ResourceGroupName $rgname -Zone 1,2 -Location $location -UserAssignedIdentity $identity -Probes $probeHttp -BackendAddressPools $pool -BackendHttpSettingsCollection $poolSetting01 -FrontendIpConfigurations $fipconfig -GatewayIpConfigurations $gipconfig -FrontendPorts $fp01 -HttpListeners $listener01 -RequestRoutingRules $rule01 -Sku $sku -SslPolicy $sslPolicy -TrustedRootCertificate $trustedRoot01 -AutoscaleConfiguration $autoscaleConfig -sslCertificates $sslCert01
		...
		# Check SSLCertificates
		$listener = Get-AzApplicationGatewayHttpListener -ApplicationGateway $getgw -Name $listener01Name
...
function Test-ApplicationGatewayCRUDSubItems2
{
	...
	try
	{
		...
		$urlPathMap2 = New-AzApplicationGatewayUrlPathMapConfig -Name $urlPathMapName2 -PathRules $videoPathRule -DefaultRedirectConfiguration $redirectConfig
