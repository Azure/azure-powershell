...
namespace Microsoft.Azure.Commands.Relay.Commands.WcfRelay
{
    ...
    public class RemoveAzureWcfRelay : AzureRelayCmdletBase
    {
        ...
        {
            // delete a WcfRelay 
            if(ShouldProcess(target:WcfRelayName, action:string.Format("Deleting WcfRelay: {0} of NnameSpace{1}",WcfRelayName,NamespaceName)))
            {
                WriteObject(Client.DeleteWcfRelay(ResourceGroupName, NamespaceName, WcfRelayName));
            }            
        }
    }
}
