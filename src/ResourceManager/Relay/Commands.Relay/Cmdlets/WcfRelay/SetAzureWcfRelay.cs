...
namespace Microsoft.Azure.Commands.Relay.Commands.WcfRelay
{
    ...
    public class SetAzureWcfRelay : AzureRelayCmdletBase
    {
        ...
        {
            WcfRelayAttributes wcfRelay = null;
            ...
            if(ShouldProcess(target: WcfRelayName, action: string.Format("Updating WcfRelay:{0} of NameSpace:{1}", WcfRelayName,NamespaceName)))
            {
                WriteObject(Client.CreateOrUpdateWcfRelay(ResourceGroupName, NamespaceName, WcfRelayName, WcfRelayObj));
            }
        }
    }
}
