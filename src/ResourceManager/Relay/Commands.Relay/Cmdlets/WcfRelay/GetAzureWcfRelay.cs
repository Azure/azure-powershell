...
namespace Microsoft.Azure.Commands.Relay.Commands.WcfRelay
{
    ...
    public class GetAzureRmRelayWcfRelay : AzureRelayCmdletBase
    {
        ...
        {
            if (!string.IsNullOrEmpty(WcfRelayName))
            {
                // Get a WcfRelay
                WcfRelayAttributes wcfRelay = Client.GetWcfRelay(ResourceGroupName, NamespaceName, WcfRelayName);
                WriteObject(wcfRelay);
            }
            else
            {
                // Get all WcfRelay
                IEnumerable<WcfRelayAttributes> wcfRelayList = Client.ListAllWcfRelay(ResourceGroupName, NamespaceName);
