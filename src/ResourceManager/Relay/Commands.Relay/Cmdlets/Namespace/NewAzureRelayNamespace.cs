...
namespace Microsoft.Azure.Commands.Relay.Commands.Namespace
{
    ...
    public class NewAzureRelayNamespace : AzureRelayCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(target: NamespaceName, action: string.Format("Create a new Relay-Namespace:{0} under Resource Group:{1}", NamespaceName, ResourceGroupName)))
            {
                WriteObject(Client.BeginCreateNamespace(ResourceGroupName, NamespaceName, Location, tagDictionary));
            }
        }
    }
}
