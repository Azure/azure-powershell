...
namespace Microsoft.Azure.Commands.Relay.Commands.HybridConnection
{
    ...
    public class NewAzureRmRelayHybridConnection : AzureRelayCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if(ShouldProcess(target: HybridConnectionsName, action:string.Format("Creating new HybridConnections:{0} under NameSpace:{1} ", HybridConnectionsName, NamespaceName)))
            {
                WriteObject(Client.CreateOrUpdateHybridConnections(ResourceGroupName, NamespaceName, HybridConnectionsName, hybridConnections));
            }
