...
function Test-EndpointCrud
{
	...
	$endpoint = New-AzureRmTrafficManagerEndpoint -Name $endpointName -ProfileName $profileName -ResourceGroupName $resourceGroup.ResourceGroupName  -Type "ExternalEndpoints" -Target "www.contoso.com" -EndpointStatus "Enabled" -EndpointLocation "North Europe" -CustomHeader "header-name-1:header-value-1","header-name-2:header-value-2"
	...
	Assert-AreEqual "header-name-1" $endpoint.CustomHeader[0].Name
	Assert-AreEqual "header-value-1" $endpoint.CustomHeader[0].Value
	Assert-AreEqual "header-name-2" $endpoint.CustomHeader[1].Name
	Assert-AreEqual "header-value-2" $endpoint.CustomHeader[1].Value
