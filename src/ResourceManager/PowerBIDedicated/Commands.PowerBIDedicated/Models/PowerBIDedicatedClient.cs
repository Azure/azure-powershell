...
{
    public class PowerBIDedicatedClient
    {
        private readonly PowerBIDedicatedManagementClient _client;
        private readonly Guid _subscriptionId;
        private readonly string _currentUser;

        public PowerBIDedicatedClient(IAzureContext context)
        {
            if (context == null)
            {
        ...
        public PowerBIDedicatedClient()
        {
        }

        #region Capacity Related Operations

        public DedicatedCapacity CreateOrUpdateCapacity(
            ...
            DedicatedCapacity existingCapacity = null)
         {
            ...
            if (existingCapacity != null)
            {
                ...
                {
                    Sku = skuName == null ? existingCapacity.Sku : GetResourceSkuFromName(skuName),
                    Tags = tags,
                };
        ...
        public bool TestCapacity(string resourceGroupName, string capacityName, out DedicatedCapacity capacity)
        {
            try
            {
        ...
        public DedicatedCapacity GetCapacity(string resourceGroupName, string capacityName)
        {
            if (string.IsNullOrEmpty(resourceGroupName))
            {
                resourceGroupName = GetResourceGroupByCapacity(capacityName);
            ...
            return _client.Capacities.GetDetails(resourceGroupName, capacityName);
        }

        public List<DedicatedCapacity> ListCapacities(string resourceGroupName)
        {
            var capacitiesList = new List<DedicatedCapacity>();
            ...
            capacitiesList.AddRange(response);
