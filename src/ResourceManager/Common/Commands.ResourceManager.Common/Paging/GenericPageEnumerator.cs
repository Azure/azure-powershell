...
{
    public class GenericPageEnumerator<T> : IEnumerator<T>
    {
        ...
        public GenericPageEnumerator(Func<IPage<T>> list, Func<string, IPage<T>> listNext, ulong first, ulong skip)
        {
            _list = list;
            _listNext = listNext;
            _first = first;
