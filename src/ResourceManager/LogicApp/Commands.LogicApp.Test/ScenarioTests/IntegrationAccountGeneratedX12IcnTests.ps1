...
<#
.SYNOPSIS
Test Get-AzureRmIntegrationAccountGeneratedX12Icn command
#>
function Test-GetIntegrationAccountGeneratedX12ControlNumber
{
	...
	Assert-ThrowsContains { Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName "Random83da135" -Name "DoesNotMatter" -AgreementName "DoesNotMatter" } "Operation returned an invalid status code 'NotFound'"
	...
	Assert-ThrowsContains { Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name "Random83da135" -AgreementName "DoesNotMatter" } "Operation returned an invalid status code 'NotFound'"
	...
	Assert-ThrowsContains { Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountX12AgreementName } "Operation returned an invalid status code 'NotFound'"
	...
	Assert-ThrowsContains { Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountX12AgreementName } "is not in a valid format."
	...
	$result =  Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountX12AgreementName
  ...
  Assert-AreEqual "X12" $result.MessageType
	...
	$result1 =  Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName
	Assert-True { $result1.Count -gt 0 }
...
<#
.SYNOPSIS
Test Set-AzureRmIntegrationAccountGeneratedX12Icn command
#>
function Test-UpdateIntegrationAccountGenX12CN
{
	$agreementX12FilePath = "$TestOutputRoot\Resources\IntegrationAccountX12AgreementContent.json"
	$agreementX12Content = [IO.File]::ReadAllText($agreementX12FilePath)
	...
	Assert-ThrowsContains { Set-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountAgreementName -ControlNumber "789" } "Operation returned an invalid status code 'NotFound'"
	...
	$initialControlNumber = Get-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountAgreementName
	...
	$updatedControlNumber = Set-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountAgreementName -ControlNumber $incrementedControlNumberValue
	...
	Assert-ThrowsContains { Set-AzureRmIntegrationAccountGeneratedX12Icn -ResourceGroupName $resourceGroup.ResourceGroupName -Name $integrationAccountName -AgreementName $integrationAccountAgreementName } "Cannot process command because of one or more missing mandatory parameters: ControlNumber."
...
<#
.SYNOPSIS
Test Get-AzureRmIntegrationAccountGeneratedX12Icn command : paging test
#>
function Test-ListIntegrationAccountGenX12CN
{
	$agreementX12FilePath = "$TestOutputRoot\Resources\IntegrationAccountX12AgreementContent.json"
	$agreementX12Content = [IO.File]::ReadAllText($agreementX12FilePath)
