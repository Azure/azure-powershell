...
{
    [Cmdlet(VerbsCommon.Remove, "AzureDataLakeStoreItemAclEntry", DefaultParameterSetName = BaseParameterSetName), OutputType(typeof(bool))]
    public class RemoveAzureDataLakeStoreItemAclEntry : DataLakeStoreFileSystemCmdletBase
    {
        ...
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = BaseParameterSetName, Position = 1, Mandatory = true, HelpMessage = "The path in the specified dataLake account that should have ACL entries removed. Can be a file or folder " +
                                                                                           "In the format 'webhdfs://<accountName>.dataLakeaccountdogfood.net/folder/file.txt', " +
                                                                                           "where the first '/' after the DNS indicates the root of the file system.")]
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = SpecificAceParameterSetName, Position = 1, Mandatory = true, HelpMessage = "The path in the specified dataLake account that should have ACL entries removed. Can be a file or folder " +
                                                                                           "In the format 'webhdfs://<accountName>.dataLakeaccountdogfood.net/folder/file.txt', " +
                                                                                           "where the first '/' after the DNS indicates the root of the file system.")]
        [ValidateNotNull]
