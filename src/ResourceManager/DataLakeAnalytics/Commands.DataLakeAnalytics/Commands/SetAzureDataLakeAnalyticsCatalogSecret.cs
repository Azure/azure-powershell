...
{
    [Cmdlet(VerbsCommon.Set, "AzureDataLakeAnalyticsCatalogSecret"), OutputType(typeof(USqlSecret))]
    public class SetAzureDataLakeAnalyticsCatalogSecret : DataLakeAnalyticsCmdletBase
    {
        ...
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = BaseParameterSetName, Position = 0, Mandatory = true, HelpMessage = "The account name that contains the catalog to update the secret in.")]
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = HostAndPortParameterSetName, Position = 0, Mandatory = true, HelpMessage = "The account name that contains the catalog to update the secret in.")]
        [ValidateNotNullOrEmpty]
        public string AccountName { get; set; }

        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = BaseParameterSetName, Position = 1, Mandatory = true, HelpMessage = "The name of the database to update the secret in.")]
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = HostAndPortParameterSetName, Position = 1, Mandatory = true, HelpMessage = "The name of the database to update the secret in.")]
        [ValidateNotNullOrEmpty]
        public string DatabaseName { get; set; }

        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = BaseParameterSetName, Position = 2, Mandatory = true, HelpMessage = "The secret to update")]
        [Parameter(ValueFromPipelineByPropertyName = true, ParameterSetName = HostAndPortParameterSetName, Position = 2, Mandatory = true, HelpMessage = "The secret to update")]
        [ValidateNotNullOrEmpty]
        ...
        protected override void ProcessRecord()
        {
            ...
            WriteObject(DataLakeAnalyticsClient.UpdateSecret(ResourceGroupName, AccountName, DatabaseName, Secret.UserName, Secret.GetNetworkCredential().Password, toUse.AbsoluteUri));
        }
    }
}