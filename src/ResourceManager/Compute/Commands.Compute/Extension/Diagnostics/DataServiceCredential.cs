...
namespace Microsoft.Azure.Commands.Compute.Extension.Diagnostics
{
    internal class DataServiceCredential
    {
        ...
        public Task<string> OnAuthentication(string authority, string resource, string scope)
        {
            ...
            {
                var b = GetTokenInternal(this.TenantId, this._authenticationFactory, this._context, this._endpointName);
                tokenStr = b.Item1.AccessToken;
            });
        ...
        private static string GetTenantId(AzureContext context)
        {
            ...
            {
                throw new ArgumentException(context.Account.Type.ToString());
            }
