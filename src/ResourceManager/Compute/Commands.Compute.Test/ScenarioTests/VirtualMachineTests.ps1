...
function Test-VirtualMachinePiping
{
    ...
    try
    {
        ...
        Get-AzureVM -ResourceGroupName $rgname | Start-AzureVM;
        Get-AzureVM -ResourceGroupName $rgname | Restart-AzureVM;
        Get-AzureVM -ResourceGroupName $rgname | Stop-AzureVM -Force -StayProvisioned;

        # Update VM
        Get-AzureVM -ResourceGroupName $rgname -Name $vmname `
        | Add-AzureVMDataDisk -Name 'testDataDisk3' -Caching 'ReadOnly' -DiskSizeInGB 12 -Lun 3 -VhdUri $dataDiskVhdUri3 -CreateOption Empty `
        | Update-AzureVM;
        ...
        Get-AzureVM -ResourceGroupName $rgname | Stop-AzureVM -Force;
        Get-AzureVM -ResourceGroupName $rgname | Set-AzureVM -Generalize;

        $dest = Get-ComputeTestResourceName;
        $templatePath = ".\template.txt";
        Get-AzureVM -ResourceGroupName $rgname | Save-AzureVMImage -DestinationContainerName $dest -VHDNamePrefix 'pslib' -Overwrite -Path $templatePath;

        $template = Get-Content $templatePath;
        Assert-True { $template[1].Contains("$schema"); }

        # Remove All VMs
        Get-AzureVM -ResourceGroupName $rgname | Remove-AzureVM -Force;
        $vms = Get-AzureVM -ResourceGroupName $rgname;
        Assert-AreEqual $vms $null;
    }
    finally
