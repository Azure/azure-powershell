...
function Test-VirtualMachineScaleSetForceUDWalk
{
    ...
    try
    {
        ...
        $result = Invoke-AzureRmVmssServiceFabricUDWalk -ResourceGroupName $rgname -VMScaleSetName $vmssName -PlatformUpdateDomain 0;
        Assert-True {$result.WalkPerformed};
        Assert-AreEqual 1 $result.NextPlatformUpdateDomain;

        $result = $vmss | Invoke-AzureRmVmssServiceFabricUDWalk -PlatformUpdateDomain 1;
        Assert-True {$result.WalkPerformed};
        Assert-AreEqual 2 $result.NextPlatformUpdateDomain;

        $result = Invoke-AzureRmVmssServiceFabricUDWalk -ResourceId $vmss.Id -PlatformUpdateDomain 2;
        Assert-True {$result.WalkPerformed};
    }
    finally
    {
