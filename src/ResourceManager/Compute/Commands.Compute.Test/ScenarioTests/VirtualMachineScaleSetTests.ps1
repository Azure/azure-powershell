...
function Test-VirtualMachineScaleSet
{
    ...
    try
    {
        ...
        $actualLb = New-AzureRmLoadBalancer -Name $lbName -ResourceGroupName $rgname -Location $loc -FrontendIpConfiguration $frontend -BackendAddressPool $backendAddressPool -Probe $probe -LoadBalancingRule $lbrule -InboundNatPool $inboundNatPool
		...
		#$frontendipconfig = New-Object Microsoft.Azure.Management.Compute.Models.IPConfigurationLoadBalancerInboundNATPool;
		#Write-Verbose ("Inbound: " + $expectedLb.InboundNatPools[0].Id);
		#$frontendipconfig.ReferenceUri = $expectedLb.InboundNatPools[0].Id;
		#$ipCfg.LoadBalancerInboundNatPools = New-Object System.Collections.Generic.List``1[Microsoft.Azure.Management.Compute.Models.IPConfigurationLoadBalancerInboundNATPool];		
		#$ipCfg.LoadBalancerInboundNatPools.Add($frontendipconfig);

		#$backendipconfig = New-Object Microsoft.Azure.Management.Compute.Models.IPConfigurationLoadBalancerAddressPool;
		#Write-Verbose ("Back: " + $expectedLb.BackendAddressPools[0].Id);
		#$backendipconfig.ReferenceUri = $expectedLb.BackendAddressPools[0].Id;
		#$ipCfg.LoadBalancerBackendAddressPools = New-Object System.Collections.Generic.List``1[Microsoft.Azure.Management.Compute.Models.IPConfigurationLoadBalancerAddressPool];
		#$ipCfg.LoadBalancerBackendAddressPools.Add($backendipconfig);		


        $ipCfg = New-AzureRmVmssIPConfigurationConfig -Name 'test' -LoadBalancerInboundNatPoolsId $expectedLb.InboundNatPools[0].Id -LoadBalancerBackendAddressPoolsId $expectedLb.BackendAddressPools[0].Id -SubnetId $subnetId;
