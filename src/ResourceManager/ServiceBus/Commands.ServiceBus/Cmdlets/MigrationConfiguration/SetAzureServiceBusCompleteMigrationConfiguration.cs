...
namespace Microsoft.Azure.Commands.ServiceBus.Commands.Migration
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, ServicebusCompleteMigrationConfiguration, DefaultParameterSetName = MigrationConfigurationParameterSet, SupportsShouldProcess = true), OutputType(typeof(bool))]
    public class SetAzureServiceBusCompleteMigrationConfiguration : AzureServiceBusCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            {
                if (ShouldProcess(target: Namespace, action: string.Format(Resources.CompleteMigrationConfiguration)))
                {
                    Client.SetServiceBusCompleteMigrationConfiguration(ResourceGroupName, Namespace);
                    if (PassThru)
                    {
                        WriteObject(true);
                    }
