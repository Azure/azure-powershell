...
namespace Microsoft.Azure.Commands.ServiceBus.Commands.Migration
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, ServicebusStartMigrationConfiguration, DefaultParameterSetName = MigrationConfigurationParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSServiceBusDRConfigurationAttributes))]
    public class NewAzureServiceBusMigrationConfiguration : AzureServiceBusCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            {
                if (ShouldProcess(target: Namespace, action: string.Format(Resources.StartMigrationConfiguration)))
                {
                    WriteObject(Client.StartServiceBusMigrationConfiguration(ResourceGroupName, Namespace, migrationConfiguration));
                }
            }
        }
    }
