...
namespace Microsoft.Azure.Commands.Automation.Cmdlet
{
    ...
    public class NewAzureAutomationRunbook : AzureAutomationBaseCmdlet
    {
        ...
        /// </summary>
        [Parameter(ParameterSetName = AutomationCmdletParameterSets.ByPath, Position = 2, Mandatory = true, ValueFromPipelineByPropertyName = true, HelpMessage = "The runbook file path.")]
        [Alias("RunbookPath")]
        [ValidateNotNullOrEmpty]
        ...
        protected override void AutomationExecuteCmdlet()
        {
            ...
            if (this.ParameterSetName == AutomationCmdletParameterSets.ByPath)
            {
                ...
                runbook = this.AutomationClient.CreateRunbookByPath(
                    this.ResourceGroupName, this.AutomationAccountName, this.ResolvePath(this.Path), this.Description, this.Tags);
            }
            else if (this.ParameterSetName == AutomationCmdletParameterSets.ByRunbookName)
            {
                ...
                runbook = this.AutomationClient.CreateRunbookByName(
                    this.ResourceGroupName, this.AutomationAccountName, this.Name, this.Description, this.Tags);
            }
