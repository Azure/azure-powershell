...
namespace Microsoft.Azure.Commands.RecoveryServices
{
    ...
    public class RpNamespaceHandler : DelegatingHandler, ICloneable
    {
            ...
            HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                var productionRpNamespace = PSRecoveryServicesClient.ProductionRpNamespace;
                request.RequestUri = new Uri(request.RequestUri.AbsoluteUri.Replace(
                    productionRpNamespace, rpNamespace));

                Logger.Instance.WriteDebug(string.Format(
                    "RpNamespaceHandler productionRpNamespace = {0}, rpNamespace = {1}",
