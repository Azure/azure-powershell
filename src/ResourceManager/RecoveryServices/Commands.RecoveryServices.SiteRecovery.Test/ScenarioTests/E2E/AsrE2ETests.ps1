...
function Test-CreatePCMap
{
    ...
    $RecoveryProtectionContainer = Get-AzureRmRecoveryServicesAsrFabric -FriendlyName $recoveryFabricName| Get-AzureRmRecoveryServicesAsrProtectionContainer | where { $_.FriendlyName -eq $RecoveryProtectionContainerName }
...
function Test-TFO
{
    ...
    $rpi = Get-AzureRmRecoveryServicesAsrReplicationProtectedItem -Name $rpi.Name -ProtectionContainer $PrimaryProtectionContainer 
    $rpi = Get-AzureRmRecoveryServicesAsrReplicationProtectedItem -ProtectableItem $VM  
