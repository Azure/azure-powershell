...
function Test-V2AAddPI
{
    ...
    $job = New-AzureRmRecoveryServicesAsrProtectableItem -IPAddress $vmIp -FriendlyName $piName -OSType Windows -ProtectionContainer $pc
    waitForJobCompletion -JobId $job.name
}
...
function Test-PCM 
{
    ...
    $Job1 = New-AzureRmRecoveryServicesAsrPolicy -Name $policyName1 -VmwaretoAzure -RecoveryPointRetentionInHours 40  -RPOWarningThresholdInMinutes 5 -ApplicationConsistentSnapshotFrequencyInHours 15
    $Job2 = New-AzureRmRecoveryServicesAsrPolicy -Name $policyName2 -AzureToVmware -RecoveryPointRetentionInHours 40  -RPOWarningThresholdInMinutes 5 -ApplicationConsistentSnapshotFrequencyInHours 15
...
function V2ACreateRPI 
{
    ...
    $fabric =  Get-AsrFabric -FriendlyName $PrimaryFabricName
    $pc =  Get-ASRProtectionContainer -FriendlyName $pcName -Fabric $fabric
    $Job1 = New-AzureRmRecoveryServicesAsrPolicy -VmwareToAzure -Name $policyName1  -RecoveryPointRetentionInHours 40  -RPOWarningThresholdInMinutes 5 -ApplicationConsistentSnapshotFrequencyInHours 15 -MultiVmSyncStatus "Enable"
    $Job2 = New-AzureRmRecoveryServicesAsrPolicy -AzureToVmware -Name $policyName2  -RecoveryPointRetentionInHours 40  -RPOWarningThresholdInMinutes 5 -ApplicationConsistentSnapshotFrequencyInHours 15 -MultiVmSyncStatus "Enable"
    WaitForJobCompletion -JobId $Job1.Name
    WaitForJobCompletion -JobId $Job2.Name
    $Policy1 = Get-AzureRmRecoveryServicesAsrPolicy -Name $PolicyName1
    $Policy2 = Get-AzureRmRecoveryServicesAsrPolicy -Name $PolicyName2
...
function Test-RPJobReverse
{
    ...
    $job = Update-AzureRmRecoveryServicesAsrProtectionDirection -AzureToVMware`
...
function V2ATestResync 
{
    ...
    $pc =  Get-ASRProtectionContainer -FriendlyName $pcName -Fabric $fabric
    $rpi = get-AzureRmRecoveryServicesAsrReplicationProtectedItem -ProtectionContainer $pc -Name $rpiName
    $job = Start-AzureRmRecoveryServicesAsrResynchronizeReplicationJob -ReplicationProtectedItem $rpi
    WaitForJobCompletion -JobId $Job.Name
}
...
function V2AUpdateMobilityService
{
    ...
    $pc =  Get-ASRProtectionContainer -FriendlyName $pcName -Fabric $fabric
    $rpi = get-AzureRmRecoveryServicesAsrReplicationProtectedItem -ProtectionContainer $pc -Name $rpiName
    $job = Update-AzureRmRecoveryServicesAsrMobilityService -ReplicationProtectedItem $rpi -Account $fabric.fabricSpecificDetails.RunAsAccounts[0]
    WaitForJobCompletion -JobId $Job.Name
}
...
function V2ASwitchProcessServer 
{
    ...
    $fabric =  Get-AsrFabric -FriendlyName $PrimaryFabricName
    $pc =  Get-ASRProtectionContainer -FriendlyName $pcName -Fabric $fabric
    $RPIList = Get-AzureRmRecoveryServicesAsrReplicationProtectedItem   -ProtectionContainer $pc
    $job = Start-AzureRmRecoveryServicesAsrSwitchProcessServerJob -Fabric $fabric -SourceProcessServer $fabric.FabricSpecificDetails.ProcessServers[0] -TargetProcessServer $fabric.FabricSpecificDetails.ProcessServers[1] -ReplicatedItem $RPIList
    WaitForJobCompletion -JobId $Job.Name
    $job = Start-AzureRmRecoveryServicesAsrSwitchProcessServerJob -Fabric $fabric -SourceProcessServer $fabric.FabricSpecificDetails.ProcessServers[0] -TargetProcessServer $fabric.FabricSpecificDetails.ProcessServers[1]
    WaitForJobCompletion -JobId $Job.Name
}
...
function V2ATestFailoverJob 
{
    ...
    function V2AFailoverJob 
    {
        ...
        $rpiName = "win-4003"
        $rpi = get-AzureRmRecoveryServicesAsrReplicationProtectedItem -ProtectionContainer $pc -FriendlyName $rpiName
...
function v2aUpdatePolicy
{
    param([string] $vaultSettingsFilePath)
        ...
        Update-AzureRmRecoveryServicesAsrPolicy  -VMwareToAzure -ApplicationConsistentSnapshotFrequencyInHours 5 -InputObject $po -MultiVmSyncStatus "Enable"
        Update-AzureRmRecoveryServicesAsrPolicy  -VMwareToAzure -ApplicationConsistentSnapshotFrequencyInHours 5 -InputObject $po
}
