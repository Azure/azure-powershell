...
online version: https://docs.microsoft.com/en-us/powershell/module/azurerm.sql/set-azurermsqlserverextendedauditing
schema: 2.0.0
---
...
## SYNOPSIS
Changes the extended auditing settings of an Azure SQL server.

## SYNTAX

### DefaultParameterSet (Default)
```
Set-AzureRmSqlServerExtendedAuditing -State <String> [-AuditActionGroup <AuditActionGroups[]>] [-PassThru]
...
## DESCRIPTION
The **Set-AzureRmSqlServerExtendedAuditing** cmdlet changes the extended auditing settings of an Azure SQL server.
...
Use the *State* parameter to enable/disable the extended policy.
...
Use the *PredicateExpression* parameter to specify the statement of the Where Clause used to filter audit logs.
After the cmdlet runs successfully, extended auditing of the Azure SQL databases that are defined in the specified Azure SQL server is enabled.
If the cmdlet succeeds and you use the *PassThru* parameter, it returns an object describing the current extended blob auditing policy in addition to the server identifiers.
Server identifiers include, but are not limited to, **ResourceGroupName** and **ServerName**.

## EXAMPLES

### Example 1: Enable the extended auditing policy of an Azure SQL server
```
PS C:\>Set-AzureRmSqlServerExtendedAuditing -State Enabled -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -StorageAccountName "Storage22" -PredicateExpression "object_name = 'SensitiveData'"
```

### Example 2: Disable the extended auditing policy of an Azure SQL server
```
PS C:\>Set-AzureRmSqlServerExtendedAuditing -State Disabled -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
```

### Example 3: Enable the extended auditing policy of an Azure SQL server using a storage account from a different subscription
```
PS C:\>Set-AzureRmSqlServerExtendedAuditing -State Enabled -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -StorageAccountName "Storage22" -StorageAccountSubscriptionId "7fe3301d-31d3-4668-af5e-211a890ba6e3" -PredicateExpression "object_name = 'SensitiveData'"
```
