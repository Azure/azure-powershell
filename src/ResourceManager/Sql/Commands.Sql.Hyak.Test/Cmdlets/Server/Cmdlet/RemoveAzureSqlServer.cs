    ...
    public class RemoveAzureSqlServer : AzureSqlServerCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            if (!Force.IsPresent && !ShouldProcess(
               string.Format(CultureInfo.InvariantCulture, "RemoveAzureSqlServerDescription: {0}", this.ServerName),
               string.Format(CultureInfo.InvariantCulture, "RemoveAzureSqlServerWarning: {0}", this.ServerName),
               "ShouldProcessCaption"))
            {
                return;
            }
