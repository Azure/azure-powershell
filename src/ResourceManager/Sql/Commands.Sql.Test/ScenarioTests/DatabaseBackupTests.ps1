...
function Test-ShortTermRetentionPolicy
{
	...
	try
	{
		...
		$retention = 28
		$policy = Set-AzSqlDatabaseBackupShortTermRetentionPolicy -ResourceGroupName $rg.ResourceGroupName -ServerName $server.ServerName -DatabaseName $databaseName -RetentionDays $retention
		Assert-AreEqual $policy.Count 1
		Assert-AreEqual $retention $policy[0].RetentionDays
		$policy = Get-AzSqlDatabaseBackupShortTermRetentionPolicy -ResourceGroupName $rg.ResourceGroupName -ServerName $server.ServerName -DatabaseName $databaseName
		Assert-AreEqual $policy.Count 1
		Assert-AreEqual $retention $policy[0].RetentionDays

		# Test InputObject
		$retention = 21
		$policy = Set-AzSqlDatabaseBackupShortTermRetentionPolicy -AzureSqlDatabase $db -RetentionDays $retention
		...
		$policy = Set-AzSqlDatabaseBackupShortTermRetentionPolicy -ResourceId $resourceId -RetentionDays $retention
		Assert-AreEqual 1 $policy.Count
		Assert-AreEqual $retention $policy[0].RetentionDays
		...
		# Test Piping
		$retention = 7
		$policy = $db | Set-AzSqlDatabaseBackupShortTermRetentionPolicy -RetentionDays $retention
		Assert-AreEqual 1 $policy.Count
		Assert-AreEqual $retention $policy[0].RetentionDays
		...
		try {
			$db | Set-AzSqlDatabaseBackupShortTermRetentionPolicy -RetentionDays $invalidRetention
		}
