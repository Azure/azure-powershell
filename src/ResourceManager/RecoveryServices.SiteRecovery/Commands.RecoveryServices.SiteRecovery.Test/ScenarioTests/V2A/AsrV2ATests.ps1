...
function V2ACreateRPI 
{
    ...
    $fabric =  Get-AsrFabric -FriendlyName $PrimaryFabricName
    $pc =  Get-ASRProtectionContainer -FriendlyName $pcName -Fabric $fabric
    $Job1 = New-AzureRmRecoveryServicesAsrPolicy -VmwareToAzure -Name $policyName1  -RecoveryPointRetentionInHours 40  -RPOWarningThresholdInMinutes 5 -ApplicationConsistentSnapshotFrequencyInHours 15 -EnableMultiVmSync $true
    $Job2 = New-AzureRmRecoveryServicesAsrPolicy -AzureToVmware -Name $policyName2  -RecoveryPointRetentionInHours 40  -RPOWarningThresholdInMinutes 5 -ApplicationConsistentSnapshotFrequencyInHours 15 -EnableMultiVmSync $true
    WaitForJobCompletion -JobId $Job1.Name
    WaitForJobCompletion -JobId $Job2.Name
    $Policy1 = Get-AzureRmRecoveryServicesAsrPolicy -Name $PolicyName1
    $Policy2 = Get-AzureRmRecoveryServicesAsrPolicy -Name $PolicyName2
...
function V2ASwitchProcessServer 
{
    ...
    $job = Start-AzureRmRecoveryServicesAsrSwitchProcessServer -Fabric $fabric -SourceProcessServer $fabric.FabricSpecificDetails.ProcessServers[0] -TargetProcessServer $fabric.FabricSpecificDetails.ProcessServers[1] -ReplicatedItem $RPIList
    WaitForJobCompletion -JobId $Job.Name
    $job = Start-AzureRmRecoveryServicesAsrSwitchProcessServer -Fabric $fabric -SourceProcessServer $fabric.FabricSpecificDetails.ProcessServers[0] -TargetProcessServer $fabric.FabricSpecificDetails.ProcessServers[1]
    WaitForJobCompletion -JobId $Job.Name
}
