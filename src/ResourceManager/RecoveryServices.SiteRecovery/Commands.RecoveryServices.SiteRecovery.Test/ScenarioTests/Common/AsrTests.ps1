...
function Test-VerifyAuth
{
    ...
    $fabric = Get-asrfabric -friendlyName "Madhavi-Cs-Roll"
    ...
    $path = Get-AzureRmRecoveryServicesVaultSettingsFile -Vault $Vault
    Import-AzureRmRecoveryServicesAsrVaultSettingsFile -Path $path.FilePath
    $fabric = Get-asrfabric -Name "mdtestsite1"
    Assert-NotNull($fabric)
...
function Test-AsrEvent
{
    ...
    $e = Get-AzureRmRecoveryServicesAsrEvent -EventType VmHealth -AffectedObjectFriendlyName 'win-4003'
    Assert-NotNull($e)
}
...
function Test-NotificationSettings
{
    ...
    Import-AzureRmRecoveryServicesAsrVaultSettingsFile -Path $vaultSettingsFilePath
    # Enumerate specific Fabric
    $NotificationSettings = Set-AzureRmRecoveryServicesAsrNotificationSetting -EmailSubscriptionOwner $true -CustomEmailAddress "abcxxxx@microsft.com"
