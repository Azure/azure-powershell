...
<#
.SYNOPSIS
Test New-AzureRmContainerRegistry, Get-AzureRmContainerRegistry, Set-AzureRmContainerRegistry, and Remove-AzureRmContainerRegistry.
#>
function Test-AzureContainerRegistry
{
    ...
    $registry = New-AzureRmContainerRegistry -ResourceGroupName $resourceGroupName -Name $registryName -Location $location -Sku $sku
    ...
    $nameStatus = Get-AzureRmContainerRegistryNameAvailability -Name $registryName
    ...
    Remove-AzureRmContainerRegistry -ResourceGroupName $resourceGroupName -Name $registryName -Force
    $registryName = Get-RandomRegistryName

    # Creating a container registry with an existing storage account
    $registry = New-AzureRmContainerRegistry -ResourceGroupName $resourceGroupName -Name $registryName -Location $location -Sku $sku -StorageAccountName $storageAccountName
    Assert-AreEqual $registry.ResourceGroupName $resourceGroupName
    Assert-AreEqual $registry.Name $registryName
    Assert-AreEqual $registry.Type "Microsoft.ContainerRegistry/registries"
    Assert-AreEqual $registry.Location $location
    ...
    $registry = Set-AzureRmContainerRegistry -ResourceGroupName $resourceGroupName -Name $registryName -AdminUserEnabled $true -StorageAccountName $registry.StorageAccountName
    Assert-AreEqual $registry.ResourceGroupName $resourceGroupName
    Assert-AreEqual $registry.Name $registryName
    Assert-AreEqual $registry.Type "Microsoft.ContainerRegistry/registries"
    Assert-AreEqual $registry.Location $location
...
<#
.SYNOPSIS
Test Get-AzureRmContainerRegistryCredential and New-AzureRmContainerRegistryCredential.
#>
function Test-AzureContainerRegistryCredential
{
    ...
    # Creating a container registry with a default new storage account
    $registry = New-AzureRmContainerRegistry -ResourceGroupName $resourceGroupName -Name $registryName -Location $location -Sku $sku -AdminUserEnabled $true
    Assert-AreEqual $registry.ResourceGroupName $resourceGroupName
    Assert-AreEqual $registry.Name $registryName
    Assert-AreEqual $registry.Type "Microsoft.ContainerRegistry/registries"
    Assert-AreEqual $registry.Location $location
    ...
    $newCredential1 = New-AzureRmContainerRegistryCredential -ResourceGroupName $resourceGroupName -Name $registryName -PasswordName password
    ...
    $newCredential2 = New-AzureRmContainerRegistryCredential -ResourceGroupName $resourceGroupName -Name $registryName -PasswordName password2
