...
namespace Microsoft.Azure.Commands.ContainerRegistry
{
    public class ContainerRegistryClient
    {
        ...
        public RegistryListCredentialsResult RegenerateRegistryCredential(string resourceGroupName, string registryName, string passwordName)
        {
            return _client.Registries.RegenerateCredential(resourceGroupName, registryName, ParsePasswordName(passwordName));
        }

        private PasswordName ParsePasswordName(string passwordName)
        {
            PasswordName result;
            if (!Enum.TryParse(passwordName, true, out result))
            {
                throw new ArgumentOutOfRangeException($"Invalid PasswordName {passwordName}.");
            }
            return result;
        }
