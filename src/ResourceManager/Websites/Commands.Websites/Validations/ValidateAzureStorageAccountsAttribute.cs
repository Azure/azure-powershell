    ...
    public class ValidateAzureStorageAccountsAttribute : ValidateArgumentsAttribute
    {
        protected override void Validate(object arguments, EngineIntrinsics engineIntrinsics)
        {
            ...
            foreach (var key in hashtable.Keys)
            {
                ...
                {
                    throw new ValidationMetadataException("Azure Storage account type value pair must be of type 'System.Collections.Hashtable'");
                }
                ...
                {
                    throw new ValidationMetadataException("Azure Storage account type must be specified.");
                }
                AzureStorageType type;
                if (!Enum.TryParse(typeValuePairIgnoreCase["Type"].ToString(), true, out type))
                {
                    throw new ValidationMetadataException("Azure Storage account type values are [AzureFiles| AzureBlob]");
                }
                if (!typeValuePairIgnoreCase.ContainsKey("AccountName") || typeValuePairIgnoreCase["AccountName"].GetType() != typeof(string))
                {
                    throw new ValidationMetadataException("AccountName value must be specified.");
                }
                if (!typeValuePairIgnoreCase.ContainsKey("ShareName") || typeValuePairIgnoreCase["ShareName"].GetType() != typeof(string))
                {
                    throw new ValidationMetadataException("ShareName  value must be specified.");
                }
