# How-To
This document describes how to develop for `Az.ConnectedKubernetes`.

## Building `Az.ConnectedKubernetes`
...
## Testing `Az.ConnectedKubernetes`
...
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.ConnectedKubernetes.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.ConnectedKubernetes.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.ConnectedKubernetes`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.ConnectedKubernetes` into the session.
