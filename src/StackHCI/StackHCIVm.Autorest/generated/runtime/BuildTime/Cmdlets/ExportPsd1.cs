...
using static Microsoft.Azure.PowerShell.Cmdlets.StackHCIVm.Runtime.PowerShell.PsHelpers;

namespace Microsoft.Azure.PowerShell.Cmdlets.StackHCIVm.Runtime.PowerShell
{
  ...
  public class ExportPsd1 : PSCmdlet
  {
    ...
    protected override void ProcessRecord()
    {
      try
      {
        ...
        string version = Convert.ToString(@"1.1.0");
        ...
        sb.AppendLine($@"{Indent}RootModule = '{"./Az.StackHCIVm.psm1"}'");
        ...
        sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: StackHciVM cmdlets"}'");
        sb.AppendLine($@"{Indent}PowerShellVersion = '5.1'");
        sb.AppendLine($@"{Indent}DotNetFrameworkVersion = '4.7.2'");
        sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.StackHCIVm.private.dll"}'");
        ...
        var formatList = customFormatPs1xmlFiles.Prepend("./Az.StackHCIVm.format.ps1xml").ToPsList();
        ...
        sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule StackHciVM".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
