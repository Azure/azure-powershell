# How-To
This document describes how to develop for `Az.DynatraceObservability`.

## Building `Az.DynatraceObservability`
...
## Testing `Az.DynatraceObservability`
...
## Packing `Az.DynatraceObservability`
...
There are multiple scripts created for performing different actions for developing `Az.DynatraceObservability`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DynatraceObservability.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DynatraceObservability.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DynatraceObservability`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DynatraceObservability` into the session.
