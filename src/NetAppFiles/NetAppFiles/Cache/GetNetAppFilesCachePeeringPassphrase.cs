...
namespace Microsoft.Azure.Commands.NetAppFiles.Cache
{
    [Cmdlet(
        "Get",
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesCachePeeringPassphrase",
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(PSNetAppFilesCachePeeringPassphrase))]
    [Alias("Get-AnfCachePeeringPassphrase")]
    public class GetAzureRmNetAppFilesCachePeeringPassphrase : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The cache object")]
        [ValidateNotNullOrEmpty]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            try
            {
                var passphrases = AzureNetAppFilesManagementClient.Caches.ListPeeringPassphrases(ResourceGroupName, AccountName, PoolName, Name);
                WriteObject(passphrases.ConvertToPs());
            }
