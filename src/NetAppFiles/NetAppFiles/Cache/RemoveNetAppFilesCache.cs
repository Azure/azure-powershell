...
namespace Microsoft.Azure.Commands.NetAppFiles.Cache
{
    ...
    [Alias("Remove-AnfCache")]
    public class RemoveAzureRmNetAppFilesCache : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The cache object to remove")]
        [ValidateNotNullOrEmpty]
        public PSNetAppFilesCache InputObject { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "Return whether the specified cache was successfully removed")]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.RemoveResourceMessage, Name)))
            {
                try
                {
                    AzureNetAppFilesManagementClient.Caches.Delete(ResourceGroupName, AccountName, PoolName, Name);
                    success = true;
                }
