...
{
    /// <summary>
    /// Moves a cache to another capacity pool.
    /// </summary>
    [Cmdlet(
        VerbsCommon.Set,
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesCachePool",
        SupportsShouldProcess = true,
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(PSNetAppFilesCache))]
    [Alias("Set-AnfCachePool")]
    public class SetAzureRmNetAppFilesCachePool : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(Mandatory = true, ParameterSetName = FieldsParameterSet, HelpMessage = "The name of the current ANF capacity pool")]
        [ValidateNotNullOrEmpty]
        ...
        [Parameter(Mandatory = true, HelpMessage = "Resource ID of the destination capacity pool")]
        [ValidateNotNullOrEmpty]
        ...
        [Parameter(ParameterSetName = ObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The cache object to move")]
        [ValidateNotNullOrEmpty]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.PoolChangeMessage, Name)))
            {
                try
                {
                    var poolChangeBody = new PoolChangeRequest(NewPoolResourceId);
                    var anfCache = AzureNetAppFilesManagementClient.Caches.PoolChange(ResourceGroupName, AccountName, PoolName, Name, poolChangeBody);
