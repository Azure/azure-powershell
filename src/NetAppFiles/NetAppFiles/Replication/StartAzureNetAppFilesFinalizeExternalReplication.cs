...
namespace Microsoft.Azure.Commands.NetAppFiles.Replication
{
    [Cmdlet(
        "Start",
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesFinalizeExternalReplication",
        SupportsShouldProcess = true,
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(bool))]
    [Alias("Start-AnfFinalizeExternalReplication")]
    public class StartAzureRmNetAppFilesFinalizeExternalReplication : AzureNetAppFilesCmdletBase
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.ApproveVolumeReplicationMessage, Name)))
            {
                try
                {
                    AzureNetAppFilesManagementClient.Volumes.FinalizeExternalReplication(ResourceGroupName, AccountName, PoolName, Name);                    
                    success = true;
                }
