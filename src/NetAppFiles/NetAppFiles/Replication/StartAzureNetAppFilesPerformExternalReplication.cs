...
namespace Microsoft.Azure.Commands.NetAppFiles.Replication
{
    [Cmdlet(
        "Start",
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesPerformExternalReplication",
        SupportsShouldProcess = true,
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(bool))]
    [Alias("Start-AnfPerformExternalReplication")]
    public class StartAzureRmNetAppFilesPerformExternalReplication : AzureNetAppFilesCmdletBase
    {
        [Parameter(
            Mandatory = true,
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.ApproveVolumeReplicationMessage, Name)))
            {
                try
                {
                    AzureNetAppFilesManagementClient.Volumes.PerformReplicationTransfer(ResourceGroupName, AccountName, PoolName, Name);                    
                    success = true;
                }
