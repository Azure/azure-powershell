...
namespace Microsoft.Azure.Commands.NetAppFiles.Bucket
{
    [Cmdlet(
        VerbsCommon.New,
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesBucketCredential",
        SupportsShouldProcess = true,
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(PSNetAppFilesBucketCredential))]
    [Alias("New-AnfBucketCredential")]
    public class NewAzureRmNetAppFilesBucketCredential : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The bucket object for which credentials should be generated")]
        [ValidateNotNullOrEmpty]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.CreateResourceMessage, Name)))
            {
                try
                {
                    var credentials = AzureNetAppFilesManagementClient.Buckets.GenerateCredentials(ResourceGroupName, AccountName, PoolName, VolumeName, Name, body);
                    WriteObject(credentials.ConvertToPs());
                }
