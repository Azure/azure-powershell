...
namespace Microsoft.Azure.Commands.NetAppFiles.Bucket
{
    ...
    [Alias("New-AnfBucket")]
    public class NewAzureRmNetAppFilesBucket : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(Mandatory = true, HelpMessage = "The name of the ANF bucket")]
        [ValidateNotNullOrEmpty]
        [Alias("BucketName")]
        public string Name { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "The volume path mounted inside the bucket. Defaults to '/'.")]
        [ValidateNotNullOrEmpty]
        public string Path { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "Access permissions for the bucket. Either ReadOnly or ReadWrite. Defaults to ReadOnly.")]
        ...
        [Parameter(Mandatory = false, HelpMessage = "Host part of the bucket URL, resolving to the bucket IP and allowed by the server certificate.")]
        [ValidateNotNullOrEmpty]
        public string ServerFqdn { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "Base64-encoded contents of the PEM file containing the bucket server certificate and private key. Mutually exclusive with the AKV certificate parameters.")]
        [ValidateNotNullOrEmpty]
        ...
        [Parameter(ParameterSetName = ParentObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The volume object containing the new bucket")]
        [ValidateNotNullOrEmpty]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            var bucketBody = new Management.NetApp.Models.Bucket
            {
                Path = Path,
                Permissions = Permissions,
                FileSystemUser = BuildFileSystemUser(),
                Server = BuildServerProperties(),
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.CreateResourceMessage, Name)))
            {
                try
                {
                    var anfBucket = AzureNetAppFilesManagementClient.Buckets.CreateOrUpdate(ResourceGroupName, AccountName, PoolName, VolumeName, Name, bucketBody);
        ...
        private BucketServerProperties BuildServerProperties()
        {
