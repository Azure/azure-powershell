...
namespace Microsoft.Azure.Commands.NetAppFiles.Bucket
{
    ...
    [Alias("Remove-AnfBucket")]
    public class RemoveAzureRmNetAppFilesBucket : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The bucket object to remove")]
        [ValidateNotNullOrEmpty]
        public PSNetAppFilesBucket InputObject { get; set; }

        [Parameter(Mandatory = false, HelpMessage = "Return whether the specified bucket was successfully removed")]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.RemoveResourceMessage, Name)))
            {
                try
                {
                    AzureNetAppFilesManagementClient.Buckets.Delete(ResourceGroupName, AccountName, PoolName, VolumeName, Name);
                    success = true;
                }
