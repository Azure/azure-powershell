...
namespace Microsoft.Azure.Commands.NetAppFiles.Bucket
{
    [Cmdlet(
        VerbsData.Update,
        ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "NetAppFilesBucketCertificate",
        SupportsShouldProcess = true,
        DefaultParameterSetName = FieldsParameterSet), OutputType(typeof(bool))]
    [Alias("Update-AnfBucketCertificate")]
    public class UpdateAzureRmNetAppFilesBucketCertificate : AzureNetAppFilesCmdletBase
    {
        ...
        [Parameter(ParameterSetName = ObjectParameterSet, Mandatory = true, ValueFromPipeline = true, HelpMessage = "The bucket object whose certificate should be refreshed from Azure Key Vault")]
        [ValidateNotNullOrEmpty]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            if (ShouldProcess(Name, string.Format(PowerShell.Cmdlets.NetAppFiles.Properties.Resources.UpdateResourceMessage, Name)))
            {
                try
                {
                    AzureNetAppFilesManagementClient.Buckets.RefreshCertificate(ResourceGroupName, AccountName, PoolName, VolumeName, Name);
                    success = true;
                }
