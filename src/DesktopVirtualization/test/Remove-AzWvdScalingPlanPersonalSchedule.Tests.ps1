if(($null -eq $TestName) -or ($TestName -contains 'Remove-AzWvdScalingPlanPersonalSchedule'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Remove-AzWvdScalingPlanPersonalSchedule.Recording.json'
...
Describe 'Remove-AzWvdScalingPlanPersonalSchedule' {
    ...
    It 'Delete' {
        try {
            ...
            $scalingPlanPersonalSchedule = Remove-AzWvdScalingPlanPersonalSchedule `
                -SubscriptionId $env.SubscriptionId `
                -ResourceGroupName $env.ResourceGroup `
                -ScalingPlanName $scalingPlanName `
                -ScalingPlanScheduleName 'PersonalSchedule1'
            ...
            $scalingPlanPersonalSchedule.Count | Should -Be 0
