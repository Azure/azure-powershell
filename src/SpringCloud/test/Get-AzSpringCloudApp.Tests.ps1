if(($null -eq $TestName) -or ($TestName -contains 'Get-AzSpringCloudApp'))
{
...
Describe 'Get-AzSpringCloudApp' {
    It 'List'  {
        $appList = Get-AzSpringCloudApp -ResourceGroupName $env.resourceGroup -ServiceName $env.standardSpringName01
