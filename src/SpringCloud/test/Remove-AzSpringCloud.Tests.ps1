if(($null -eq $TestName) -or ($TestName -contains 'Remove-AzSpringCloud'))
{
...
Describe 'Remove-AzSpringCloud' {
    It 'Delete' {
        Remove-AzSpringCloud -ResourceGroupName $env.resourceGroup -Name $env.springName01
        $springList = Get-AzSpringCloud -ResourceGroupName $env.resourceGroup
        $springList.Name| Should -Not -Contain $env.springName02
    }

    It 'DeleteViaIdentity' {
        $spring = New-AzSpringCloud -ResourceGroupName $env.resourceGroup -Name $env.springName02 -Location $env.location2
