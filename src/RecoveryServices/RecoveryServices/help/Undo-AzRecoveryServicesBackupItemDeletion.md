...
### Example 3: Rehydrate a soft-deleted Azure File Share item
...
The first command gets the storage container that holds the file share.
The second command fetches the soft-deleted file share backup item.
The last command rehydrates the file share to a `ProtectionStopped` state where the recovery points are retained forever (until the user disables soft delete or explicitly removes the item).
