            ...
            ExecutionBlock(() =>
            {
                try
                {
                    ...
                    if (SoftDeleteFeatureState != null || DisableHybridBackupSecurityFeature != null || SoftDeleteRetentionPeriodInDays != null)
                    {
                        ...
                        param.Properties.SoftDeleteRetentionPeriodInDays = SoftDeleteRetentionPeriodInDays ?? currentConfig.Properties.SoftDeleteRetentionPeriodInDays; // to: Nov24 - should we get from vault API / MUA angle ?

                        if(SoftDeleteFeatureState !=null && SoftDeleteFeatureState.ToLower() == "alwayson")
                        {
                            if(DisableHybridBackupSecurityFeature != null)
                            {
                                throw new ArgumentException(Resources.InvalidSoftDeleteFeatureStateException);
                            }
                            param.Properties.EnhancedSecurityState = "AlwaysON";
                        }
                        else
                        {
                        ...
                        BackupResourceVaultConfigResource result = ServiceClientAdapter.SetVaultProperty(vaultName, resourceGroupName, param, Token, isMUAProtected); // todo: Nov24 - should call vault API ?
