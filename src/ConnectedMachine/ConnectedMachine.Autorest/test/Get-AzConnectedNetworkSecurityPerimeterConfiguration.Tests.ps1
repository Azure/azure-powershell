...
Describe 'Get-AzConnectedNetworkSecurityPerimeterConfiguration' {
    ...
    It 'Get' -skip{
        $all = @(Get-AzConnectedNetworkSecurityPerimeterConfiguration -PerimeterName $env.PerimeterName -ResourceGroupName $env.ResourceGroupName -ScopeName $env.PrivateLinkScopeName)
        $all.Count | Should -Not -BeNullOrEmpty
    }
}
