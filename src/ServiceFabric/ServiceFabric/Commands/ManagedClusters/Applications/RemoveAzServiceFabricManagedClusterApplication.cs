...
{
    [Cmdlet(VerbsCommon.Remove, ResourceManager.Common.AzureRMConstants.AzurePrefix + Constants.ServiceFabricPrefix + "ManagedClusterApplication", DefaultParameterSetName = ByResourceGroup, SupportsShouldProcess = true), OutputType(typeof(bool))]
    public class RemoveAzServiceFabriManagedClusterApplication : ManagedApplicationCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            var resourceMessage = $"Managed Application '{this.Name}' in resource group '{this.ResourceGroupName}', cluster name {this.ClusterName}";
            if (ShouldProcess(target: this.Name, action: $"Remove {resourceMessage}"))
            {
                ConfirmAction(Force.IsPresent,
                    $"Do you want to remove the {resourceMessage}? This will remove all managed services under this resource",
                    "Removing managed application.",
                    resourceMessage,
                    () =>
                    {
                        try
