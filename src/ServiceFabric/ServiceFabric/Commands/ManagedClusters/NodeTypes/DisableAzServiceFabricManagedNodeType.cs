...
namespace Microsoft.Azure.Commands.ServiceFabric.Commands
{
    ...
    public class DisableAzServiceFabricManagedNodeType : ServiceFabricManagedCmdletBase
    {
        ...
        {
            if (ShouldProcess(target: this.Name, action: string.Format("Disable node(s) {0}, from node type {1} on cluster {2}", string.Join(", ", this.NodeName), this.Name, this.ClusterName)))
            {
                try
                {
