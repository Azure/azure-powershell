...
```
New-AzTrafficManagerProfile -ProfileName <String> -ResourceGroupName <String> -SubscriptionId <String>
...
### CreateViaIdentity
```
New-AzTrafficManagerProfile -InputObject <ITrafficManagerIdentity> -Parameters <IProfile> [-Confirm] [-WhatIf]
 [<CommonParameters>]
```

### CreateViaIdentityExpanded
```
New-AzTrafficManagerProfile -InputObject <ITrafficManagerIdentity> [-AllowedEndpointRecordTypes <String[]>]
...
### Example 1: Create a new Traffic Manager profile with Performance routing
```powershell
New-AzTrafficManagerProfile -ProfileName "myprofile" -ResourceGroupName "myRG" -TrafficRoutingMethod "Performance" -DnsConfigRelativeName "myprofile" -DnsConfigTtl 30 -MonitorConfigProtocol "HTTPS" -MonitorConfigPort 443 -MonitorConfigPath "/health" -ProfileStatus "Enabled"
```
...
Creates a new Traffic Manager profile with Performance traffic routing method and HTTPS health monitoring.

### Example 2: Create a profile with Weighted routing
```powershell
New-AzTrafficManagerProfile -ProfileName "weighted-profile" -ResourceGroupName "myRG" -TrafficRoutingMethod "Weighted" -DnsConfigRelativeName "weighted-profile" -DnsConfigTtl 60 -MonitorConfigProtocol "HTTP" -MonitorConfigPort 80 -MonitorConfigPath "/"
```

Creates a Traffic Manager profile using the Weighted traffic routing method.
