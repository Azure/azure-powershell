if(($null -eq $TestName) -or ($TestName -contains 'New-AzPrivateTrafficManagerTopologyMap'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'New-AzPrivateTrafficManagerTopologyMap.Recording.json'
...
Describe 'New-AzPrivateTrafficManagerTopologyMap' {
    It 'CreateExpanded - should have correct parameter set' {
        $cmd = Get-Command New-AzPrivateTrafficManagerTopologyMap
