if(($null -eq $TestName) -or ($TestName -contains 'Remove-AzPrivateTrafficManagerEndpoint'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Remove-AzPrivateTrafficManagerEndpoint.Recording.json'
...
Describe 'Remove-AzPrivateTrafficManagerEndpoint' {
    It 'Delete - should have correct parameter set' {
        $cmd = Get-Command Remove-AzPrivateTrafficManagerEndpoint
