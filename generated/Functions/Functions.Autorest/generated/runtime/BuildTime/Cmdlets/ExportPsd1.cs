    ...
    public class ExportPsd1 : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                sb.AppendLine($@"{Indent}RootModule = '{"./Az.Functions.psm1"}'");
                ...
                {
                    sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.Functions.private.dll"}'");
                }
                ...
                else
                {
                    ...
                    var formatList = customFormatPs1xmlFiles.Prepend("./Az.Functions.format.ps1xml").ToPsList();
                    sb.AppendLine($@"{Indent}FormatsToProcess = {formatList}");
                }

                // TypesToProcess
                if (!IsUndefined("'./custom/Functions.types.ps1xml'"))
                {
                    sb.AppendLine($@"{Indent}TypesToProcess = @({"'./custom/Functions.types.ps1xml'"})");
                }

                // ScriptsToProcess
                if (!IsUndefined("'./custom/HelperFunctions.ps1'"))
                {
                    sb.AppendLine($@"{Indent}ScriptsToProcess = @({"'./custom/HelperFunctions.ps1'"})");
                }
