// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License. See License.txt in the project root for license information.
// Code generated by Microsoft (R) AutoRest Code Generator.
// Changes may cause incorrect behavior and will be lost if the code is regenerated.

namespace Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models
{
    using static Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.Extensions;

    /// <summary>Query results.</summary>
    public partial class PolicyEventsQueryResults :
        Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEventsQueryResults,
        Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEventsQueryResultsInternal
    {

        /// <summary>Backing field for <see cref="OdataContext" /> property.</summary>
        private string _odataContext;

        /// <summary>
        /// OData context string; used by OData clients to resolve type information based on metadata.
        /// </summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Origin(Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.PropertyOrigin.Owned)]
        public string OdataContext { get => this._odataContext; set => this._odataContext = value; }

        /// <summary>Backing field for <see cref="OdataCount" /> property.</summary>
        private int? _odataCount;

        /// <summary>OData entity count; represents the number of policy event records returned.</summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Origin(Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.PropertyOrigin.Owned)]
        public int? OdataCount { get => this._odataCount; set => this._odataCount = value; }

        /// <summary>Backing field for <see cref="OdataNextLink" /> property.</summary>
        private string _odataNextLink;

        /// <summary>Odata next link; URL to get the next set of results.</summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Origin(Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.PropertyOrigin.Owned)]
        public string OdataNextLink { get => this._odataNextLink; set => this._odataNextLink = value; }

        /// <summary>Backing field for <see cref="Value" /> property.</summary>
        private System.Collections.Generic.List<Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEvent> _value;

        /// <summary>Query results.</summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Origin(Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.PropertyOrigin.Owned)]
        public System.Collections.Generic.List<Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEvent> Value { get => this._value; set => this._value = value; }

        /// <summary>Creates an new <see cref="PolicyEventsQueryResults" /> instance.</summary>
        public PolicyEventsQueryResults()
        {

        }
    }
    /// Query results.
    public partial interface IPolicyEventsQueryResults :
        Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.IJsonSerializable
    {
        /// <summary>
        /// OData context string; used by OData clients to resolve type information based on metadata.
        /// </summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.Info(
        Required = false,
        ReadOnly = false,
        Read = true,
        Create = true,
        Update = true,
        Description = @"OData context string; used by OData clients to resolve type information based on metadata.",
        SerializedName = @"@odata.context",
        PossibleTypes = new [] { typeof(string) })]
        string OdataContext { get; set; }
        /// <summary>OData entity count; represents the number of policy event records returned.</summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.Info(
        Required = false,
        ReadOnly = false,
        Read = true,
        Create = true,
        Update = true,
        Description = @"OData entity count; represents the number of policy event records returned.",
        SerializedName = @"@odata.count",
        PossibleTypes = new [] { typeof(int) })]
        int? OdataCount { get; set; }
        /// <summary>Odata next link; URL to get the next set of results.</summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.Info(
        Required = false,
        ReadOnly = false,
        Read = true,
        Create = true,
        Update = true,
        Description = @"Odata next link; URL to get the next set of results.",
        SerializedName = @"@odata.nextLink",
        PossibleTypes = new [] { typeof(string) })]
        string OdataNextLink { get; set; }
        /// <summary>Query results.</summary>
        [Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.Info(
        Required = false,
        ReadOnly = false,
        Read = true,
        Create = true,
        Update = true,
        Description = @"Query results.",
        SerializedName = @"value",
        PossibleTypes = new [] { typeof(Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEvent) })]
        System.Collections.Generic.List<Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEvent> Value { get; set; }

    }
    /// Query results.
    internal partial interface IPolicyEventsQueryResultsInternal

    {
        /// <summary>
        /// OData context string; used by OData clients to resolve type information based on metadata.
        /// </summary>
        string OdataContext { get; set; }
        /// <summary>OData entity count; represents the number of policy event records returned.</summary>
        int? OdataCount { get; set; }
        /// <summary>Odata next link; URL to get the next set of results.</summary>
        string OdataNextLink { get; set; }
        /// <summary>Query results.</summary>
        System.Collections.Generic.List<Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Models.IPolicyEvent> Value { get; set; }

    }
}