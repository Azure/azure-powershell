...
using static Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.PowerShell.PsHelpers;

namespace Microsoft.Azure.PowerShell.Cmdlets.PolicyInsights.Runtime.PowerShell
{
    ...
    public class ExportPsd1 : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                string version = Convert.ToString(@"2.0.0");
                ...
                sb.AppendLine($@"{Indent}RootModule = '{"./Az.PolicyInsights.psm1"}'");
                ...
                sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: PolicyInsights cmdlets"}'");
                ...
                {
                    sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.PolicyInsights.private.dll"}'");
                }
                ...
                else
                {
                    ...
                    var formatList = customFormatPs1xmlFiles.Prepend("./Az.PolicyInsights.format.ps1xml").ToPsList();
                ...
                // ScriptsToProcess
                if (!IsUndefined("'./custom/Helpers.ps1'"))
                {
                    sb.AppendLine($@"{Indent}ScriptsToProcess = @({"'./custom/Helpers.ps1'"})");
                }
                ...
                sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule PolicyInsights".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
