...
using static Microsoft.Azure.PowerShell.Cmdlets.Monitor.ScheduledQueryRule.Runtime.PowerShell.PsHelpers;
    ...
    public class ExportPsd1 : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                sb.AppendLine($@"{Indent}RootModule = '{"./Az.ScheduledQueryRule.psm1"}'");
                ...
                sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: ScheduledQueryRule cmdlets"}'");
                ...
                {
                    sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.ScheduledQueryRule.private.dll"}'");
                }
                ...
                else
                {
                    ...
                    var formatList = customFormatPs1xmlFiles.Prepend("./Az.ScheduledQueryRule.format.ps1xml").ToPsList();
                ...
                sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule ScheduledQueryRule".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
