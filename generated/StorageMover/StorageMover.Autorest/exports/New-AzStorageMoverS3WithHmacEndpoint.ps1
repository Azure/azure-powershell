...
Creates an S3 with HMAC endpoint resource, which represents a data transfer source or destination.
.Example
New-AzStorageMoverS3WithHmacEndpoint -Name "myendpoint" -ResourceGroupName "myresourcegroup" -StorageMoverName "mystoragemover" -SourceUri "https://s3.example.com/bucket" -SourceType "MINIO" -CredentialsAccessKeyUri "https://examples-azureKeyVault.vault.azure.net/secrets/accesskey" -CredentialsSecretKeyUri "https://examples-azureKeyVault.vault.azure.net/secrets/secretkey"
...
begin {
    try {
        ...
        $mapping = @{
            CreateExpanded = 'Az.StorageMover.custom\New-AzStorageMoverS3WithHmacEndpoint';
        }
