...
$modulePsd1 = Get-Item -Path (Join-Path $PSScriptRoot './Az.Discovery.psd1')
...
{
  if ($TestMode -ne 'playback' -and $setupCompleted)
  {
    cleanupEnv
  }
  elseif ($TestMode -ne 'playback' -and -not $setupCompleted)
  {
    Write-Host -ForegroundColor Yellow "setupEnv failed — skipping cleanupEnv to preserve resources"
  }
  else {
