...
using static Microsoft.Azure.PowerShell.Cmdlets.MarketplaceRpService.Runtime.PowerShell.PsHelpers;
    ...
    public class ExportPsd1 : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                sb.AppendLine($@"{Indent}RootModule = '{"./Az.MarketplaceRpService.psm1"}'");
                ...
                sb.AppendLine($@"{Indent}Description = '{"Microsoft Azure PowerShell: MarketplaceRpService cmdlets"}'");
                ...
                {
                    sb.AppendLine($@"{Indent}RequiredAssemblies = '{"./bin/Az.MarketplaceRpService.private.dll"}'");
                }
                ...
                else
                {
                    ...
                    var formatList = customFormatPs1xmlFiles.Prepend("./Az.MarketplaceRpService.format.ps1xml").ToPsList();
                ...
                sb.AppendLine($@"{Indent}{Indent}{Indent}Tags = {"Azure ResourceManager ARM PSModule MarketplaceRpService".Split(' ').ToPsList().NullIfEmpty() ?? "''"}");
