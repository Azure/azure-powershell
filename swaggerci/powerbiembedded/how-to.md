# How-To
This document describes how to develop for `Az.PowerBiEmbedded`.

## Building `Az.PowerBiEmbedded`
...
There are multiple scripts created for performing different actions for developing `Az.PowerBiEmbedded`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.PowerBiEmbedded.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.PowerBiEmbedded.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.PowerBiEmbedded`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.PowerBiEmbedded` into the session.
