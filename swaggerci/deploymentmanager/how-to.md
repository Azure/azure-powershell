# How-To
This document describes how to develop for `Az.DeploymentManager`.

## Building `Az.DeploymentManager`
...
There are multiple scripts created for performing different actions for developing `Az.DeploymentManager`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DeploymentManager.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DeploymentManager.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DeploymentManager`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DeploymentManager` into the session.
