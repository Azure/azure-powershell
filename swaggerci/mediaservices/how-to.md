# How-To
This document describes how to develop for `Az.MediaServices`.

## Building `Az.MediaServices`
...
There are multiple scripts created for performing different actions for developing `Az.MediaServices`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.MediaServices.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.MediaServices.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.MediaServices`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.MediaServices` into the session.
