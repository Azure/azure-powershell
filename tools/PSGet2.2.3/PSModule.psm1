...
function Publish-PSArtifactUtility {
    ...
    if ($PSScriptInfo) {
        ...
        if ($PSBoundParameters.ContainsKey('AllowPrerelease')) {
            $GetModuleDependencies_Params.Add('AllowPrerelease', $true)
        }
    ...
    ForEach ($Dependency in $DependentModuleDetails) {
        ...
        # If found module is preview version, we have to specify as [x.x.x-preview], otherwise Install-Module will fail.
        if($Dependency.Keys -Contains "FoundVersion" -and $Dependency.FoundVersion.Contains("-")){
            $VersionString = "[$($Dependency.FoundVersion)]"
        } elseif ($Dependency.Keys -Contains "RequiredVersion") {
...
function ValidateAndGet-RequiredModuleDetails
{
    ...
    if($ModuleManifestRequiredModules)
    {
        ForEach($RequiredModule in $ModuleManifestRequiredModules)
        {
            ...
            # One possible enhancement: try find stable module first, otherwise try to find preview version
            if ($PSBoundParameters.ContainsKey('AllowPrerelease'))
            {
                $FindModuleArguments.Add('AllowPrerelease',$true)
            }
            ...
            if(-not $psgetItemInfo)
            {
                ...
                $ReqModuleInfo["FoundVersion"] = $psgetItemInfo.Version
            }
...
function Publish-InternalModule {
    <#
    .ExternalHelp PSModule-help.xml
    #>
    ...
    (
        ...
        [Parameter(ParameterSetName = "ModulePathParameterSet")]
        [Parameter(ParameterSetName = "ModuleNameParameterSet")]
        [switch]
        $AllowPrerelease,
    ...
    Process {
        ...
        try {
            ...
            if ($Force -or $PSCmdlet.ShouldProcess($shouldProcessMessage, "Publish-InternalModule")) {
                ...
                if($PSBoundParameters.Containskey('AllowPrerelease')){
                    $PublishPSArtifactUtility_Params.Add('AllowPrerelease', $true)
                }
...
try {
    if (Get-Command -Name Register-ArgumentCompleter -ErrorAction SilentlyContinue) {
        Register-ArgumentCompleter -CommandName Publish-InternalModule -ParameterName Name -ScriptBlock {
