...
function Get-AssemblyName
{
    ...
    {
        return $MatchedAssembly.Trim().Trim("<AssemblyName>").Trim("</AssemblyName>")
    }
}
...
function Add-SolutionReference
{
    ...
    $SolutionFiles = Get-ChildItem -Path $ServiceFolderPath -Filter "*.sln" | where { $_.FulLName -notlike "*Stack*" -and $_.FullName -notlike "*Netcore*" }
    if ($SolutionFiles -ne $null)
    {
        $Values = $SolutionFiles | % { $_.FullName }
...
function Create-ModuleMappings
{
    ...
    foreach ($ServiceFolder in $FilteredServiceFolders)
    {
        ...
        {
            $Value = @()
            $ModuleManifestFiles | % { $Value += $_.Name }
            $Mappings[$Key] = $Value
        }
    }
...
function Add-SolutionMappings
{
    ...
    if ($SolutionFiles -ne $null)
    {
        ...
        foreach ($SolutionFile in $SolutionFiles)
        {
            $Script:SolutionToProjectMappings[$SolutionFile.FullName] | % { $Projects.Add($_) | Out-Null }
        }
