...
foreach ($path in $outputPaths)
{
    ...
    Get-ChildItem -Include $resourcesFolders -Recurse -Force -Path $path | ForEach-Object { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force -Recurse -ErrorAction SilentlyContinue }

    Write-Verbose "Removing autogenerated XML help files, code analysis, config files, and symbols."
    Get-ChildItem -Include $removeItems -Exclude $keepItems -Recurse -Path $path | ForEach-Object { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force -Recurse }
    Get-ChildItem -Recurse -Path $path -Include *.dll-Help.psd1 | ForEach-Object { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force }

    Write-Verbose "Removing markdown help files and folders"
    Get-ChildItem -Recurse -Path $path -Include *.md | ForEach-Object { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force -Confirm:$false }
    Get-ChildItem -Directory -Include help -Recurse -Path $path | ForEach-Object { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force -Recurse -Confirm:$false -ErrorAction "Ignore" }

    Write-Verbose "Removing unneeded web deployment dependencies"
    Get-ChildItem -Include $webdependencies -Recurse -Path $path | ForEach-Object { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force }
}
...
foreach($RMPath in $resourceManagerPaths)
{
    ...
    foreach ($RMFolder in $resourceManagerFolders)
    {
        ...
        $removedDlls = Get-ChildItem -Path $RMFolder.FullName -Filter "*.dll" -Recurse | Where-Object { $acceptedDlls -notcontains $_.Name -and !$_.FullName.Contains("Assemblies") }
        ...
        $removedDlls | % { Write-Host "[!] Removing $($_.FullName)"; Remove-Item $_.FullName -Force }
