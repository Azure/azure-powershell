...
namespace Tools.Common.Models
{
    ...
    public class TypeMetadata
    {
        ...
        private List<ConstructorMetadata> _constructors = new List<ConstructorMetadata>();
        ...
        [JsonIgnore]
        public List<MethodMetadata> Methods { get { return _methods; } }

        [JsonIgnore]
        public List<ConstructorMetadata> Constructors { get { return _constructors; } }
        ...
        public override bool Equals(Object obj)
        {
            ...
            flag &= string.Equals(this.ElementType, other.ElementType, StringComparison.OrdinalIgnoreCase);
            this.GenericTypeArguments.ForEach(t => flag &= other.GenericTypeArguments.Contains(t));
            flag &= this.GenericTypeArguments.Count == other.GenericTypeArguments.Count;

            ModuleMetadata moduleMetadata = CmdletLoader.ModuleMetadata;

            foreach (var thisPropertyName in this.Properties.Keys)
            {
                var otherPropertyFullName = other.Properties[thisPropertyName];
                ...
                flag &= thisPropertyTypeMetadata.Equals(otherPropertyTypeMetadata);
            }

            flag &= this.Properties.Keys.Count == other.Properties.Keys.Count;

            // Compare methods
            if (this.Methods.Count > 0)
            {
                if (this.Methods.Count != other.Methods.Count)
                {
                    return false;
                }

                foreach (var thisMethod in this.Methods)
                {
                    bool found = false;
                    var candidateMethods = other.Methods.Where(m => string.Equals(m.Name, thisMethod.Name)).ToList();
            ...
            {
                if (this.Constructors.Count != other.Constructors.Count)
                {
                    return false;
                }

                foreach (var thisConstructor in this.Constructors)
                {
                    bool found = false;
                    foreach (var otherConstructor in other.Constructors)
                    {
                        var thisParameters = thisConstructor.Parameters;
                        var otherParameters = otherConstructor.Parameters;
                        if (thisParameters.Count != otherParameters.Count)
                        {
                            continue;
                        }
